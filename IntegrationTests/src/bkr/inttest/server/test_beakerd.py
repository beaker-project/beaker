...
from bkr.inttest.assertions import (
...
class TestBeakerd(DatabaseTestCase):
    def setUp(self):
        with session.begin():
            ...
            running = Recipe.query.filter(
                not_(Recipe.status.in_([s for s in TaskStatus if s.finished]))
            )
        ...
        with session.begin():
            ...
            shutil.copyfile(
                pkg_resources.resource_filename(
                    "bkr.inttest.server", "task-rpms/empty.rpm"
                ),
                rpm_path,
            )
    ...
    def test_failed_recipe_started_before_upgrade_finished_after_upgrade_does_not_fail(
        self,
    ):
        with session.begin():
        ...
        with session.begin():
            ...
            data_setup.mark_recipe_tasks_finished(
                recipe, only=True, result=TaskResult.fail
            )
    ...
    def test_host_uses_latest_guest(self):
        ...
        with session.begin():
            ...
            older_dt.date_created = datetime.datetime.utcnow() - datetime.timedelta(
                hours=10
            )
        ...
        with session.begin():
            ...
            self.assertEqual(
                host_recipe.resource.system.lab_controller.id,
                host_lab_controller2.id,
                host_recipe.resource.system.lab_controller,
            )
    ...
    def test_host_and_guest_no_common_lab_controllers_stay_queued(self):
        ...
        with session.begin():
            ...
            dt_for_host = data_setup.create_distro_tree(
                lab_controllers=[host_lab_controller]
            )
            dt_for_guest = data_setup.create_distro_tree(
                lab_controllers=[guest_lab_controller]
            )
        ...
        with session.begin():
            ...
            dt_for_guest.lab_controller_assocs.append(
                LabControllerDistroTree(
                    lab_controller=host_lab_controller, url=u"http://whatevs.com"
                )
            )
    ...
    def test_loaned_machine_can_be_scheduled(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                status=u"Automated", shared=False, lab_controller=self.lab_controller
            )
            # User is denied by the policy, but system is loaned to the user
            self.assertFalse(
                system.custom_access_policy.grants(user, SystemPermission.reserve)
            )
            system.loaned = user
    ...
    def test_reservations_are_created(self):
        ...
        with session.begin():
            ...
            assert_datetime_within(
                system.reservations[0].start_time,
    ...
    def test_empty_and_element(self):
        with session.begin():
            ...
            job.recipesets[0].recipes[
                0
            ]._host_requires = u"<hostRequires><and></and></hostRequires>"
    ...
    def test_or_lab_controller(self):
        with session.begin():
            ...
            system1 = data_setup.create_system(
                arch=u"i386", shared=True, lab_controller=lc1
            )
            system2 = data_setup.create_system(
                arch=u"i386", shared=True, lab_controller=lc2
            )
            system3 = data_setup.create_system(
                arch=u"i386", shared=True, lab_controller=lc3
            )
    ...
    def test_nonshared_system_owner(self):
        with session.begin():
            user = data_setup.create_user()
            system = data_setup.create_system(
                lab_controller=self.lab_controller, shared=False, owner=user
            )
    ...
    def test_nonshared_system_admin(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                lab_controller=self.lab_controller, shared=False
            )
    ...
    def test_shared_system_not_owner(self):
        with session.begin():
            user = data_setup.create_user()
            system = data_setup.create_system(
                lab_controller=self.lab_controller, shared=True
            )
        self.check_user_can_run_job_on_system(user, system)
    ...
    def test_loaned_system_with_admin(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                lab_controller=self.lab_controller, shared=True, loaned=loanee
            )
            admin = data_setup.create_admin()
    ...
    def test_disabled_lab_controller(self):
        with session.begin():
            system = data_setup.create_system(
                status=u"Automated", shared=True, lab_controller=self.lab_controller
            )
    ...
    def test_harness_repo_not_required_contained_harness(self):
        with session.begin():
            distro_tree = data_setup.create_distro_tree(
                osmajor=u"MyAwesomeNewLinux", harness_dir=False
            )
    ...
    def test_single_processor_priority(self):
        ...
        with session.begin():
            ...
            recipe2._host_requires = (
                '<hostRequires><cpu_count op="=" value="1"/></hostRequires>'
            )
    ...
    def test_task_versions_are_recorded(self):
        with session.begin():
            ...
            recipe = data_setup.create_recipe(distro_tree=distro_tree, task_list=[task])
            recipe._host_requires = (
                u"""
                <hostRequires>
                    <hostname op="=" value="%s" />
        ...
        with session.begin():
            job = Job.query.get(job.id)
            self.assertEquals(
                job.recipesets[0].recipes[0].tasks[0].version, task.version
            )
    ...
    def test_recipe_install_options_can_remove_system_options(self):
        with session.begin():
            ...
            system.provisions[distro_tree.arch] = Provision(
                arch=distro_tree.arch, kernel_options="console=ttyS0 vnc"
            )
            job = data_setup.create_job(distro_tree=distro_tree)
        ...
        with session.begin():
            ...
            self.assertEqual(job.status, TaskStatus.waiting)
            self.assertNotIn(
                "vnc", job.recipesets[0].recipes[0].installation.kernel_options
            )
    ...
    def test_kernel_options_are_not_quoted(self):
        ...
        with session.begin():
            ...
            self.assertEqual(job.status, TaskStatus.waiting)
            self.assertIn(
                bad_arg, job.recipesets[0].recipes[0].installation.kernel_options
            )
        ...
        with session.begin():
            ...
            for x in range(0, 3):
                data_setup.create_system(
                    shared=True, owner=user, lab_controller=lab_controller
                )
...
<job retention_tag="scratch">
	...
	<recipeSet priority="Normal">
        ...
        with session.begin():
            job = Job.query.get(job.id)
            for x in range(0, 2):
                self.assertEqual(job.recipesets[x].recipes[0].status, TaskStatus.queued)
            for x in range(2, 3):
                self.assertEqual(
                    job.recipesets[x].recipes[0].status, TaskStatus.scheduled
                )
            for x in range(5, 3):
                self.assertEqual(job.recipesets[x].recipes[0].status, TaskStatus.queued)
    ...
    def test_provision_broken_manual_systems(self):
        with session.begin():
            ...
            job1.recipesets[0].recipes[0]._host_requires = host_requires.format(
                system1.fqdn
            )
            job2.recipesets[0].recipes[0]._host_requires = host_requires.format(
                system2.fqdn
            )
            job3.recipesets[0].recipes[0]._host_requires = host_requires.format(
                system3.fqdn
            )
        ...
        with session.begin():
            ...
            self.assertEqual(job.recipesets[0].recipes[0].status, TaskStatus.scheduled)
            ...
            result = RecipeTaskResult.query.filter(
                RecipeTaskResult.recipe_task_id == recipetask_id
            ).one()
            self.assertEquals(
                result.log,
    ...
    def test_recipes_with_force_are_queued(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                status=u"Manual", shared=True, lab_controller=self.lab_controller
            )
    ...
    def test_force_ignores_excluded_families(self):
        with session.begin():
            ...
            self.assertFalse(
                system.compatible_with_distro_tree(
                    arch=distro_tree.arch,
    ...
    def test_recipe_state_reserved(self):
        ...
        with session.begin():
            ...
            assert_datetime_within(
                job.recipesets[0].recipes[0].watchdog.kill_time,
                tolerance=datetime.timedelta(seconds=10),
                reference=datetime.datetime.utcnow() + datetime.timedelta(seconds=3600),
            )
    ...
    def test_recipe_is_not_reserved_when_no_systems_match(self):
        ...
        with session.begin():
            ...
            recipe._host_requires = u'<hostRequires><hostname op="=" value="Ineverexisted.fqdn"/></hostRequires>'
            job = data_setup.create_job_for_recipes([recipe])
    ...
    def test_bz1120052(self):
        ...
        with session.begin():
            ...
            job.recipesets[0].recipes[0]._host_requires = (
                '<hostRequires><hostname value="%s"/></hostRequires>' % system_a.fqdn
            )
            job.recipesets[0].recipes[1]._host_requires = (
                '<hostRequires><hostname value="%s"/></hostRequires>' % system_b.fqdn
            )
    ...
    def test_priority_is_bumped_when_recipe_matches_one_system(self):
        with session.begin():
            ...
            job = data_setup.create_job_for_recipes([recipe], priority=TaskPriority.low)
        beakerd.process_new_recipes()
        beakerd.update_dirty_jobs()
        with session.begin():
            job = Job.query.get(job.id)
    ...
    def test_installation_table_parameters_filled_out_at_provisioning_time(self):
        with session.begin():
            ...
            distro_tree = data_setup.create_distro_tree(
                distro_name=u"MyUniqueLinux5-5", lab_controllers=[lc]
            )
            user = data_setup.create_user(
                user_name=u"test-job-owner", email_address=u"test-job-owner@example.com"
            )
    ...
    # https://bugzilla.redhat.com/show_bug.cgi?id=911515
    def test_provision_does_not_fail_when_kickstart_element_used_with_custom_distro(
        self,
    ):
        with session.begin():
            lc = data_setup.create_labcontroller()
        ...
        with session.begin():
            ...
            recipe = job.recipesets[0].recipes[0]
        self.assertIn(
            "Awesome kickstart stuff", recipe.installation.rendered_kickstart.kickstart
        )
    ...
    def test_user_defined_distro_provisioning_uses_correct_exclusions_and_kernel_options(
        self,
    ):
        with session.begin():
            ...
            excluded_system = data_setup.create_system(
                lab_controller=lc, exclude_osmajor=[osmajor]
            )
            included_system = data_setup.create_system(lab_controller=lc)
            arch = Arch.by_name("i386")
            included_system.provisions[arch] = Provision(
                arch=arch, kernel_options="anwesha"
            )
            recipe = data_setup.create_recipe(
                custom_distro=True,
                osmajor="DansAwesomeLinux6",
                arch="i386",
                task_list=[Task.by_name(u"/distribution/check-install")] * 2,
            )
            job = data_setup.create_job_for_recipes([recipe])
        ...
        with session.begin():
            ...
            self.assertIn(
                "anwesha", job.recipesets[0].recipes[0].installation.kernel_options
            )
    ...
    def test_recipesets_provisioned_when_guest_recipe_is_user_defined(self):
        with session.begin():
            ...
            guest_recipe = data_setup.create_guestrecipe(
                host=host_recipe,
                custom_distro=True,
                osmajor="DansAwesomeLinux6",
                arch="i386",
    ...
    def test_recipesets_provisioned_when_host_recipe_is_user_defined(self):
        with session.begin():
            lc = data_setup.create_labcontroller()
            system = data_setup.create_system(lab_controller=lc)
            host_recipe = data_setup.create_recipe(
                custom_distro=True,
                osmajor="DansAwesomeLinux6",
                task_list=[Task.by_name(u"/distribution/check-install")] * 2,
            )
            guest_recipe = data_setup.create_guestrecipe(
                host=host_recipe, custom_distro=True
            )
            job = data_setup.create_job_for_recipes([host_recipe, guest_recipe])
            session.flush()
            job_id = job.id
    ...
    def test_beakerd_aborts_recipe_when_provision_fails(self, recipe_mock):
        ...
        with session.begin():
            ...
            msg = (
                u"Failed to provision recipeid %s, 'NoneType' object has no attribute 'distro_name'"
                % r2.id
            )
...
class TestProvisionVirtRecipes(DatabaseTestCase):
    def setUp(self):
        ...
        with session.begin():
            ...
            # OpenStack.
            System.query.filter(System.status == SystemStatus.automated).update(
                dict(status=SystemStatus.removed), synchronize_session=False
            )
    ...
    def test_openstack_instance_created(self):
        ...
        with session.begin():
            recipe = Recipe.query.get(self.recipe.id)
            self.assertIsNotNone(recipe, "Failed to get recipe ID %s" % self.recipe.id)
    ...
    def test_cheapest_OpenStack_flavor_should_be_picked(self):
        ...
        with session.begin():
            ...
            instance_flavor = self.virt_manager.novaclient.flavors.get(
                instance.flavor["id"]
            )
    ...
    def test_floating_ip_is_assigned(self):
        ...
        with session.begin():
            ...
            self.assertEquals(
                fips["floatingips"][0]["floating_ip_address"],
    ...
    def test_cleanup_openstack(self):
        ...
        try:
            ...
            self.assertIn(
                "Instance %s could not be found" % recipe.resource.instance_id,
                e.message,
            )
        if self.virt_manager.is_create_floating_ip:
            ...
            self.assertFalse(
                self.virt_manager.neutronclient.list_floatingips(
                    floating_ip_address=recipe.resource.floating_ip
                )["floatingips"]
            )
...
class TestBeakerdMetrics(DatabaseTestCase):
    ...
    def test_system_count_metrics(self, mock_metrics):
        ...
        expected = dict(("%s.%s" % (g, c), 0) for g in gauges for c in categories)
        for arch in u"i386 x86_64 ppc ppc64".split():
            data_setup.create_system(lab_controller=lc, arch=arch)
            data_setup.create_system(
                lab_controller=lc, arch=arch, status=SystemStatus.removed
            )
            categories = [
                "all",
                "shared",
        ...
        data_setup.create_system(lab_controller=lc, status=SystemStatus.removed)
        session.flush()
    ...
    def test_system_count_metrics_uses_active_access_policy(self, mock_metrics):
        ...
        session.flush()
        beakerd.system_count_metrics()
        mock_metrics.measure.assert_any_call("gauges.systems_idle_automated.shared", 2)
    ...
    def test_system_command_metrics(self, mock_metrics):
        ...
        categories = [
            "all",
            "by_lab.testcommandmetrics_invalid",
            "by_arch.x86_64",
            "by_power_type.drac",
        ]
