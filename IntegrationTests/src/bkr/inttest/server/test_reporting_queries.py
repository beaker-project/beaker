...
class ReportingQueryTest(DatabaseTestCase):
    ...
    def execute_reporting_query(self, name):
        sql = pkg_resources.resource_string(
            "bkr.server", "reporting-queries/%s.sql" % name
        )
    ...
    def test_wait_duration_by_resource(self):
        ...
        data_setup.mark_recipe_complete(
            system_recipe2, system=system_recipe.resource.system
        )
        ...
        virt_recipe.resource.recipe.start_time = (
            virt_recipe.resource.recipe.recipeset.queue_time + one_hour
        )
        virt_recipe2.resource.recipe.start_time = (
            virt_recipe2.resource.recipe.recipeset.queue_time + two_hours
        )

        system_recipe.resource.recipe.start_time = (
            system_recipe.resource.recipe.recipeset.queue_time + one_hour
        )
        system_recipe2.resource.recipe.start_time = (
            system_recipe2.resource.recipe.recipeset.queue_time + three_hours
        )
        session.flush()
        ...
        system_rows = [
            row
            for row in all_rows
            if row.fqdn in (system_recipe.resource.fqdn, system_recipe2.resource.fqdn)
        ]
    ...
    def test_install_duration_by_resource(self):
        ...
        virt_recipe.installation.install_finished = (
            virt_recipe.installation.install_started + one_hour
        )
        virt_recipe2.installation.install_finished = (
            virt_recipe2.installation.install_started + two_hours
        )

        guest_recipe.installation.install_finished = (
            guest_recipe.installation.install_started + two_hours
        )
        guest_recipe2.installation.install_finished = (
            guest_recipe2.installation.install_started + three_hours
        )

        system_recipe.installation.install_finished = (
            system_recipe.installation.install_started + one_hour
        )
        system_recipe2.installation.install_finished = (
            system_recipe2.installation.install_started + three_hours
        )
        session.flush()
    ...
    def test_machine_utilization(self):
        ...
        data_setup.create_manual_reservation(
            system=system2, start=datetime.datetime(2002, 5, 2, 22, 30, 0)
        )
        data_setup.create_manual_reservation(
            system=system3,
    ...
    def test_job_priority_changes(self):
        ...
        for j in [job1, job2, job3]:
            for rs in j.recipesets:
                ...
                activity.created = datetime.datetime(year=2012, month=10, day=10)
    ...
    def test_systems_by_cpu_type(self):
        ...
        rare_cpu_stepping = 4
        for _ in range(2):
            data_setup.create_system().cpu = Cpu(
                vendor=vendor,
                model=1,
        ...
        data_setup.create_system(status=SystemStatus.removed).cpu = Cpu(vendor=vendor)
    ...
    def test_systems_by_vendor(self):
        ...
        data_setup.create_system(status=SystemStatus.removed).vendor = common_vendor
        data_setup.create_system().vendor = None  # un-inventoried
        session.flush()
        system_counts = dict(
            (vendor, count)
            for vendor, count in self.execute_reporting_query("system-count-by-vendor")
        )
    ...
    def test_systems_by_memory(self):
        ...
        session.flush()
        system_counts = dict(
            ((arch, memory_gb), count)
            for arch, memory_gb, count in self.execute_reporting_query(
                "system-count-by-arch-memory-gb"
            )
        )
        self.assert_(system_counts[("i386", None)] >= 1, system_counts)
        self.assert_(system_counts[("i386", 39)] >= 3, system_counts)
    ...
    def test_system_age(self):
        system = data_setup.create_system(
            date_added=datetime.datetime.utcnow() - datetime.timedelta(days=100),
        ...
        (row,) = [
            row
            for row in self.execute_reporting_query("system-age")
    ...
    def test_system_breakages(self):
        system = data_setup.create_system()
        data_setup.create_system_status_history(
            system,
            [
        ...
        (row,) = [
            row
            for row in self.execute_reporting_query("system-breakages")
            if row.fqdn == system.fqdn
        ]
    ...
    def test_cee_ops_provided_queries(self):
        ...
        reserved_system = data_setup.create_system(
            fqdn=u"gsslab.reserved", status="Manual"
        )
        data_setup.create_manual_reservation(
            reserved_system, user=cee_user, start=datetime.datetime(2016, 1, 1, 0, 0)
        )
        # Create a system which is loaned out
        loaned_system = data_setup.create_system(fqdn=u"gsslab.loaned", status="Manual")
        data_setup.create_system_loan(
            loaned_system, user=cee_user, start=datetime.datetime(2016, 1, 2, 0, 0)
        )
