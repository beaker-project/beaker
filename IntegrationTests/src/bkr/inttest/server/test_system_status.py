...
class SystemStatusTest(DatabaseTestCase):
    ...
    def check_status_durations(self, system):
        ...
        # finish_time.
        self.assert_(
            len([sd for sd in system.status_durations if sd.finish_time is None]) == 1,
            system.status_durations,
        )

    def setup_system_with_queued_commands(self):
        with session.begin():
            system = data_setup.create_system(
                status=u"Automated", lab_controller=data_setup.create_labcontroller()
            )
    ...
    def test_queued_commands_aborted_when_lab_controller_removed(self):
        ...
        for c in system.command_queue:
            self.assertEqual(
                u"System disassociated from lab controller", c.error_message
            )
