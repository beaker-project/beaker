...
class CreateKickstartTest(DatabaseTestCase):
    ...
    def _create_recipe(self, system=None):
        with session.begin():
            ...
            recipe = data_setup.create_recipe(
                distro_tree=rhel62_server_x86_64, task_list=[install_task, reserve_task]
            )
            data_setup.create_job_for_recipes(
                [recipe], owner=create_user(), whiteboard=u""
            )
    ...
    def _create_recipe_with_user_defined_distro(self, **kwargs):
        with session.begin():
            ...
            system = create_x86_64_automated(lc)
            recipe = (
                data_setup.create_recipe(
                    custom_distro=True,
                    osmajor=kwargs["osmajor"],
                    task_list=[install_task, reserve_task],
                )
                if "osmajor" in kwargs
                else data_setup.create_recipe(
                    custom_distro=True, task_list=[install_task, reserve_task]
                )
            )
            data_setup.create_job_for_recipes(
                [recipe], owner=create_user(), whiteboard=u""
    ...
    def _create_i386_distro(self, lc):
        ...
        i386_distro_tree = data_setup.create_distro_tree(
            distro=i386_distro,
            lab_controllers=[lc],
            urls=[
                u"http://lab.test-kickstart.example.com/distros/RHEL-6.3/"
    ...
    def test_version(self):
        out = run_command(
            "create_kickstart.py", "beaker-create-kickstart", ["--version"]
        )
    ...
    def test_nonexistent_recipe_id(self):
        try:
            ...
            self.fail("Should raise an exception when passed an invalid recipe" " id")
        except CommandError as e:
            self.assertIn("RuntimeError: Recipe id '0' does not exist", e.stderr_output)

    def test_create_kickstart_of_unknown_osmajor_does_not_fail(self):
        system = data_setup.create_system()
        recipe = self._create_recipe_with_user_defined_distro(
            osmajor="SomeRandomLinux1"
        )
        self.assertIsNotNone(
            self._run_create_kickstart(
                ["--recipe-id", str(recipe.id), "--system", system.fqdn]
            )
        )
    ...
    def test_nonexistent_system_fqdn(self):
        ...
        try:
            self._run_create_kickstart(
                ["--recipe-id", str(recipe.id), "--system", "dsaffds124143g"]
            )
            self.fail("Should raise an exception when passed an invalid system" " fqdn")
        except CommandError as e:
            self.assertIn(
                "RuntimeError: System 'dsaffds124143g' does not " "exist",
                e.stderr_output,
            )
    ...
    def test_nonexistent_distro_tree_id(self):
        ...
        try:
            ...
            self.fail(
                "Should raise an exception when passed an invalid distro" " tree id"
            )
        except CommandError as e:
            self.assertIn(
                "RuntimeError: Distro tree id '0' does not exist", e.stderr_output
            )
    ...
    def test_snippet_dir(self):
        ...
        try:
            ...
            output = self._run_create_kickstart(
                ["--template-dir", template_dir, "--recipe-id", str(recipe.id)]
            )
        finally:
            shutil.rmtree(snippet_dir)

        self.assertIn("timezone Africa/Juba", output.splitlines(), output)

        # Assert that snippets are still being pulled from the normal
        # places as well
        self.assertIn(
            "nfs --server lab.test-kickstart.invalid --dir "
            "/distros/RHEL-6.2/Server/x86_64/os/",
            output.splitlines(),
            output,
        )
    ...
    def test_ks_meta_koptions_post(self):
        ...
        output = self._run_create_kickstart(
            [
                ...
                "method=http",
                "--kernel-options-post",
                "console=ttyS0,9600n8 pci=nomsi",
                "--recipe-id",
                str(recipe.id),
            ]
        )
        session.expire_all()
        self.assertIn(
            "url --url=http://lab.test-kickstart.invalid/distros/RHEL-6.2/Server/x86_64/os/",
            output.splitlines(),
            output,
        )
        self.assertIn(
    ...
    def test_rhel6_defaults_recipe(self):
        ...
        compare_expected(
            "RedHatEnterpriseLinux6-scheduler-defaults-beaker-create-kickstart",
            recipe.id,
            generated_ks,
        )
    ...
    def test_system_overrides_recipe(self):
        with session.begin():
            lc = create_lab_controller()
            system1 = data_setup.create_system(lab_controller=lc, arch="x86_64")
            system2 = data_setup.create_system(lab_controller=lc, arch="x86_64")
            system1.provisions[system1.arch[0]] = Provision(
                arch=system1.arch[0], kernel_options_post="adalovelace"
            )
            system2.provisions[system2.arch[0]] = Provision(
                arch=system2.arch[0], kernel_options_post="phonykerneloption"
            )
    ...
    def test_distro_overrides_recipe(self):
        ...
        self.assertIn(
            ...
            "RHEL-6.3/Workstation/i386/os/",
            kickstart,
        )
        self.assertNotIn(
            "url=http://lab.test-kickstart.invalid/distros/"
            "RHEL-6.2/Server/x86_64/os/",
            kickstart,
        )
    ...
    def test_distro_and_system_overrides_recipe(self):
        ...
        with session.begin():
            ...
            system2.provisions[system2.arch[0]] = Provision(
                arch=system2.arch[0], kernel_options_post="usshopper"
            )
        recipe = self._create_recipe(system1)
