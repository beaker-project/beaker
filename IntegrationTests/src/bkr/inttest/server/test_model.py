...
class TestSystem(DatabaseTestCase):
    ...
    def test_install_options_override(self):
        ...
        OSMajorInstallOptions(
            osmajor=osmajor, arch=distro_tree.arch, kernel_options="serial"
        )
        system = data_setup.create_system()
        system.provisions[distro_tree.arch] = Provision(
            arch=distro_tree.arch, kernel_options="console=ttyS0 ksdevice=eth0 vnc"
        )
    ...
    def test_mark_broken_updates_history(self):
        ...
        session.flush()
        system_activity = system.dyn_activity.filter(
    ...
    def test_cannot_set_status_reason_when_system_is_not_bad(self):
        ...
        try:
            ...
            self.assertIn(
                "Cannot set status reason when status is %s" % SystemStatus.automated,
                str(e),
            )
    ...
    def test_distros(self):
        ...
        tree_excluded = data_setup.create_distro_tree(
            arch=u"i386", osmajor=excluded_osmajor.osmajor
        )
        included_osmajor = OSMajor.lazy_create(
            osmajor=data_setup.unique_name(u"osmajor_test_distros%s")
        )
        tree_not_in_lab = data_setup.create_distro_tree(
            arch=u"i386", osmajor=included_osmajor.osmajor, lab_controllers=[]
        )
        tree_in_lab = data_setup.create_distro_tree(
            arch=u"i386", osmajor=included_osmajor.osmajor, lab_controllers=[lc]
        )
        system = data_setup.create_system(
            arch=u"i386", lab_controller=lc, exclude_osmajor=[excluded_osmajor]
        )
        session.flush()
    ...
    def test_create_system_multi_arch(self):
        ...
        tree_excluded_1 = data_setup.create_distro_tree(
            arch=u"i386", osmajor=excluded_osmajor.osmajor
        )
        tree_excluded_2 = data_setup.create_distro_tree(
            arch=u"x86_64", osmajor=excluded_osmajor.osmajor
        )
        system = data_setup.create_system(
            arch=[u"i386", u"x86_64"], exclude_osmajor=[excluded_osmajor]
        )
        session.flush()
        distros = system.distros()
    ...
    def test_system_using_pool_access_policy(self):
        ...
        pool = data_setup.create_system_pool(name=u"pool-1", systems=[system1])
...
class TestSystemPool(DatabaseTestCase):
    ...
    def test_create_system_pool(self):
        ...
        data_setup.create_system_pool(name=u"pool-1", systems=[system1, system2])
        session.flush()
        ...
        self.assertRaises(
            ValueError, lambda: data_setup.create_system_pool(name=u"pool-1/")
        )
    ...
    def test_pool_permissions(self):
        ...
        session.flush()
        self.assert_(
            SystemAccessPolicy.query.filter(SystemAccessPolicy.id == pool_policy.id)
            .filter(SystemAccessPolicy.grants(user1, SystemPermission.edit_policy))
            .count()
        )
...
class SystemFilterMethodsTest(DatabaseTestCase):
    ...
    def check_hybrid(self, base_query, func, included, excluded):
        ...
        if included:
            ...
            self.assertItemsEqual(
                included, query.filter(System.id.in_([s.id for s in included])).all()
            )
    ...
    def test_is_free(self):
        ...
        attached_to_disabled_lc = data_setup.create_system(lab_controller=disabled_lc)
    ...
    def test_compatible_with_distro_tree(self):
        ...
        osmajor_excluded = data_setup.create_system(
            arch=u"x86_64", exclude_osmajor=[distro_tree.distro.osversion.osmajor]
        )
        osversion_excluded = data_setup.create_system(
            arch=u"x86_64", exclude_osversion=[distro_tree.distro.osversion]
        )
        compatible = data_setup.create_system(arch=u"x86_64")
        self.check_hybrid(
            System.query,
            lambda s: s.compatible_with_distro_tree(
    ...
    def test_in_lab_with_distro_tree(self):
        ...
        self.check_hybrid(
            System.all(data_setup.create_user()),
            lambda s: s.in_lab_with_distro_tree(distro_tree),
            included=[in_lab],
            excluded=[in_wrong_lab, in_no_lab],
        )
...
class SystemPermissionsTest(DatabaseTestCase):
    ...
    def test_can_borrow(self):
        ...
        self.check_borrowing_permissions(allow=[lender], deny=[borrower])
...
class SystemAccessPolicyTest(DatabaseTestCase):
    ...
    def test_add_rule_for_user(self):
        ...
        self.assert_(
            SystemAccessPolicy.query.filter(SystemAccessPolicy.id == self.policy.id)
            .filter(SystemAccessPolicy.grants(user, perm))
            .count()
        )
        other_user = data_setup.create_user()
        self.assertFalse(self.policy.grants(other_user, perm))
        self.assert_(
            not SystemAccessPolicy.query.filter(SystemAccessPolicy.id == self.policy.id)
            .filter(SystemAccessPolicy.grants(other_user, perm))
            .count()
        )
    ...
    def test_add_rule_for_group(self):
        ...
        self.assert_(
            SystemAccessPolicy.query.filter(SystemAccessPolicy.id == self.policy.id)
            .filter(SystemAccessPolicy.grants(member, perm))
            .count()
        )
        ...
        self.assert_(
            not SystemAccessPolicy.query.filter(SystemAccessPolicy.id == self.policy.id)
            .filter(SystemAccessPolicy.grants(non_member, perm))
            .count()
        )
    ...
    def test_add_rule_for_everybody(self):
        ...
        self.assertEquals(self.policy.rules[0].permission, perm)
        self.assert_(
            self.policy.rules[0].user is None and self.policy.rules[0].group is None
        )
        ...
        self.assert_(
            SystemAccessPolicy.query.filter(SystemAccessPolicy.id == self.policy.id)
            .filter(SystemAccessPolicy.grants_everybody(perm))
            .count()
        )
        user = data_setup.create_user()
        self.assertTrue(self.policy.grants(user, perm))
...
class TestJob(DatabaseTestCase):
    ...
    def check_progress_bar(self, bar, *expected_widths):
        ...
        expected_styles = ["width:%.3f%%" % width for width in expected_widths]
        styles = [elem.get("style") for elem in bar.getchildren()[0].getchildren()]
        self.assertEquals(styles, expected_styles)
        widths = [
            float(re.match(r"width:(\d+\.\d{3})%", style).group(1)) for style in styles
        ]
    ...
    def test_progress_bar_sums_to_100_pass1_fail2(self):
        ...
        data_setup.mark_job_complete(job)
        self.check_progress_bar(job.progress_bar, 0, 33.333, 0, 66.667, 0)
    ...
    def test_progress_bar_sums_to_100_pass4_warn1_fail1(self):
        ...
        data_setup.mark_job_complete(job)
        self.check_progress_bar(job.progress_bar, 0, 66.667, 16.667, 16.667, 0)
    ...
    def test_create_inventory_job(self):
        ...
        self.assertIn('<hostRequires force="%s"/>' % system.fqdn, job_xml)
        self.assertIn(
            '<task name="/distribution/check-install" role="STANDALONE"/>', job_xml
        )
        self.assertIn(
            '<task name="/distribution/inventory" role="STANDALONE"/>', job_xml
        )
    ...
    def test_completed_n_days_ago(self):
        ...
        partially_complete_job = data_setup.create_running_job(
            num_recipes=2,
            queue_time=datetime.datetime.utcnow() - datetime.timedelta(days=92),
            start_time=datetime.datetime.utcnow() - datetime.timedelta(days=91),
        )
        data_setup.mark_recipe_complete(
            partially_complete_job.recipesets[0].recipes[0],
            only=True,
            finish_time=datetime.datetime.utcnow() - datetime.timedelta(days=90),
        )
...
class DistroTreeByFilterTest(DatabaseTestCase):
    ...
    def test_distro_tag_equal(self):
        excluded = data_setup.create_distro_tree(distro_tags=[u"INSTALLS", u"STABLE"])
    ...
    def test_distro_tag_notequal(self):
        ...
        included = data_setup.create_distro_tree(distro_tags=[u"INSTALLS", u"STABLE"])
        session.flush()
        distro_trees = DistroTree.by_filter(
            """
            <distroRequires>
    ...
    def test_distrolabcontroller(self):
        ...
        included = data_setup.create_distro_tree()
        lc = data_setup.create_labcontroller()
        included.lab_controller_assocs.append(
            LabControllerDistroTree(lab_controller=lc, url=u"http://notimportant")
        )
        session.flush()
        distro_trees = DistroTree.by_filter(
            """
    ...
    def test_distrolabcontroller_notequal(self):
        ...
        included = data_setup.create_distro_tree()
        lc = data_setup.create_labcontroller()
        excluded.lab_controller_assocs.append(
            LabControllerDistroTree(lab_controller=lc, url=u"http://notimportant")
        )
        session.flush()
        distro_trees = DistroTree.by_filter(
...
class DistroTreeTest(DatabaseTestCase):
    ...
    def test_url_in_lab(self):
        ...
        distro_tree.lab_controller_assocs[:] = [
            ...
            LabControllerDistroTree(
                lab_controller=self.lc, url=u"nfs+iso://unimportant"
            ),
        ]
        other_lc = data_setup.create_labcontroller()
        session.flush()

        self.assertEquals(distro_tree.url_in_lab(self.lc), "ftp://unimportant")
        self.assertEquals(distro_tree.url_in_lab(other_lc), None)
        self.assertRaises(
            ValueError, lambda: distro_tree.url_in_lab(other_lc, required=True)
        )

        self.assertEquals(
            distro_tree.url_in_lab(self.lc, scheme="ftp"), "ftp://unimportant"
        )
        self.assertEquals(distro_tree.url_in_lab(self.lc, scheme="http"), None)
        self.assertRaises(
            ValueError,
            lambda: distro_tree.url_in_lab(self.lc, scheme="http", required=True),
        )

        self.assertEquals(
            distro_tree.url_in_lab(self.lc, scheme=["http", "ftp"]), "ftp://unimportant"
        )
        self.assertEquals(distro_tree.url_in_lab(self.lc, scheme=["http", "nfs"]), None)
        self.assertRaises(
            ValueError,
            lambda: distro_tree.url_in_lab(
    ...
    def test_ksmeta_ks_keyword(self):
        ...
        distro_rhel6 = data_setup.create_distro(
            osmajor=u"RedHatEnterpriseLinux6", osminor=u"5"
        )
        rhel6_ppc64 = data_setup.create_distro_tree(distro=distro_rhel6, arch=u"ppc64")
    ...
    def test_inst_ks_option_for_newer_systems(self):
        ...
        self.assertFalse(
            re.search(r"(?<!inst\.)ks=", recipe.installation.kernel_options)
        )
    ...
    def test_custom_netbootloader(self):
        ...
        # ppc64, RHEL 7.0
        distro_rhel7 = data_setup.create_distro(
            osmajor=u"RedHatEnterpriseLinux7", osminor=u"0"
        )
        rhel7_ppc64 = data_setup.create_distro_tree(distro=distro_rhel7, arch=u"ppc64")
        r2 = self.provision_distro_tree(rhel7_ppc64)
        self.assertIn("netbootloader=yaboot", r2.installation.kernel_options)

        # ppc64, RHEL 7.1
        distro_rhel71 = data_setup.create_distro(
            osmajor=u"RedHatEnterpriseLinux7", osminor=u"1"
        )
        rhel71_ppc64 = data_setup.create_distro_tree(
            distro=distro_rhel71, arch=u"ppc64"
        )
        r3 = self.provision_distro_tree(rhel71_ppc64)
        self.assertIn(
            "netbootloader=boot/grub2/powerpc-ieee1275/core.elf",
        ...
        distro_tree = data_setup.create_distro_tree(
            ...
            osmajor_installopts_arch={
                "kernel_options": "netbootloader=something/weird"
            },
        )
        ...
        # ppc64, Fedora 21
        distro_f21 = data_setup.create_distro(osmajor=u"Fedora21", osminor=u"0")
        f21_ppc64 = data_setup.create_distro_tree(distro=distro_f21, arch=u"ppc64")
        ...
        self.assertIn("netbootloader=a/new/bootloader", r6.installation.kernel_options)

        # Manual provision
        system7 = data_setup.create_system(shared=True, lab_controller=self.lc)
        system7.provisions[f21_ppc64.arch] = Provision(
            arch=f21_ppc64.arch, kernel_options="netbootloader=a/new/bootloader"
        )
    ...
    def test_leavebootorder_kernel_option_is_set_by_default_for_ppc(self):
        ...
        recipe = self.provision_distro_tree(distro_tree)
        self.assertIn(
            u"inst.leavebootorder", recipe.installation.kernel_options.split()
        )
...
class OSMajorTest(DatabaseTestCase):
    ...
    def test_arches(self):
        ...
        self.assertEquals(set(["ia64", "ppc64"]), set(arch.arch for arch in arches))
    ...
    def test_install_options(self):
        ...
        session.flush()
        self.assertEquals(
            set(o.install_options_by_arch.keys()),
            set([None, ia64]),
            o.install_options_by_arch,
        )
        self.assertEquals(o.install_options_by_arch[None].ks_meta, u"one=two")
        self.assertEquals(o.install_options_by_arch[ia64].kernel_options, u"serial")
...
class GroupTest(DatabaseTestCase):
    ...
    def check_activity(self, activity, user, service, action, field, old, new):
    ...
    def test_set_name(self):
        ...
        group = Group(group_name=orig_name, display_name=u"Beaker Developers")
        session.flush()
        ...
        self.check_activity(
            group.activity[0], None, u"TEST", u"Changed", u"Name", orig_name, new_name
        )

    def test_set_display_name(self):
        orig_display_name = u"Beaker Developers"
        group = Group(group_name=u"beakerdevs", display_name=orig_display_name)
        session.flush()
        # Setting the same name is a no-op
    ...
    def test_cannot_rename_protected_groups(self):
        ...
        for group in groups:
            ...
            self.assertRaises(
                BeakerException, group.set_name, None, u"TEST", u"bad_rename"
            )
    ...
    def test_cannot_exclude_user_from_a_normal_group(self):
        ...
        user = data_setup.create_user()
        self.assertRaises(
            RuntimeError, group.exclude_user, "Cannot exclude users from normal groups"
        )
...
class RecipeTest(DatabaseTestCase):
    ...
    def test_roles_to_xml(self):
        ...
        for i in range(3):
            data_setup.mark_recipe_running(
                job.recipesets[0].recipes[i], system=systems[i]
            )
        xml = lxml.etree.tostring(
            job.recipesets[0].recipes[0].to_xml(clone=False), encoding=unicode
        )
        self.assert_(
            u"<roles>"
    ...
    def test_installation_in_xml(self):
        ...
        self.assertRegexpMatches(
            installation.get("install_started"),
            r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$",
        )
        self.assertRegexpMatches(
            installation.get("install_finished"),
            r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$",
        )
        self.assertRegexpMatches(
    ...
    def test_hostrequires_force_clone_success(self):
        ...
        xml = lxml.etree.tostring(
            job.recipesets[0].recipes[0].to_xml(clone=True), encoding=unicode
        )
    ...
    def test_recipe_reservesys_clone(self):
        ...
        xml = lxml.etree.tostring(
            job.recipesets[0].recipes[1].to_xml(clone=True), encoding=unicode
        )
        reservation_string = (
            u'<task name="/distribution/check-install" role="STANDALONE"/>'
            + u'<task name="/distribution/check-install" role="STANDALONE"/>'
    ...
    def test_systemtype_is_injected_into_hostrequires(self):
        ...
        recipe_other_hr.host_requires = (
            u'<hostRequires><hostname value="blorp"/></hostRequires>'
        )
        self.assertEquals(
            recipe_other_hr.host_requires,
            u"<hostRequires>"
...
class CheckDynamicVirtTest(DatabaseTestCase):
    ...
    def assertVirtPossible(self, recipe, msg):
        self.assertEqual(
            recipe.check_virtualisability(), RecipeVirtStatus.possible, msg
        )

    def assertVirtPrecluded(self, recipe, msg):
        self.assertEqual(
            recipe.check_virtualisability(), RecipeVirtStatus.precluded, msg
        )
    ...
    def test_virt_precluded_guest_recipes(self):
        for arch in [u"i386", u"x86_64"]:
            ...
            job = data_setup.create_job(
                num_guestrecipes=1, distro_tree=dt, owner=self.user
            )
    ...
    def test_virt_precluded_multihost(self):
        for arch in [u"i386", u"x86_64"]:
            ...
            self.assertVirtPrecluded(
                recipe1, "Multihost recipeset did not preclude virt"
            )
            self.assertVirtPrecluded(
                recipe2, "Multihost recipeset did not preclude virt"
            )
...
class MachineRecipeTest(DatabaseTestCase):
    ...
    def _create_demo_recipes(self, arch, num_recipes, whiteboard):
        dt = data_setup.create_distro_tree(arch=arch)
        recipes = [
            data_setup.create_recipe(dt, whiteboard=whiteboard)
    ...
    def test_get_queue_stats(self):
        ...
        def _get_queue_stats():
            cls = MachineRecipe
            return cls.get_queue_stats(cls.query.filter(cls.whiteboard == whiteboard))
    ...
    def test_get_queue_stats_by_arch(self):
        ...
        expected_stats = dict((arch, default_stats.copy()) for arch in expected_arches)
        whiteboard = u"test_get_queue_stats_by_arch"

        def _get_queue_stats():
            cls = MachineRecipe
            return cls.get_queue_stats_by_group(
                Arch.arch,
    ...
    def test_custom_distro_xml(self):
        ...
        xml = recipe.to_xml()
        self.assertEqual(
            xml.find("recipeSet/recipe").get("distro"), "MyAwesomeLinux1.0"
        )
        self.assertEqual(xml.find("recipeSet/recipe").get("arch"), "i386")
        self.assertEqual(
            xml.find("recipeSet/recipe").get("family"), "DansAwesomeLinux6"
        )
        self.assertEqual(xml.find("recipeSet/recipe").get("variant"), "Server")
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/tree").get("url"),
            "ftp://dummylab.example.com/distros/MyAwesomeLinux1/",
        )
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/initrd").get("url"), "pxeboot/initrd"
        )
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/kernel").get("url"), "pxeboot/vmlinuz"
        )
        self.assertEqual(xml.find("recipeSet/recipe/distro/arch").get("value"), "i386")
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/variant").get("value"), "Server"
        )
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/name").get("value"), "MyAwesomeLinux1.0"
        )
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/osversion").get("major"),
            "DansAwesomeLinux6",
        )
        self.assertEqual(
            xml.find("recipeSet/recipe/distro/osversion").get("minor"), "0"
        )
...
class GuestRecipeTest(DatabaseTestCase):
    ...
    def test_locations(self):
        ...
        distro_tree = data_setup.create_distro_tree(
            lab_controllers=[lc],
            urls=[u"nfs://something:/somewhere", u"http://something/somewhere"],
        )
        job = data_setup.create_completed_job(
    ...
    def test_guestname(self):
        ...
        self.assertEqual(
            guestxml_1.find("recipeSet/recipe/guestrecipe").get("guestname"), u""
        )
        self.assertEqual(
...
class MACAddressAllocationTest(DatabaseTestCase):
    def setUp(self):
        ...
        running = RecipeSet.query.filter(
            not_(RecipeSet.status.in_([s for s in TaskStatus if s.finished]))
        )
        for rs in running:
            rs.cancel()
    ...
    def test_lowest_free_mac_none_in_use(self):
        self.assertEquals(
            RecipeResource._lowest_free_mac(), netaddr.EUI("52:54:00:00:00:00")
        )
    ...
    def test_lowest_free_mac_one_in_use(self):
        ...
        self.assertEquals(
            RecipeResource._lowest_free_mac(), netaddr.EUI("52:54:00:00:00:01")
        )
    ...
    def test_lowest_free_mac_gap_at_start(self):
        ...
        self.assertEquals(
            first_job.recipesets[0].recipes[0].guests[0].resource.mac_address,
            netaddr.EUI("52:54:00:00:00:00"),
        )
        second_job = data_setup.create_job(num_guestrecipes=1)
        data_setup.mark_job_running(second_job)
        self.assertEquals(
            second_job.recipesets[0].recipes[0].guests[0].resource.mac_address,
            netaddr.EUI("52:54:00:00:00:01"),
        )
        self.assertEquals(
            RecipeResource._lowest_free_mac(), netaddr.EUI("52:54:00:00:00:02")
        )
        first_job.cancel()
    ...
    def test_guestrecipe_complete_but_recipeset_incomplete(self):
        ...
        data_setup.mark_recipe_complete(
            job.recipesets[0].recipes[0].guests[0], only=True
        )
    ...
    def test_in_use_below_base_address(self):
        ...
        job.recipesets[0].recipes[0].guests[0].resource.mac_address = netaddr.EUI(
            "52:53:FF:00:00:00"
        )
        self.assertEquals(
            RecipeResource._lowest_free_mac(), netaddr.EUI("52:54:00:00:00:00")
...
class VirtResourceTest(DatabaseTestCase):
    ...
    def test_link(self):
        ...
        expected_hyperlink = (
            '<a href="http://openstack.example.invalid/'
            'dashboard/project/instances/{0}/">{0}</a>'.format(
                recipe.resource.instance_id
            )
        )
        self.assertEquals(
            serialize_kid_element(recipe.resource.link),
        ...
        data_setup.mark_recipe_installation_finished(
            recipe, fqdn=u"my-openstack-instance"
        )
        self.assertEquals(
            serialize_kid_element(recipe.resource.link),
            "<span>my-openstack-instance (OpenStack instance {0})</span>".format(
                expected_hyperlink
            ),
        )
...
class LogRecipeTest(DatabaseTestCase):
    ...
    def test_lazy_create_can_deal_with_deadlocks(self):
            ...
            def inner(*args, **kwargs):
                ...
                if _raise_counter < raise_this_many:
                    ...
                    raise OperationalError(
                        "statement", {}, "(OperationalError) (1213, blahlbha"
                    )
        ...
        try:
            ...
            lr1 = LogRecipe.lazy_create(
                path=u"/", filename=u"dummy.log", recipe_id=self.recipe.id
            )
            ...
            lr2 = LogRecipe.lazy_create(
                path=u"/", filename=u"dummy2.log", recipe_id=self.recipe.id
            )
            ...
            _raise_counter = 0
            model.base.ConditionalInsert = _raise_deadlock_exception(
                _max_valid_attempts + 1
            )
            try:
                LogRecipe.lazy_create(
                    path=u"/", filename=u"dummy3.log", recipe_id=self.recipe.id
                )
...
class DeviceClassTest(DatabaseTestCase):
    ...
    def test_lazy_create_does_not_cause_duplicates(self):
        ...
        self.assert_(first is second)
        self.assertEquals(
            DeviceClass.query.filter_by(device_class=u"washing_machine").count(), 1
        )
...
class DeviceTest(DatabaseTestCase):
    ...
    def test_lazy_create_does_not_cause_duplicates(self):
        ...
        params = dict(
            ...
            vendor_id=u"8086",
            device_id=u"1111",
            subsys_vendor_id=u"8086",
            subsys_device_id=u"1111",
            bus=u"pci",
            driver=u"e1000",
            description=u"lol",
        )
...
class TaskTest(DatabaseTestCase):
    ...
    def test_compatible_with_osmajor_obeys_excluded_releases(self):
        ...
        incompatible_task = data_setup.create_task(
            exclude_osmajors=[u"RedHatEnterpriseLinux6"]
        )
        compatible_task = data_setup.create_task(
            exclude_osmajors=[
                u"RedHatEnterpriseLinuxServer5",
                u"RedHatEnterpriseLinuxClient5",
            ]
        )
        session.flush()
    ...
    def test_compatible_with_osmajor_obeys_exclusive_releases(self):
        rhel6 = OSMajor.lazy_create(osmajor=u"RedHatEnterpriseLinux6")
        compatible_task = data_setup.create_task(
            exclusive_osmajors=[u"RedHatEnterpriseLinux6"]
        )
        incompatible_task = data_setup.create_task(
            exclusive_osmajors=[
                u"RedHatEnterpriseLinuxServer5",
...
class TaskLibraryTest(DatabaseTestCase):
    def setUp(self):
        ...
        self.task_rpm_new = pkg_resources.resource_filename(
            "bkr.inttest.server", "task-rpms/" + self.task_rpm_name_new
        )
        self.task_rpm_old = pkg_resources.resource_filename(
            "bkr.inttest.server", "task-rpms/" + self.task_rpm_name_old
        )
    ...
    def query_task_repo(self, task_name):
        ...
        base.repos.add_new_repo(
            "myrepo", base.conf, baseurl=["file://" + os.path.abspath(task_dir)]
        )
    ...
    def test_downgrade_task(self):
        ...
        Task.update_task(
            self.task_rpm_name_new, self.write_data_file(self.task_rpm_new)
        )
        # Downgrade now
        Task.update_task(
            self.task_rpm_name_old, self.write_data_file(self.task_rpm_old)
        )
...
class RecipeTaskTest(DatabaseTestCase):
    ...
    def test_logs_appear_in_results_xml(self):
        ...
        self.assertEqual(
            urlparse.urljoin(logs[0].base, logs[0].get("href")),
            get_server_base()
            + "recipes/%s/tasks/%s/logs/asdf/log.txt"
...
class RecipeTaskResultTest(DatabaseTestCase):
    ...
    def test_display_label(self):
        ...
        rtr = RecipeTaskResult(
            recipetask=rt, path=u"/distribution/check-install/Sysinfo"
        )
    ...
    def test_coerces_string_for_score(self):
        ...
        rt._result(
            TaskResult.pass_, path=".", score="123foo", summary="numbered string"
        )
    ...
    def test_caps_very_large_score(self):
        self.recipe_task.pass_(
            path=u"/distribution/kernelinstall/Sysinfo",
            score="12345678900",
            summary=None,
        )
    ...
    def test_logs_appear_in_results_xml(self):
        ...
        self.assertEqual(
            urlparse.urljoin(logs[0].base, logs[0].get("href")),
            get_server_base()
            + "recipes/%s/tasks/%s/results/%s/logs/asdf/log.txt"
...
class TestSystemInventoryDistro(DatabaseTestCase):
    def setUp(self):
        with session.begin():
            ...
            self.distro_tree1 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux6",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree2 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux6",
                arch=u"x86_64",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree3 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux7",
                arch=u"ppc64",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree4 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux7",
                arch=u"ppc64le",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree5 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux6",
                arch=u"s390x",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree6 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux6",
                arch=u"s390",
                distro_tags=[u"RELEASED"],
                lab_controllers=[self.lc],
            )
            self.distro_tree7 = data_setup.create_distro_tree(
                osmajor=u"RedHatEnterpriseLinux7",
                arch=u"aarch64",
                lab_controllers=[self.lc],
            )
            ...
            self.distro_tree8 = data_setup.create_distro_tree(
                osmajor=u"Fedora22",
                arch=u"i386",
                distro_tags=[u"RELEASED"],
            ...
            self.distro_tree9 = data_setup.create_distro_tree(
                osmajor=u"MyDistro",
                arch=u"x86_64",
                distro_tags=[u"RELEASED"],
    ...
    def test_select_inventory_distro_tree_released(self):
        ...
        with session.begin():
            self.assertEquals(
                self.system1.distro_tree_for_inventory(), self.distro_tree2
            )
        # For system2, i386 tree should be chosen
        with session.begin():
            self.assertEquals(
                self.system2.distro_tree_for_inventory(), self.distro_tree1
            )

        # For system3, ppc64 tree should be preferred
        with session.begin():
            self.assertEquals(
                self.system3.distro_tree_for_inventory(), self.distro_tree3
            )
        # For system4, ppc64le tree should be chosen
        with session.begin():
            self.assertEquals(
                self.system4.distro_tree_for_inventory(), self.distro_tree4
            )

        # For system5, s390x tree should be preferred
        with session.begin():
            self.assertEquals(
                self.system5.distro_tree_for_inventory(), self.distro_tree5
            )
        # For system6, s390 tree should be chosen
        with session.begin():
            self.assertEquals(
                self.system6.distro_tree_for_inventory(), self.distro_tree6
            )
        ...
        with session.begin():
            self.assertEquals(
                self.system8.distro_tree_for_inventory(), self.distro_tree8
            )

    def test_distro_tree_for_inventory_no_released_tree(self):
        with session.begin():
            self.assertEquals(
                self.system7.distro_tree_for_inventory(), self.distro_tree7
            )

    def test_distro_tree_for_inventory_no_preferred_tree(self):
        with session.begin():
            self.assertEquals(
                self.system9.distro_tree_for_inventory(), self.distro_tree9
            )
