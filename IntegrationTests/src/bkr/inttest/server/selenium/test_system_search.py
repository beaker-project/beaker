...
class Search(WebDriverTestCase):
    ...
    def test_by_date_added(self):
        with session.begin():
            ...
            new_system.date_added = datetime.datetime(2099, 6, 21, 11, 30, 0)
        ...
        perform_search(b, [('System/Added', 'after', '2099-12-31')])
        # no results
        b.find_element_by_xpath('//table[@id="widget" and not(.//td)]')

        perform_search(b, [('System/Added', 'after', '2099-06-20')])
        check_system_search_results(b, present=[new_system], absent=[old_system])

        perform_search(b, [('System/Added', 'after', '2099-06-20'),
                           ('System/Added', 'before', '2099-06-22')])
        check_system_search_results(b, present=[new_system], absent=[old_system])
