...
class SearchJobs(SeleniumTestCase):
    ...
    def test_quick_search(self):
        ...
        sel.wait_for_page_to_load("30000")
        self.assertEqual(sel.get_table("//table[@id='widget'].0.0"),'J:%s' % self.queued_job.id)
        ...
        sel.wait_for_page_to_load("30000")
        self.assertEqual(sel.get_table("//table[@id='widget'].0.0"),'J:%s' % self.running_job.id)
        ...
        sel.wait_for_page_to_load("30000")
        self.assertEqual(sel.get_table("//table[@id='widget'].0.0"),'J:%s' % self.completed_job.id)
