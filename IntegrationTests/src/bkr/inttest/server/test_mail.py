...
class BrokenSystemNotificationTest(DatabaseTestCase):
    def test_broken_system_notification_on(self):
        with session.begin():
            ...
            data_setup.configure_system_power(
                system, power_type=u"drac", address=u"pdu2.home-one", power_id=u"42"
            )
        ...
        self.assertEqual(msg["Subject"], "System home-one automatically marked broken")
    ...
    def test_broken_system_notification_off(self):
        with session.begin():
            owner = data_setup.create_user(
                email_address=u"derp@derpmail.com", notify_broken_system=False
            )
            lc = data_setup.create_labcontroller()
            system = data_setup.create_system(
                fqdn=u"home-two",
            ...
            system.arch.append(Arch.by_name(u"x86_64"))
            data_setup.configure_system_power(
                system, power_type=u"drac", address=u"pdu3.home-one", power_id=u"42"
            )
        mail_capture_thread.start_capturing()
        with session.begin():
...
class SystemReservationNotificationTest(DatabaseTestCase):
    ...
    def test_system_reserved_notification_on(self):
        with session.begin():
            owner = data_setup.create_user(email_address=u"lizlemon@kabletown.com")
            system = data_setup.create_system(
        ...
        self.assertEqual(
            msg["Subject"], "[Beaker System Reserved] funcooker.ge.invalid"
        )
...
**  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **""" % dict(
            base=get_server_base(), recipeid=recipe.id, jobid=job.id
        )
    ...
    def test_reserved_openstack_instance(self):
        with session.begin():
            owner = data_setup.create_user(email_address=u"jackdonaghy@kabletown.com")
            distro_tree = data_setup.create_distro_tree(
        ...
        self.assertEqual(msg["To"], owner.email_address)
        self.assertEqual(
            msg["Subject"], "[Beaker System Reserved] bitenuker.ge.invalid"
        )
        self.assertEqual(msg["X-Beaker-Notification"], "system-reservation")
...
**  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **  **""" % dict(
    ...
    def test_system_reserved_notification_off(self):
        with session.begin():
            owner = data_setup.create_user(
                email_address=u"derp@derptown.com", notify_reservesys=False
            )
            system = data_setup.create_system(
                fqdn=u"funcooker.ge.valid",
                lab_controller=data_setup.create_labcontroller(),
            )
...
class JobCompletionNotificationTest(DatabaseTestCase):
    ...
    def test_job_cc_list_is_notified(self):
        ...
        with session.begin():
            job_owner = data_setup.create_user()
            job = data_setup.create_job(
                owner=job_owner, cc=[u"dan@example.com", u"ray@example.com"]
            )
            session.flush()
            data_setup.mark_job_complete(job)
        ...
        msg = email.message_from_string(raw_msg)
        self.assertEqual(
            [job_owner.email_address, "dan@example.com", "ray@example.com"], rcpts
        )
        self.assertEqual(job_owner.email_address, msg["To"])
    ...
    def test_contains_job_hyperlink(self):
        ...
        self.assert_(
            job_link in first_line,
            "Job link %r should appear in first line %r" % (job_link, first_line),
        )
...
class UsageReminderTest(DatabaseTestCase):
    ...
    def _create_expiring_reservation(self):
        recipe = data_setup.create_recipe_reservation(
            self.user, u"/distribution/reservesys", (self.reservation_expiry - 1) * 3600
        )
        email_content = u"""
...
""" % (
    ...
    def test_send_usage_reminder(self):
        ...
        self.assertTrue(
            msg["Subject"],
            "[Beaker] Usage report for %s (%s)" % (self.user.user_name, current_date),
        )
        expected_mail_body = u"""=========
