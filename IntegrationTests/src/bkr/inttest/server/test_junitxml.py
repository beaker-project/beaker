...
class JUnitXMLUnitTest(unittest.TestCase):
    ...
    def test_passing_result(self):
        ...
        recipe.tasks[0].results[0].start_time = datetime.datetime(
            2015, 12, 14, 0, 0, 10
        )
...
""".format(
            server=get_server_base(),
            recipe_id=recipe.id,
            task_id=recipe.tasks[0].id,
    ...
    def test_failing_result(self):
        job = data_setup.create_completed_job(
            recipe_whiteboard=u"failing result",
            fqdn=u"happysystem.testdata",
            server_log=True,
...
""".format(
    ...
    def test_aborted(self):
        job = data_setup.create_running_job(
            recipe_whiteboard=u"ewd", fqdn=u"sadsystem.testdata"
        )
...
"""
    ...
    def test_cancelled(self):
        job = data_setup.create_running_job(
            recipe_whiteboard=u"cancelled", fqdn=u"sadsystem.testdata"
        )
...
"""
    ...
    def test_incomplete_job(self):
        ...
        job = data_setup.create_running_job(
            recipe_whiteboard=u"running job",
        ...
        data_setup.mark_recipe_tasks_finished(
            recipe, only=True, num_tasks=1, result=TaskResult.pass_, server_log=True
        )
        recipe.tasks[0].start_time = datetime.datetime(2015, 12, 14, 0, 0, 0)
        recipe.tasks[0].results[0].start_time = datetime.datetime(
            2015, 12, 14, 0, 0, 10
...
""".format(
    ...
    def test_duration(self):
        job = data_setup.create_running_job(
            recipe_whiteboard=u"duration", fqdn=u"happysystem.testdata"
        )
        recipe = job.recipesets[0].recipes[0]
        recipe.tasks[0].start_time = datetime.datetime(2015, 12, 14, 0, 0, 0)
        ...
        recipe.tasks[0].results[1].start_time = datetime.datetime(
            2015, 12, 14, 0, 0, 13
        )
...
"""
    ...
    def test_result_names(self):
        ...
        recipe.tasks[0].pass_(
            path=u"/junitxml/names/with-suffix", score=0, summary=u"(Pass)"
        )
...
"""
    ...
    def test_cancelled_recipe_has_tasks_never_started(self):
        job = data_setup.create_job(
            recipe_whiteboard=u"cancelled", task_name=u"/test_junixml/cancelled"
        )
        job.cancel(msg=u"I cancelled it")
        recipe = job.recipesets[0].recipes[0]
        job.update_status()
