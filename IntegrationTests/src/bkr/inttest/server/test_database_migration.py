...
class MigrationTest(unittest.TestCase):
        ...
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/21.sql"
                )
            )
    ...
    def test_empty_check_and_create(self):
        EmptyOpt = collections.namedtuple(
            "EmtpyOpt",
            [
                "user_name",
                "password",
        ...
        opts = EmptyOpt._make(
            ["empty", "empty", "Empty DB Test", "empty@example.com", True, False]
        )
    ...
    def test_can_downgrade_to_current_version(self):
        ...
        if not pkg_resources.resource_exists("bkr.inttest.server", dump_filename):
            raise AssertionError(
                "Schema dump for version %s not found "
                "in IntegrationTests/src/bkr/inttest/server/database-dumps"
                % major_version_to_test
            )
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string("bkr.inttest.server", dump_filename)
            )
    ...
    def test_full_upgrade(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.11.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_012(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.12.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_013(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.13.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_014(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.14.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_015(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.15.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_016(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.16.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_017(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/0.17.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_19(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/19.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_20(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/20.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_22(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/22.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_23(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/23.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_24(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/24.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_25(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/25.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_26(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/26.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_from_27(self):
        with self.migration_engine.connect() as connection:
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "database-dumps/27.sql"
                )
            )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
    ...
    def test_redhat_production_20120216(self):
        ...
        raise unittest.SkipTest(
            "Database migrations are not implemented " "far enough into the past yet"
        )
        upgrade_db(self.migration_metadata)
        self.check_migrated_schema()
        downgrade_db(self.migration_metadata, "base")
    ...
    def check_migrated_schema(self):
        ...
        for table_name in metadata.tables:
            ...
            if table_name == "command_queue":
                ...
                actual_indexes = dict(
                    (name, cols)
                    for name, cols in actual_indexes.iteritems()
            ...
            for constraint in metadata.tables[table_name].constraints:
                if isinstance(
                    constraint,
                    (sqlalchemy.PrimaryKeyConstraint, sqlalchemy.ForeignKeyConstraint),
                ):
    ...
    def find_expected_indexes_for_table(self, table):
        ...
        for constraint in table.constraints:
            if isinstance(constraint, sqlalchemy.UniqueConstraint):
                ...
                if any(
                    has_initial_sublist(index_cols, cols)
                    for index_cols in expected_indexes.values()
                ):
                    continue
                if has_initial_sublist(
                    table.primary_key.columns.values(), constraint.columns.values()
                ):
                    continue
    ...
    def assert_columns_equivalent(self, expected, actual):
        ...
        if isinstance(actual.type, sqlalchemy.dialects.mysql.TINYINT):
            self.assertTrue(
                isinstance(expected.type, sqlalchemy.Boolean),
                "Actual type was TINYINT which is equivalent to Boolean, "
                "but expected type was %r" % expected.type,
            )
        else:
            self.assertTrue(
                actual.type._compare_type_affinity(expected.type),
                "Actual type %r should be equivalent to expected type %r"
                % (actual.type, expected.type),
            )

        if hasattr(expected.type, "length") and not isinstance(
            expected.type, UnicodeText
        ):
            self.assertEquals(
                actual.type.length, expected.type.length, "%r has wrong length" % actual
            )
        if hasattr(expected.type, "precision"):
            self.assertEquals(
                actual.type.precision,
                expected.type.precision,
        ...
        self.assertEquals(
            actual.nullable,
            expected.nullable,
            "%r should%s be nullable" % (actual, "" if expected.nullable else " not"),
        )
        ...
        if expected.server_default:
            ...
            if not actual.nullable and actual.server_default:
                ...
                if default_text not in ["''", "'0'", "0"]:
                    raise AssertionError(
                        "%r should not have database default %s"
                        % (actual, default_text)
                    )
            else:
                self.assertIsNone(
    ...
    def assert_indexes_equivalent(self, expected, actual):
        ...
        self.assertEquals(
            [col.name for col in expected.columns], [col.name for col in actual.columns]
        )
    ...
    def test_migrate_system_groups_to_pools(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                "INSERT INTO system_group(system_id, group_id) VALUES (1, 3)"
            )
            connection.execute(
                "INSERT INTO system_group(system_id, group_id) VALUES (2, 3)"
            )
            connection.execute(
                "INSERT INTO system_group(system_id, group_id) VALUES (1, 4)"
            )
            connection.execute(
                "INSERT INTO system_group(system_id, group_id) VALUES (3, 4)"
            )
        ...
        self.assertItemsEqual(
            ["group1", "group2"], [pool.name for pool in created_pools]
        )
        self.assertItemsEqual(
            ["Pool migrated from group group1", "Pool migrated from group group2"],
        ...
        for pool in expected_system_pool_owners.keys():
            p = (
                self.migration_session.query(SystemPool)
                .filter(SystemPool.name == pool)
                .one()
            )
            self.assertEquals(
                p.owning_group,
                self.migration_session.query(Group)
                .filter(Group.group_name == pool)
                .one(),
            )
        ...
        for system in expected_system_pools_map.keys():
            ...
            self.assertItemsEqual(
                [p.name for p in s.pools], expected_system_pools_map[system]
            )

        min_user_id = min(
            [user.id for user in self.migration_session.query(User).all()]
        )
    ...
    def test_migrate_system_access_policies_to_custom_access_policies(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                "INSERT INTO system_access_policy(id, system_id) VALUES (1, 2)"
            )
            connection.execute(
                "INSERT INTO system_access_policy(id, system_id) VALUES (2, 1)"
            )
            connection.execute(
                "INSERT INTO system_access_policy(id, system_id) VALUES (3, 3)"
            )
        ...
        for s in systems:
            self.assertEquals(
                s.custom_access_policy_id, expected_system_policy_map[s.fqdn]
            )
            self.assertEquals(
                s.active_access_policy_id, expected_system_policy_map[s.fqdn]
            )
    ...
    def test_delete_orphaned_system_access_policies(self):
        ...
        self.assertEquals(
            self.migration_session.query(SystemAccessPolicy).filter_by(id=2).count(), 0
        )
    ...
    def test_delete_duplicate_osmajor_install_options(self):
        ...
        row_counts = self.migration_session.query(
            OSMajorInstallOptions.osmajor_id,
            OSMajorInstallOptions.arch_id,
            func.count(),
        ).group_by(OSMajorInstallOptions.osmajor_id, OSMajorInstallOptions.arch_id)
        ...
        installopts = (
            self.migration_session.query(OSMajorInstallOptions)
            .join(OSMajorInstallOptions.osmajor)
            .filter(
                OSMajor.osmajor == u"RedHatEnterpriseLinux6",
    ...
    def test_clear_meaningless_system_activities(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                "INSERT INTO system_activity (id, system_id) " "VALUES (1, 1)"
            )
        # run migration
        upgrade_db(self.migration_metadata)
        # check that systtem activity and activity have been deleted
        self.assertEquals(
            self.migration_session.query(SystemActivity).filter_by(id=1).count(), 0
        )
    ...
    def test_migrate_recipe_set_comments_and_waived_from_nacked(self):
        ...
        self.assertEqual(
            comments[0].created, datetime.datetime(2015, 11, 5, 16, 32, 40)
        )
    ...
    def test_populate_installation_from_recipe_resource(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "bz991245-migration-setup.sql"
                )
            )
        # Run migration
        upgrade_db(self.migration_metadata)
        ...
        self.assertEqual(
            recipe.installation.rebooted, datetime.datetime(2016, 2, 16, 1, 0, 5)
        )
        self.assertEqual(
            recipe.installation.install_started, datetime.datetime(2016, 2, 16, 1, 1, 0)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            recipe.installation.created, datetime.datetime(2016, 2, 16, 1, 0, 0)
        )
        ...
        self.assertEqual(
            recipe.installation.install_started,
    ...
    def test_delete_recipe_task_results_for_deleted_job(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "bz1322700-and-bz1337790-migration-setup.sql"
                )
            )
        # run migration
        upgrade_db(self.migration_metadata)
        ...
        self.assertEquals(
            self.migration_session.query(RecipeTaskResult)
            .filter_by(recipe_task_id=1)
            .count(),
            1,
        )
        # Job one's log recipe task results should not be deleted
        self.assertEquals(
            self.migration_session.query(LogRecipeTaskResult)
            .filter_by(recipe_task_result_id=1)
            .count(),
            1,
        )
        ...
        self.assertEquals(
            self.migration_session.query(LogRecipeTaskResult)
            .filter_by(recipe_task_result_id=2)
            .count(),
            0,
        )
    ...
    def test_Installing_status_is_mapped_on_downgrade(self):
        ...
        with self.migration_metadata.bind.connect() as connection:
            self.assertEquals(
                connection.scalar("SELECT status FROM job WHERE id = 1"), u"Running"
            )
            self.assertEquals(
            ...
            self.assertEquals(
                connection.scalar("SELECT status FROM recipe WHERE id = 1"), u"Running"
            )
    ...
    def test_command_queue_separated_from_activity(self):
        ...
        self.assertIsNone(
            self.migration_session.query(Activity).filter_by(id=1).first()
        )
        ...
        with self.migration_metadata.bind.connect() as connection:
            ...
            self.assertEquals(
                activity_rows[0].created, datetime.datetime(2016, 8, 11, 16, 47, 56)
            )
            ...
            self.assertEquals(
                activity_rows[0].new_value,
                u"ValueError: Power script /usr/lib/python2.7/site-packages/bk",
            )
            self.assertEquals(activity_rows[2].id, 3)
            self.assertEquals(activity_rows[2].user_id, 1)
            self.assertEquals(
                activity_rows[2].created, datetime.datetime(2016, 9, 19, 11, 54, 7)
            )
            ...
            command_rows = connection.execute(
                "SELECT * FROM command_queue ORDER BY id"
            ).fetchall()
            ...
            self.assertEquals(
                command_rows[0].updated, datetime.datetime(2016, 8, 11, 16, 47, 56)
            )
            ...
            self.assertEquals(
                command_rows[1].updated, datetime.datetime(2016, 9, 19, 11, 54, 7)
            )
    ...
    def test_remove_task_arch_and_osmajor_exclude_orphans_duplicates(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                "INSERT INTO osmajor(id, osmajor) VALUES (1, 'redhat loonix');"
            )
            connection.execute(
        ...
        with self.migration_metadata.bind.connect() as connection:
            ...
            self.assertEqual(
                connection.scalar("SELECT count(*) FROM task_exclude_osmajor"), 1
            )
            self.assertEqual(
                connection.scalar("SELECT count(*) FROM task_exclude_arch"), 1
            )
    ...
    def test_abort_commands_without_lab_controller(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                pkg_resources.resource_string(
                    "bkr.inttest.server", "bz1362371-migration-setup.sql"
                )
            )
        ...
        self.assertEquals(
            self.migration_session.query(Command).filter_by(system_id=1).count(), 4
        )

        # Failed Command left in place
        self.assertEquals(
            self.migration_session.query(Command)
            .filter_by(status=CommandStatus.aborted, system_id=2)
            .count(),
            4,
        )
        ...
        self.assertEquals(
            self.migration_session.query(Command)
            .filter_by(status=CommandStatus.failed, system_id=2)
            .count(),
            1,
        )
    ...
    def test_marks_all_migrations_as_finished_on_fresh_db(self):
        ...
        with self.migration_session.begin():
            for name in DataMigration.all_names():
                migration = (
                    self.migration_session.query(DataMigration)
                    .filter(DataMigration.name == name)
                    .one()
                )
    ...
    def test_values_are_preserved_after_migration(self):
        ...
        with self.migration_metadata.bind.connect() as connection:
            self.assertEquals(
                connection.scalar(
                    "SELECT ipxe_image_id FROM openstack_region "
    ...
    def test_downgrading_task_exclusive_osmajors_converts_to_excluded_osmajors(self):
        ...
        with self.migration_metadata.bind.connect() as connection:
            osmajor_ids = connection.execute(
                "SELECT osmajor_id FROM task_exclude_osmajor WHERE task_id = 1"
            ).fetchall()
    ...
    def test_downgrading_task_exclusive_arches_converts_to_excluded_arches(self):
        ...
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute(
                "INSERT INTO task_exclusive_arch (task_id, arch_id) "
    ...
    def test_queued_recipe_has_installation_column_after_upgrade(self):
        with self.migration_metadata.bind.connect() as connection:
            ...
            # add a queued job
            connection.execute("INSERT INTO osmajor (osmajor) " "VALUES ('Tiara9')")
            connection.execute(
                "INSERT INTO osversion (osmajor_id, osminor) " "VALUES (1, 9)"
            )
            connection.execute(
        ...
        with self.migration_metadata.bind.connect() as connection:
            self.assertEqual(4, connection.scalar("SELECT count(*) FROM installation;"))
    ...
    def test_queue_admin_group_is_granted_permission(self):
        ...
        change_prio = (
            self.migration_session.query(Permission)
            .filter_by(permission_name=u"change_prio")
            .one()
        )
    ...
    def test_downgrading_upgrading_distro_element_will_not_leave_NULL_columns(self):
        ...
        with self.migration_metadata.bind.connect() as connection:
            ...
            connection.execute("INSERT INTO arch (id, arch) " "VALUES (2, 'i386')")
            connection.execute(
                "INSERT INTO recipe_set (job_id, queue_time, waived) "
    ...
    def test_upgrade_will_not_insert_duplicate_installation_rows(self):
        ...
        upgrade_db(self.migration_metadata)
        with self.migration_metadata.bind.connect() as connection:
            self.assertEqual(3, connection.scalar("SELECT count(*) FROM installation;"))
    ...
    def test_scheduled_recipe_has_installation_column_after_upgrade(self):
        ...
        migration = DataMigration(
            name=u"insert-installation-row-for-scheduled-recipes-before-25"
        )
        finished = migration.migrate_one_batch(self.migration_metadata.bind)
        self.assertTrue(finished)
        ...
        migration = DataMigration(
            name=u"insert-installation-row-for-recipes-before-25-take-2"
        )
        finished = migration.migrate_one_batch(self.migration_metadata.bind)
        self.assertTrue(finished)
        ...
        self.assertEqual(recipe.installation.osminor, u"9")
        with self.migration_metadata.bind.connect() as connection:
            self.assertEqual(1, connection.scalar("SELECT count(*) FROM installation;"))

    # https://bugzilla.redhat.com/show_bug.cgi?id=1568224
    def test_scheduled_recipe_has_installation_column_after_upgrade_when_cancelled(
        self,
    ):
        with self.migration_metadata.bind.connect() as connection:
            connection.execute(
        ...
        for recipe in self.migration_session.query(Recipe).all():
            ...
            self.assertEqual(recipe.installation.osminor, u"9")
        with self.migration_metadata.bind.connect() as connection:
            self.assertEqual(6, connection.scalar("SELECT count(*) FROM installation;"))
