...
class TestJobsController(DatabaseTestCase):
    ...
    def test_job_xml_can_be_roundtripped(self):
        ...
        roundtripped_xml = lxml.etree.tostring(
            job.to_xml(clone=True), pretty_print=True, encoding="utf8"
        )
    ...
    def test_job_with_custom_distro_without_optional_attributes_can_be_roundtripped(
        ...
        self.assertIn(
            '<osversion major="RedHatEnterpriseLinux7" minor="0"/>', roundtripped_xml
        )
    ...
    def test_complete_job_results(self):
        ...
        actual_results_xml = lxml.etree.tostring(
            job.to_xml(clone=False), pretty_print=True, encoding="utf8"
        )
    ...
    def test_preserves_arbitrary_XML_elements_in_namespace(self):
        ...
        self.assertEqual(
            u'<f:test xmlns:f="http://example.com/foo">unicode text: heißer Шис</f:test>'.encode(
                "utf8"
            ),
