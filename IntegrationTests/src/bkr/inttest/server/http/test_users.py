...
class UserHTTPTest(DatabaseTestCase):
    ...
    def test_get_self_for_proxied_user(self):
        ...
        s = requests.Session()
        response = s.post(get_server_base() + 'auth/login_password',
                          json={'username': proxying_user.user_name,
                                'password': u'password',
                                'proxy_user': proxied_user.user_name})
        response.raise_for_status()
        response = s.get(get_server_base() + 'users/+self',
                         headers={'Accept': 'application/json'})
        response.raise_for_status()
    ...
    def test_add_submission_delegate(self):
        ...
        with session.begin():
            session.expire_all()
            assertCountEqual(self, user.submission_delegates, [other_user])
