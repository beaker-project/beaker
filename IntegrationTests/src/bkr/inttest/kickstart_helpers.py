...
def create_x86_64_automated(lab_controller):
    return data_setup.create_system(
        arch=u"x86_64",
        fqdn=kickstart_system_fqdn,
        status=u"Automated",
...
def create_rhel62():
    try:
        ...
        rhel62 = data_setup.create_distro(
            name=u"RHEL-6.2", osmajor=u"RedHatEnterpriseLinux6", osminor=u"2"
        )
        return rhel62
...
def create_rhel62_server_x86_64(lab_controller):
    ...
    try:
        return DistroTree.query.filter_by(
            distro=rhel62, variant=u"Server", arch=x86_64
        ).one()
...
def compare_expected(name, recipe_id, actual):
    expected = pkg_resources.resource_string(
        "bkr.inttest", "server/kickstarts/%s.expected" % name
    )
    ...
    if expected != actual:
        ...
        diff = "".join(
            difflib.ndiff(expected.splitlines(True), actual.splitlines(True))
        )
        raise AssertionError(
            "Actual kickstart does not match expected %s\n%s" % (expected_path, diff)
        )
...
EOF
""",
                    "snippets/per_system/packages/bz728410-system-with-packages": "special-weird-driver-package\n",
                }
            ),
        ]
    )
