...
class PowerTest(LabControllerTestCase):
    ...
    def test_power_quiescent_period_statefulness_not_elapsed(self):
        ...
        self.assertIn(
            "Entering quiescent period, delaying 1 seconds for "
            "command %s" % system.command_queue[0].id,
            provision_output,
        )
    ...
    def test_quiescent_period_only_applies_between_power_commands(self):
        ...
        assert_command_is_delayed(
            commands[1], quiescent_period - 0.5, timeout=2 * loop_interval
        )
    ...
    def test_power_commands_are_not_run_twice(self):
        ...
        with session.begin():
            ...
            self.assertEquals(
                system.dyn_activity.filter_by(
                    field_name=u"Power", new_value=u"Completed"
                ).count(),
                3,
            )
    ...
    def test_blank_power_passwords(self):
        ...
        try:
            ...
            with session.begin():
                ...
                system.action_power(action=u"off", service=u"testdata")
            wait_for_commands_to_finish(
                system, timeout=2 * get_conf().get(u"SLEEP_TIME")
            )
        finally:
            provision_output = provision_process.finish_output_capture()
    ...
    def test_power_passwords_are_not_logged(self):
        ...
        try:
            ...
            with session.begin():
                ...
                system.action_power(action=u"off", service=u"testdata")
            wait_for_commands_to_finish(
                system, timeout=2 * get_conf().get("SLEEP_TIME")
            )
        finally:
            provision_output = provision_process.finish_output_capture()
        ...
        self.assert_(
            system.power.power_passwd not in provision_output, provision_output
        )
    ...
    def test_power_passwords_are_not_reported_in_failure_message(self):
        with session.begin():
            ...
            system.action_power(action=u"off", service=u"testdata")
        timeout = 2 * get_conf().get("SLEEP_TIME") + get_conf().get(
            "POWER_ATTEMPTS"
...
class ConfigureNetbootTest(LabControllerTestCase):
    ...
    def test_timeout_is_enforced_for_fetching_images(self):
        ...
        wait_for_commands_to_finish(
            system,
            timeout=(
                2 * get_conf().get("SLEEP_TIME") + get_conf().get("IMAGE_FETCH_TIMEOUT")
            ),
        )
    ...
    def test_system_not_marked_broken_for_missing_distro_tree_images(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                arch=u"x86_64", lab_controller=lc, status=SystemStatus.automated
            )
            self.addCleanup(self.cleanup_system, system)
            distro_tree = data_setup.create_distro_tree(
                arch=u"x86_64",
...
class ProvisionXmlrpcTest(TestCase):
    def test_clear_orphaned_commands(self):
        ...
        initial_commands = {1: {"id": 1}, 2: {"id": 2}}
