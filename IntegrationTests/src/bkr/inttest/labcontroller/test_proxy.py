...
from bkr.inttest.labcontroller import (
    LabControllerTestCase,
    processes,
    config_file,
    ...
    def setUp(self):
        with session.begin():
            self.job = data_setup.create_running_job(
                num_recipes=1, num_guestrecipes=1, lab_controller=self.get_lc()
            )
...
class GetRecipeTest(LabControllerTestCase):
    ...
    def check_recipe_xml(self, xml):
        ...
        self.assertEquals(
            root.find("./recipeSet/recipe").get("id"), str(self.recipe.id)
        )
...
class TaskResultTest(LabControllerTestCase):
    ...
    def test_xmlrpc_pass(self):
        s = xmlrpclib.ServerProxy(self.get_proxy_url())
        result_id = s.task_result(
            self.recipe.tasks[0].id, u"pass_", u"/random/junk", 123, u"The thing worked"
        )
        self.check_result(
            result_id, TaskResult.pass_, u"/random/junk", 123, u"The thing worked"
        )

    def test_xmlrpc_fail(self):
        s = xmlrpclib.ServerProxy(self.get_proxy_url())
        result_id = s.task_result(
            self.recipe.tasks[0].id, u"fail", u"/random/junk", 456, u"The thing failed"
        )
        self.check_result(
            result_id, TaskResult.fail, u"/random/junk", 456, u"The thing failed"
        )

    def test_xmlrpc_warn(self):
        s = xmlrpclib.ServerProxy(self.get_proxy_url())
        result_id = s.task_result(
            self.recipe.tasks[0].id, u"warn", u"/random/junk", -1, u"The thing broke"
        )
        self.check_result(
            result_id, TaskResult.warn, u"/random/junk", -1, u"The thing broke"
        )
    ...
    def test_xmlrpc_panic(self):
        ...
        self.check_result(
            result_id, TaskResult.panic, u"/random/junk", 0, u"The thing really broke"
        )

    def test_POST_pass(self):
        results_url = "%srecipes/%s/tasks/%s/results/" % (
            self.get_proxy_url(),
            self.recipe.id,
            self.recipe.tasks[0].id,
        )
        ...
        self.assert_(
            response.headers["Location"].startswith(results_url),
    ...
    def test_POST_none(self):
        ...
        response = requests.post(
            results_url,
            data=dict(
                result="None", path="/random/junk", message="See elsewhere for results"
            ),
        )
        self.assertEquals(response.status_code, 201)
        self.assert_(
    ...
    def test_POST_skip(self):
        ...
        response = requests.post(
            results_url, data=dict(result="Skip", path="/", message="Did not run")
        )
        self.assertEquals(response.status_code, 201)
        self.assert_(
            response.headers["Location"].startswith(results_url),
        ...
        result_id = int(posixpath.basename(response.headers["Location"]))
        self.check_result(result_id, TaskResult.skip, u"/", None, u"Did not run")
    ...
    def test_POST_missing_result(self):
        ...
        response = requests.post(
            results_url, data=dict(asdf="lol"), allow_redirects=False
        )
    ...
    def test_POST_unknown_result(self):
        ...
        response = requests.post(
            results_url, data=dict(result="Eggplant"), allow_redirects=False
        )
        self.assertEquals(response.status_code, 400)
    ...
    def test_result_for_finished_task(self):
        ...
        response = requests.post(
            results_url, data=dict(result="Pass"), allow_redirects=False
        )
    ...
    def test_max_results_per_recipe_limit_is_enforced(self):
        ...
        with session.begin():
            ...
            self.assertEqual(
                self.recipe.tasks[0].results[-1].log, u"Too many results in recipe"
            )
...
class TaskStatusTest(LabControllerTestCase):
    def setUp(self):
        with session.begin():
            self.recipe = data_setup.create_recipe(
                task_list=[data_setup.create_task(), data_setup.create_task()]
            )
            data_setup.create_job_for_recipes([self.recipe])
            data_setup.mark_recipe_running(self.recipe)
    ...
    def test_cannot_complete_aborted_task(self):
        ...
        try:
            ...
            self.assertIn("Cannot change status for finished task", fault.faultString)
        with session.begin():
            session.expire_all()
            task = self.recipe.tasks[0]
    ...
    def test_POST_task_abort(self):
        ...
        response = requests.post(
            status_url, data=dict(status="Aborted", message="fooed the bar up")
        )
        self.assertEquals(response.status_code, 204)
        with session.begin():
            session.expire_all()
...
class UpdateTaskTest(LabControllerTestCase):
    def setUp(self):
        with session.begin():
            self.recipe = data_setup.create_recipe(
                task_name=u"/distribution/check-install"
            )
    ...
    def test_start_and_complete_external_task(self):
        ...
        task_url = "%srecipes/%s/tasks/%s/" % (
            self.get_proxy_url(),
            self.recipe.id,
            self.recipe.tasks[2].id,
        )
        name = "/external/example"
        version = "3.14-1"
        response = requests.patch(
            task_url, data=dict(status="Running", name=name, version=version)
        )
        self.assertEquals(response.status_code, 200)
        json = response.json()
    ...
    def test_abort_task(self):
        ...
        response = requests.patch(
            task_url, data=dict(status="Aborted", message="fooed the bar up")
        )
        self.assertEquals(response.status_code, 200)
    ...
    def test_initial_watchdog_set_correctly(self):
        ...
        with session.begin():
            ...
            assert_datetime_within(
                self.recipe.watchdog.kill_time,
...
class RecipeStatusTest(LabControllerTestCase):
    ...
    def test_xmlrpc_recipe_abort(self):
        ...
        with session.begin():
            ...
            self.assertEquals(self.recipe.tasks[0].results[-1].log, u"fooed the bar up")
            self.assertEquals(self.recipe.tasks[1].status, TaskStatus.aborted)
            self.assertEquals(self.recipe.tasks[1].results[-1].log, u"fooed the bar up")

    def test_POST_recipe_abort(self):
        status_url = "%srecipes/%s/status" % (self.get_proxy_url(), self.recipe.id)
        response = requests.post(
            status_url, data=dict(status="Aborted", message="fooed the bar up")
        )
        self.assertEquals(response.status_code, 204)
...
class WatchdogTest(LabControllerTestCase):
    ...
    def test_GET_watchdog(self):
        ...
        watchdog_url = "%srecipes/%s/watchdog" % (self.get_proxy_url(), self.recipe.id)
...
class InstallStartTest(LabControllerTestCase):
    ...
    def test_install_start(self):
        ...
        with session.begin():
            session.expire_all()
            assert_datetime_within(
                self.recipe.installation.install_started,
                tolerance=datetime.timedelta(seconds=10),
    ...
    def test_install_start_GET(self):
        response = requests.get(
            "%sinstall_start/%s" % (self.get_proxy_url(), self.recipe.id)
        )
        response.raise_for_status()
        with session.begin():
            session.expire_all()
...
class InstallDoneTest(LabControllerTestCase):
    ...
    def test_install_done_GET(self):
        response = requests.get(
            "%sinstall_done/%s/%s" % (self.get_proxy_url(), self.recipe.id, "somefqdn")
        )
        response.raise_for_status()
        with session.begin():
            session.expire_all()
    ...
    def test_install_done_without_fqdn(self):
        response = requests.get(
            "%sinstall_done/%s/" % (self.get_proxy_url(), self.recipe.id)
        )
        response.raise_for_status()
        with session.begin():
            session.expire_all()
...
class InstallFailTest(LabControllerTestCase):
    ...
    def test_install_fail_get(self):
        response = requests.get(
            reduce(
                urljoin,
                [self.get_proxy_url(), "install_fail/", "{}".format(self.recipe.id)],
            )
        )
        response.raise_for_status()
        with session.begin():
            ...
            self.assertEquals(self.recipe.tasks[0].status, TaskStatus.aborted)
            self.assertEquals(
                self.recipe.tasks[0].results[-1].log, u"Installation failed"
            )
            self.assertEquals(self.recipe.tasks[1].status, TaskStatus.aborted)
            self.assertEquals(
                self.recipe.tasks[1].results[-1].log, u"Installation failed"
            )
...
class PostrebootTest(LabControllerTestCase):
    ...
    def test_postreboot_GET(self):
        response = requests.get(
            "%spostreboot/%s" % (self.get_proxy_url(), self.recipe.id)
        )
        response.raise_for_status()
        with session.begin():
            session.expire_all()
...
class LogUploadTestRestartProxy(LabControllerTestCase):
    ...
    def _run_modified_log_url(self, config_arg):
        ...
        s.recipe_upload_file(
            self.recipe.id, "/", "recipe-log", 10, None, 0, b64encode("a" * 10)
        )

        with session.begin():
            self.assertEquals(
                self.recipe.logs[0].server,
...
class LogUploadTest(LabControllerTestCase):
    ...
    def test_log_storage_base_url(self):
        ...
        self.assertEquals(
            proxy.log_storage.base_url, "https://testingme.com/beaker/logs/"
        )
    ...
    def test_xmlrpc_recipe_log(self):
        ...
        local_log_dir = "%s/recipes/%s+/%s/" % (
            get_conf().get("CACHEPATH"),
            self.recipe.id // 1000,
            self.recipe.id,
        )
        with session.begin():
        ...
        s.recipe_upload_file(
            self.recipe.id, "/", "recipe-log", 10, None, 10, b64encode("b" * 10)
        )
        with session.begin():
            self.assertEquals(
                open(os.path.join(local_log_dir, "recipe-log"), "r").read(),
    ...
    def test_xmlrpc_recipe_log_after_finished(self):
        ...
        try:
            ...
            self.assert_(
                "Cannot register file for finished recipe" in fault.faultString
            )

    def test_PUT_recipe_log(self):
        upload_url = "%srecipes/%s/logs/PUT-recipe-log" % (
            self.get_proxy_url(),
            self.recipe.id,
        )
        ...
        response = requests.put(
            upload_url, data="b" * 10, headers={"Content-Range": "bytes 10-19/20"}
        )
        self.assertEquals(response.status_code, 204)
        with session.begin():
            self.assertEquals(
    ...
    def test_xmlrpc_task_log(self):
        with session.begin():
            task = self.recipe.tasks[0]
        s = xmlrpclib.ServerProxy(self.get_proxy_url(), allow_none=True)
        s.task_upload_file(task.id, "/", "task-log", 10, None, 0, b64encode("a" * 10))
        local_log_dir = "%s/tasks/%s+/%s/" % (
            get_conf().get("CACHEPATH"),
            task.id // 1000,
            task.id,
        )
        with session.begin():
            ...
            self.assertEquals(
                open(os.path.join(local_log_dir, "task-log"), "r").read(), "aaaaaaaaaa"
            )
        s.task_upload_file(task.id, "/", "task-log", 10, None, 10, b64encode("b" * 10))
        with session.begin():
            self.assertEquals(
    ...
    def test_xmlrpc_task_log_after_finished(self):
        ...
        try:
            s.task_upload_file(
                task.id, "/", "task-log", 10, None, 0, b64encode("a" * 10)
            )
            self.fail("should raise")
        except xmlrpclib.Fault, fault:
            self.assert_("Cannot register file for finished task" in fault.faultString)

    def test_PUT_task_log(self):
        with session.begin():
            task = self.recipe.tasks[0]
        upload_url = "%srecipes/%s/tasks/%s/logs/PUT-task-log" % (
            self.get_proxy_url(),
            self.recipe.id,
            task.id,
        )
    ...
    def test_PUT_task_log_after_finished(self):
        with session.begin():
            ...
            task.stop()
            assert task.is_finished()
        upload_url = "%srecipes/%s/tasks/%s/logs/after-finished" % (
            self.get_proxy_url(),
            self.recipe.id,
            task.id,
        )
    ...
    def test_xmlrpc_result_log(self):
        with session.begin():
            ...
            result = self.recipe.tasks[0].results[0]
        s = xmlrpclib.ServerProxy(self.get_proxy_url(), allow_none=True)
        s.result_upload_file(
            result.id, "/", "result-log", 10, None, 0, b64encode("a" * 10)
        )
        local_log_dir = "%s/results/%s+/%s/" % (
            get_conf().get("CACHEPATH"),
            result.id // 1000,
            result.id,
        )
        ...
        s.result_upload_file(
            result.id, "/", "result-log", 10, None, 10, b64encode("b" * 10)
        )
        with session.begin():
            self.assertEquals(
                open(os.path.join(local_log_dir, "result-log"), "r").read(),
    ...
    def test_GET_nonexistent_log(self):
        log_url = "%srecipes/%s/logs/notexist" % (self.get_proxy_url(), self.recipe.id)
        response = requests.get(log_url)
        self.assertEquals(response.status_code, 404)

    # https://bugzilla.redhat.com/show_bug.cgi?id=961300
    def test_PUT_empty_log(self):
        upload_url = "%srecipes/%s/logs/empty-log" % (
            self.get_proxy_url(),
            self.recipe.id,
        )
        local_log_dir = "%s/recipes/%s+/%s/" % (
    ...
    def test_large_PUT_request_is_rejected(self):
        upload_url = "%srecipes/%s/logs/asdf" % (self.get_proxy_url(), self.recipe.id)
    ...
    def test_max_logs_per_recipe_limit_is_enforced(self):
        with session.begin():
            self.recipe.tasks[0].pass_(
                path=u"test-bz1293007", score=0, summary=u"Result with many logs"
            )
            task = self.recipe.tasks[0]
            result = task.results[0]
            # 4000 task logs, 3500 result logs
            session.execute(
                LogRecipeTask.__table__.insert().values(
                    [
                        {
                            "recipe_task_id": task.id,
                            "path": u"/",
                            "filename": u"bz1293007.log",
                            "start_time": datetime.datetime.utcnow(),
                        }
                        for i in range(4000)
                    ]
            ...
            session.execute(
                LogRecipeTaskResult.__table__.insert().values(
                    [
                        {
...
class LogIndexTest(LabControllerTestCase):
    ...
    def check_atom_index(self, response, logs):
        ...
        hrefs = [
            entry.find("{http://www.w3.org/2005/Atom}link").get("href")
    ...
    def test_result_log_index_html(self):
        url = "%srecipes/%s/tasks/%s/results/%s/logs/" % (
            self.get_proxy_url(),
            self.recipe.id,
            self.task.id,
            self.result.id,
