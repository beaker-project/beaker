...
config_file = os.environ.get(
    "BEAKER_LABCONTROLLER_CONFIG_FILE", "labcontroller-test.cfg"
)
...
class LabControllerTestCase(DatabaseTestCase):
    ...
    def _check_lc_leaks(self):
        with session.begin():
            ...
            watchdogs = (
                Watchdog.query.join(Watchdog.recipe)
            ...
            if watchdogs.count():
                ...
                raise AssertionError(
                    "Leaked watchdogs for %s: %s" % (lc_fqdn, watchdogs.all())
                )
            ...
            if systems.count():
                ...
                raise AssertionError(
                    "Leaked systems for %s: %s" % (lc_fqdn, systems.all())
                )
