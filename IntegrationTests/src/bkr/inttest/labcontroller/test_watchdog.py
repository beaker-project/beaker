...
from bkr.labcontroller.proxy import (
    ConsoleWatchFile,
    InstallFailureDetector,
    PanicDetector,
    ProxyHelper,
    Monitor,
)
...
class TestHelper(LabControllerTestCase):
    def assert_panic_detected(self, message):
        with session.begin():
            ...
            self.assertEquals(self.recipe.tasks[0].results[0].result, TaskResult.panic)
            self.assertEquals(self.recipe.tasks[0].results[0].log, message)

    def check_console_log_registered(self):
        with session.begin():
            return (
                LogRecipe.query.filter_by(
                    parent=self.recipe, filename=u"console.log"
                ).count()
                == 1
            )
    ...
    def check_console_log2_registered(self):
        with session.begin():
            return (
                LogRecipe.query.filter_by(
                    parent=self.recipe, filename=u"console-bmc.log"
                ).count()
                == 1
            )
...
class WatchdogConsoleLogTest(TestHelper):
    ...
    def setUp(self):
        ...
        self.console_log = os.path.join(
            get_conf().get("CONSOLE_LOGS"), self.system.fqdn
        )
        ...
        self.console_log_2 = os.path.join(
            get_conf().get("CONSOLE_LOGS"), "{}-bmc".format(self.system.fqdn)
        )
    ...
    def test_stores_console_log(self):
        ...
        wait_for_condition(
            lambda: self.check_cached_log_contents(first_line + second_line)
        )
    ...
    def test_stores_second_console_log(self):
        ...
        wait_for_condition(
            lambda: self.check_cached_log2_contents(first_line + second_line)
        )
    ...
    def test_stops_collecting_console_log_when_recipe_aborted(self):
        ...
        time.sleep(get_conf().get("SLEEP_TIME") * 2)
        self.assertTrue(
            self.check_cached_log_contents(first_line),
    ...
    def test_incomplete_lines_not_buffered_forever(self):
        ...
        long_panic_line = (
            "z" * (ConsoleWatchFile.blocksize - 10)
    ...
    def test_detects_arm_Oops_successfully(self):
        oops_line = (
            "Internal error: Oops - SP/PC alignment exception: 8a000000 [#1] SMP\n"
        )
        with open(self.console_log, "w") as f:
            f.write(oops_line)
        wait_for_condition(self.check_console_log_registered)
    ...
    def test_install_failure_is_not_reported_after_installation_is_finished(self):
        ...
        wait_for_condition(
            lambda: self.check_cached_log_contents(anaconda_success + anaconda_failure)
        )
        with session.begin():
            task = self.recipe.tasks[0]
            session.refresh(task)
    ...
    def test_console_log_not_recreated_after_removed(self):
        ...
        with session.begin():
            self.assertEquals(
                LogRecipe.query.filter_by(parent=self.recipe, filename=u"console.log")
                .one()
                .server,
                u"http://elsewhere",
            )
...
class WatchdogVirtConsoleLogTest(TestHelper):
    def setUp(self):
        with session.begin():
            ...
            data_setup.mark_recipe_running(
                self.recipe, virt=True, lab_controller=self.get_lc()
            )
            self.cached_console_log = os.path.join(
                get_conf().get("CACHEPATH"),
                "recipes",
        ...
        self.monitor = Monitor(
            {"recipe_id": self.recipe.id, "is_virt_recipe": True}, self.watchdog
        )
    ...
    def test_handles_non_ascii_characters_in_openstack_console(
        ...
        log_contents = (
            u"Firmware for Intel\ufffd\ufffd Wireless 5150 A/G/N network adaptors\n"
        )
...
class ExternalWatchdogScriptTest(LabControllerTestCase):
    ...
    def test_recipe_is_extended(self):
        ...
        with session.begin():
            system = data_setup.create_system(
                lab_controller=self.get_lc(), fqdn=u"watchdog.script.please.extend.600"
            )
    ...
    def test_recipe_is_aborted_on_script_failure(self):
        ...
        with session.begin():
            system = data_setup.create_system(
                lab_controller=self.get_lc(), fqdn=u"watchdog.script.please.crash"
            )
            job = data_setup.create_running_job(system=system)
            self.addCleanup(self.cleanup_job, job)
            recipe = job.recipesets[0].recipes[0]
...
def test_anaconda_failure_samples():
    for filename in pkg_resources.resource_listdir(
        "bkr.inttest.labcontroller", "install-failure-logs"
    ):
        yield check_anaconda_failure_sample, filename


def check_anaconda_failure_sample(filename):
    log = pkg_resources.resource_string(
        "bkr.inttest.labcontroller", "install-failure-logs/" + filename
    )
