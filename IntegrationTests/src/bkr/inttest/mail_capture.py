...
class CapturingSMTPServer(smtpd.SMTPServer):
    ...
    def process_message(self, peer, mailfrom, rcpttos, data):
        if self.capturing:
            log.debug("Captured mail from peer %r: %r", peer, (mailfrom, rcpttos, data))
    ...
    def close(self):
        ...
        for dispatcher in asyncore.socket_map.values():
            if (
                isinstance(dispatcher, smtpd.SMTPChannel)
                and dispatcher._SMTPChannel__server is self
            ):
                log.debug("%r: Forcing close of orphaned channel %r", self, dispatcher)
                dispatcher.close()
...
class MailCaptureThread(object):
    def start_capturing(self):
        if self.server.capturing:
            raise RuntimeError(
                "%r is already capturing, missing a call to stop_capturing?"
            )
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self.server.run, name="MailCaptureThread-%s" % id(self)
        )
