...
class JobModifyTest(ClientTestCase):
    ...
    def test_retention_tag_product(self):
        ...
        out = run_client(
            ["bkr", "job-modify", self.job.t_id, "--retention-tag", "%s" % rt1.tag]
        )
    ...
    def test_multiple_response_job(self):
        ...
        self.assert_(
            "Successfully modified jobs" in out
            and self.job.t_id in out
            and self.job_for_rs.t_id in out
        )
        with session.begin():
            session.expire_all()
            for rs in self.job.recipesets:
    ...
    def test_ack_rs(self):
        out = run_client(
            ["bkr", "job-modify", self.job.recipesets[0].t_id, "--response", "ack"]
        )
        self.assert_(
            out == "Successfully modified jobs %s\n" % self.job.recipesets[0].t_id
        )
        with session.begin():
            session.expire_all()
    ...
    def test_nak_rs(self):
        out = run_client(
            ["bkr", "job-modify", self.job.recipesets[0].t_id, "--response", "nak"]
        )
        self.assert_(
            out == "Successfully modified jobs %s\n" % self.job.recipesets[0].t_id
        )
    ...
    def test_multiple_response_recipeset(self):
        out = run_client(
            [
                ...
                "job-modify",
                self.job.recipesets[0].t_id,
                self.job_for_rs.recipesets[0].t_id,
                "--response",
                "nak",
            ]
        )
        self.assert_(
            "Successfully modified jobs" in out
            and self.job_for_rs.recipesets[0].t_id in out
            and self.job.recipesets[0].t_id in out,
        )
        with session.begin():
            session.expire_all()
            self.assertEqual(self.job.recipesets[0].waived, True)
    ...
    def test_record_retention_tag(self):
        ...
        out = run_client(
            ["bkr", "job-modify", self.job.t_id, "--retention-tag", "%s" % rt.tag]
        )
    ...
    def test_set_job_whiteboard(self):
        out = run_client(
            ["bkr", "job-modify", self.job.t_id, "--whiteboard", "Gregor Samsa awoke"]
        )
        self.assertIn("Successfully modified jobs %s" % self.job.t_id, out)
        with session.begin():
            session.expire_all()
