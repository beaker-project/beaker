...
from bkr.inttest.client import (
    run_client,
    start_client,
...
class WorkflowSimpleTest(ClientTestCase):
    ...
    def test_job_group(self):
        ...
        config1 = create_client_config(
            username=user_in_group.user_name, password="password"
        )
        out = run_client(
            [
                "bkr",
        ...
        config2 = create_client_config(
            username=user_not_in_group.user_name, password="password"
        )
        try:
            run_client(
                [
            ...
            self.assertTrue(
                ...
                % (user_not_in_group.user_name, group.group_name)
                in e.stderr_output,
                e,
            )
    ...
    def test_hostrequire(self):
        out = run_client(
            [
                ...
                "--prettyxml",
                "--hostrequire",
                "hostlabcontroller=lab.example.com",
                "--systype",
                "machine",
                "--arch",
                self.distro_tree.arch.arch,
    ...
    def test_hostfilter_preset(self):
        ...
        test_home = pkg_resources.resource_filename("bkr.inttest.client", ".")
        out = run_client(
            [
                "bkr",
                "workflow-simple",
        ...
        try:
            ...
            self.assertIn(
                "Pre-defined host-filter does not exist: awesomefilter", e.stderr_output
            )
    ...
    def test_machine_ignore_other_options(self):
        ...
        self.assertIn(
            "Warning: Ignoring --hostrequire because " "--machine was specified",
            err.split("\n"),
        )
        self.assertIn(
            "Warning: Ignoring --random because " "--machine was specified",
            err.split("\n"),
        )
        self.assertIn(
            "Warning: Ignoring --host-filter because " "--machine was specified",
            err.split("\n"),
        )
        self.assertNotIn('<hostlabcontroller op="=" value="lab.example.com"/>', out)
    ...
    def submit_job_and_check_arches(self, workflow_options, expected_arches):
        out = run_client(
            ["bkr", "workflow-simple", "--task", self.task.name] + workflow_options
        )
        self.assertTrue(out.startswith("Submitted:"), out)
        m = re.search("J:(\d+)", out)
        job_id = m.group(1)
        with session.begin():
            ...
            actual_arches = [
                rs.recipes[0].distro_tree.arch.arch for rs in job.recipesets
            ]
    ...
    def test_lookup_arches_by_family(self):
        ...
        with session.begin():
            distro = data_setup.create_distro(
                osmajor=u"DansAwesomeLinux7", tags=[u"STABLE"]
            )
        ...
        self.submit_job_and_check_arches(
            ["--family", distro.osversion.osmajor.osmajor], [u"x86_64", u"s390x"]
        )
    ...
    def test_looks_up_arches_for_suitable_osminor(self):
        ...
        self.submit_job_and_check_arches(
            ["--family", "DansAwesomeLinux8"], newer_arches
        )
    ...
    def test_kickstart_template(self):
        ...
        with session.begin():
            job = Job.by_id(job_id)
            self.assertEquals(job.recipesets[0].recipes[0].kickstart, template_contents)
...
%end
        ...
        with session.begin():
            job = Job.by_id(job_id)
            self.assertEquals(job.recipesets[0].recipes[0].kernel_options, "sshd=1")
    ...
    def test_can_do_dry_run_anonymously(self):
        config = create_client_config(
            username=u"ignored", password=u"ignored", auth_method=u"none"
        )
        out = run_client(
            [
                "bkr",
    ...
    def test_ignore_system_status(self):
        with session.begin():
            ...
            system = data_setup.create_system(
                lab_controller=lc, status=SystemStatus.broken
            )
        out = run_client(
            [
                "bkr",
