...
class JobSubmitTest(ClientTestCase):
    ...
    def test_submit_job(self):
        ...
        config = create_client_config(username=user.user_name, password=u"password")
        out = run_client(
            [
                "bkr",
                "job-submit",
    ...
    def test_missing_distro_and_distroRequires_element(self):
        ...
        self.assertIn(
            "You must define either <distroRequires/> or <distro/> element",
            str(cm.exception),
        )
    ...
    def test_delegate_submission_fails(self):
        ...
        error = cm.exception
        msg = "%s is not a valid submission delegate for %s" % (bot_name, user_bar_name)
