...
class JobDeleteTest(ClientTestCase):
    ...
    def test_delete_group_job(self):
        ...
        out = run_client(["bkr", "job-delete", self.job.t_id], config=client_config)
        self.assert_(out.startswith("Jobs deleted:"), out)
        self.assert_(self.job.t_id in out, out)

    def test_delete_job(self):
        out = run_client(
            ["bkr", "job-delete", self.job.t_id], config=self.client_config
        )
        self.assert_(out.startswith("Jobs deleted:"), out)
        self.assert_(self.job.t_id in out, out)
    ...
    def test_delete_others_job(self):
        ...
        try:
            out = run_client(
                ["bkr", "job-delete", other_job.t_id], config=self.client_config
            )
    ...
    def test_cant_delete_group_mates_job(self):
        ...
        try:
            run_client(["bkr", "job-delete", test_job.t_id], config=self.client_config)
            self.fail("We should not have permission to delete %s" % test_job.t_id)
        except ClientError as e:
            self.assertIn(
                "You don't have permission to delete job %s" % test_job.t_id,
                e.stderr_output,
            )
    ...
    def test_delete_job_with_admin(self):
        with session.begin():
            ...
            job2 = data_setup.create_completed_job(
                owner=other_user, retention_tag=tag.tag
            )
    ...
    def test_zero_value_completeDays(self):
        try:
            ...
            self.assertIn(
                "Please pass a positive integer to completeDays", e.stderr_output
            )
