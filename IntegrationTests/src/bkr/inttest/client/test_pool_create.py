...
class CreateSystemPool(ClientTestCase):
    def test_create_pool_defaults(self):
        ...
        try:
            ...
            self.assertIn(
                "System pool with name u'%s' already exists" % pool_name,
                e.stderr_output,
            )

    def test_create_pool_set_description(self):
        pool_name = data_setup.unique_name(u"mypool%s")
        run_client(["bkr", "pool-create", "--description", "My Pool", pool_name])
    ...
    def test_create_pool_set_owner(self):
        ...
        run_client(["bkr", "pool-create", "--owner", owner.user_name, pool_name])
        with session.begin():
            p = SystemPool.by_name(pool_name)
            self.assertFalse(p.owning_group)
            self.assertTrue(p.owning_user.user_name, owner.user_name)

        pool_name = data_setup.unique_name(u"mypool%s")

        owning_group = data_setup.create_group()
        run_client(
            ["bkr", "pool-create", "--owning-group", owning_group.group_name, pool_name]
        )
        with session.begin():
            p = SystemPool.by_name(pool_name)
        ...
        try:
            ...
            self.assertIn(
                "Only one of owner or owning-group must be specified", e.stderr_output
            )
