...
class SystemListTest(ClientTestCase):
    ...
    def test_list_systems_lc_disabled(self):
        with session.begin():
            ...
            system1 = data_setup.create_system(
                fqdn=data_setup.unique_name(u"aaaa%s.testdata")
            )
    ...
    def test_xml_filter(self):
        ...
        out = run_client(
            ["bkr", "system-list", "--xml-filter", '<key_value key="MODULE" />']
        )
    ...
    def test_predefined_host_filter(self):
        with session.begin():
            ...
            matching.cpu = Cpu(
                vendor=u"GenuineIntel", family=6, model=47, model_name=u"Intel"
            )
            nonmatching = data_setup.create_system()
        out = run_client(["bkr", "system-list", "--host-filter", "INTEL__WESTMERE"])
        returned_systems = out.splitlines()
    ...
    def test_multiple_xml_filters(self):
        with session.begin():
            ...
            matching.key_values_string.append(Key_Value_String(module_key, u"cciss"))
            nonmatching = data_setup.create_system()
        out = run_client(
            [
                "bkr",
                "system-list",
                "--xml-filter",
                '<not><lender value="shark"/></not>',
                "--xml-filter",
                '<key_value key="MODULE" value="cciss"/>',
                "--host-filter",
                "INTEL__WESTMERE",
            ]
        )
    ...
    def test_added_date_search(self):
        ...
        with session.begin():
            sys_today1 = data_setup.create_system(
                arch=u"i386", shared=True, date_added=time_now
            )
            sys_today2 = data_setup.create_system(
                arch=u"i386", shared=True, date_added=time_delta1
            )
            sys_tomorrow = data_setup.create_system(
                arch=u"i386", shared=True, date_added=time_tomorrow
            )
        ...
        out = run_client(
            [
                ...
                "system-list",
                "--xml-filter",
                "<system>" '<added op="=" value="%s" />' "</system>" % date_today,
            ]
        )
        ...
        try:
            run_client(
                [
                    ...
                    "system-list",
                    "--xml-filter",
                    "<system>" '<added op="=" value="%s" />' "</system>" % time_now,
                ]
            )
            self.fail("Must Fail or Die")
        except ClientError as e:
