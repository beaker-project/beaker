...
class GroupCreateTest(ClientTestCase):
    def test_group_create(self):
        ...
        try:
            _ = run_client(["bkr", "group-create", "areallylonggroupname" * 20])
            self.fail("Must fail or die")
        except ClientError as e:
            self.assertIn(
    ...
    def test_ldap_group(self):
        ...
        rand_client_config = create_client_config(
            username=rand_user.user_name, password="asdf"
        )
        ...
        try:
            ...
            self.assert_("Only admins can create LDAP groups" in e.stderr_output)
    ...
    def test_group_passwords(self):
        ...
        try:
            _ = run_client(
                ["bkr", "group-create", "--root-password", "fa1l", group_name]
            )
        ...
        try:
            _ = run_client(
                ["bkr", "group-create", "--root-password", "melanoma", group_name]
            )

            self.fail("Expected to fail due to dictionary words")
        except ClientError as e:
            self.assertTrue(
                "The group root password fails the dictionary check" in e.stderr_output,
                e.stderr_output,
            )
        out = run_client(
            ["bkr", "group-create", "--root-password", "Borrow or rob?", group_name]
        )
    ...
    def test_group_duplicate(self):
        ...
        try:
            ...
            self.assert_("Group already exists" in e.stderr_output, e.stderr_output)
