...
class UpdateInventoryTest(ClientTestCase):
    ...
    def test_update_inventory_xml(self):
        ...
        self.assertIn('<hostRequires force="%s"/>' % self.system1.fqdn, out)
        self.assertIn('<task name="/distribution/inventory" role="STANDALONE"/>', out)

    def test_update_inventory_wait(self):
        args = ["bkr", "update-inventory", "--wait", self.system1.fqdn]
