...
class SystemLoanTest(ClientTestCase):
    ...
    def test_owner_can_borrow_system(self):
        ...
        out = run_client(
            ["bkr", "loan-grant", self.system.fqdn], config=self.client_config
        )
        self.assertRegexpMatches("^Borrowed %s" % self.system.fqdn, out)
        self.assertLoanedTo(self.user)

        out = run_client(
            ["bkr", "loan-return", self.system.fqdn], config=self.client_config
        )
        self.assertRegexpMatches("^Returned loan for %s" % self.system.fqdn, out)
        self.assertNotLoaned()

    def test_loan_comment(self):
        # Set a comment while granting the loan
        out = run_client(
            ["bkr", "loan-grant", "--comment", "Mine! All mine!", self.system.fqdn],
    ...
    def test_owner_can_lend_system(self):
        ...
        self.assertRegexpMatches(
            "^Loaned %s to %s" % (self.system.fqdn, self.user2.user_name), out
        )
        ...
        out = run_client(
            ["bkr", "loan-return", self.system.fqdn], config=self.client_config2
        )
        self.assertRegexpMatches("^Returned loan for %s" % self.system.fqdn, out)
        self.assertNotLoaned()
    ...
    def test_admin_can_lend_system(self):
        ...
        out = run_client(
            ["bkr", "loan-grant", "--recipient", self.user.user_name, self.system.fqdn],
    ...
    def test_user_cannot_borrow_system(self):
        ...
        with self.assertTriggersPermissionsError(details):
            run_client(
                ["bkr", "loan-grant", self.system.fqdn], config=self.client_config2
            )
        self.assertNotLoaned()
