...
class UserModifyTest(ClientTestCase):
    def test_add_invalid_submission_delegate(self):
        ...
        try:
            ...
            self.assertTrue(
                "1thatdoesnotexist is not a valid user" in e.stderr_output,
                e.stderr_output,
            )
    ...
    def test_add_submission_delegate(self):
        ...
        out = run_client(
            ["bkr", "user-modify", "--add-submission-delegate", delegate.user_name],
            config=client_config,
        )
    ...
    def test_remove_invalid_submission_delegate(self):
        ...
        try:
            ...
            self.assertTrue(
                "%s is not a submission delegate of %s" % (notadelegate.user_name, user)
                in e.stderr_output,
                e.stderr_output,
            )
    ...
    def test_remove_submission_delegate(self):
        ...
        out = run_client(
            ["bkr", "user-modify", "--remove-submission-delegate", delegate1.user_name],
            config=client_config,
        )
