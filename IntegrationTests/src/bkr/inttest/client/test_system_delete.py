...
class SystemDeleteTest(ClientTestCase):
    ...
    def test_user_cannot_delete_system(self):
        ...
        try:
            out = run_client(
                ["bkr", "system-delete", self.system.fqdn], config=self.client_config2
            )
    ...
    def test_user_can_delete_system(self):
        ...
        out = run_client(
            ["bkr", "system-delete", self.system.fqdn], config=self.client_config
        )
    ...
    def test_admin_can_delete_system(self):
        ...
        out = run_client(
            ["bkr", "system-delete", self.system.fqdn], config=self.client_config3
        )
        self.assert_(out.startswith("Deleted %s" % self.system.fqdn), out)
