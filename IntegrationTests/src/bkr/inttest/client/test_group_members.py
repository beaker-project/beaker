...
class GroupMembersTest(ClientTestCase):
    def test_group_members(self):
        ...
        # list output
        out = run_client(["bkr", "group-members", "--format", "list", group.group_name])
        ...
        out = json.loads(out)
        self.assert_(
            dict(username=user2.user_name, email=user2.email_address, owner=True)
            in out,
            out,
        )
        self.assert_(
            dict(username=user1.user_name, email=user1.email_address, owner=False)
            in out,
            out,
        )
        ...
        try:
            ...
            self.assertIn(
                "Group %s does not exist" % non_existent_group, e.stderr_output
            )
