...
class LabControllerModifyTest(ClientTestCase):
    def test_change_fqdn(self):
        ...
        run_client(["bkr", "labcontroller-modify", "--fqdn", new_fqdn, lc.fqdn])
        with session.begin():
            session.refresh(lc)
    ...
    def test_change_fqdn_being_used_by_another_lab_controller(self):
        ...
        try:
            run_client(
                ["bkr", "labcontroller-modify", "--fqdn", another_lc.fqdn, lc.fqdn]
            )
            self.fail("Must error out")
        except ClientError as e:
            self.assertIn("FQDN %s already in use" % another_lc.fqdn, e.stderr_output)

    def test_change_user_name(self):
        with session.begin():
            lc = data_setup.create_labcontroller()
            user = data_setup.create_user()
        run_client(["bkr", "labcontroller-modify", "--user", user.user_name, lc.fqdn])
        with session.begin():
            session.expire_all()
    ...
    def test_change_password(self):
        with session.begin():
            lc = data_setup.create_labcontroller()
        run_client(
            ["bkr", "labcontroller-modify", "--password", u"newpassword", lc.fqdn]
        )
        with session.begin():
            session.expire_all()
    ...
    def test_change_email_address(self):
        ...
        run_client(
            [
                "bkr",
                "labcontroller-modify",
                "--email",
                u"newaddress@beaker-project.com",
                lc.fqdn,
            ]
        )
        with session.begin():
    ...
    def test_enable_the_lab_controller(self):
        with session.begin():
            ...
            lc.disabled = True
            user = data_setup.create_user()
        run_client(["bkr", "labcontroller-modify", "--enable", lc.fqdn])
        with session.begin():
            session.expire_all()
    ...
    def test_disable_the_lab_controller(self):
        with session.begin():
            lc = data_setup.create_labcontroller()
            user = data_setup.create_user()
        run_client(["bkr", "labcontroller-modify", "--disable", lc.fqdn])
        with session.begin():
            session.expire_all()
    ...
    def test_create_the_lab_controller_if_it_does_not_exist(self):
        with session.begin():
            ...
            user = data_setup.create_user()
        run_client(
            ["bkr", "labcontroller-modify", "--user", user.user_name, "--create", fqdn]
        )
        with session.begin():
