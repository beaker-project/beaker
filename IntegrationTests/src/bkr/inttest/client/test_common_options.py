...
class CommonOptionsTest(ClientTestCase):
    def test_hub(self):
        ...
        run_client(
            ["bkr", "--hub", get_server_base().rstrip("/"), "labcontroller-list"],
            config=config,
        )
    ...
    def test_on_error_warns_if_server_version_does_not_match(self):
        ...
        config = create_client_config(
            hub_url="http://localhost:19998", auth_method=u"none"
        )
        try:
            ...
            self.assertIn("HTTP error: 404 Client Error: Not Found", error_lines[1])
