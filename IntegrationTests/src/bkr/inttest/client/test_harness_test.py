...
class HarnessTest(ClientTestCase):
    ...
    def setUp(self):
        ...
        self.distro = data_setup.create_distro(
            osmajor=u"MyAwesomeLinux", tags=[u"STABLE"], arches=[i386, x86_64]
        )
    ...
    def test_machine_hostfilter(self):
        ...
        self.assertIn('<hostname op="=" value="test.system"/>', out)
        ...
        with session.begin():
            ...
            for recipe in new_job.all_recipes:
                ...
                if recipe.installation.osmajor == "Fedorarawhide":
                    self.assertEqual(
                        recipe.tasks[0].name, "/distribution/check-install"
                    )
