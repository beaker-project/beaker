...
class JobCloneTest(ClientTestCase):
    ...
    def test_can_print_xml(self):
        with session.begin():
            distro_tree = data_setup.create_distro_tree(
                distro_name=u"DansAwesomeLinux6.5-20160418.0"
            )
            job = data_setup.create_completed_job(
                whiteboard=u"foo", distro_tree=distro_tree
            )
        expected_xml = (
    ...
    def test_can_print_prettyxml(self):
        with session.begin():
            distro_tree = data_setup.create_distro_tree(
                distro_name=u"DansAwesomeLinux6.5-20160418.1"
            )
            job = data_setup.create_completed_job(
                whiteboard=u"foo", distro_tree=distro_tree
            )
  ...
  <recipeSet priority="Normal">
    ...
    def test_must_provide_job_or_recipeset(self):
        try:
            ...
            self.assertIn("Please specify a job or recipeset to clone", e.stderr_output)
