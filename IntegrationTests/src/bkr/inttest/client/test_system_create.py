...
class CreateSystem(ClientTestCase):
    def test_create_system_defaults(self):
        ...
        with session.begin():
            ...
            self.assertTrue(
                system.custom_access_policy.grants_everybody(SystemPermission.view)
            )
        # duplicate
        try:
            run_client(["bkr", "system-create", fqdn])
            ...
            self.assertIn("System with fqdn %r already exists" % fqdn, e.stderr_output)

    def test_create_system_set_labcontroller(self):
        fqdn = data_setup.unique_name(u"mysystem%s")
        lc = data_setup.create_labcontroller()
        run_client(["bkr", "system-create", "--lab-controller", str(lc.fqdn), fqdn])
        with session.begin():
            system = System.by_fqdn(fqdn, User.by_user_name(u"admin"))
    ...
    def test_create_system_set_arches(self):
        fqdn = data_setup.unique_name(u"mysystem%s")
        run_client(
            ["bkr", "system-create", "--arch", u"i386", "--arch", u"x86_64", fqdn]
        )
        with session.begin():
            system = System.by_fqdn(fqdn, User.by_user_name(u"admin"))
