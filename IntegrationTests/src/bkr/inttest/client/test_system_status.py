...
class SystemStatusTest(ClientTestCase):
    ...
    def test_reserve_with_recipe(self):
        ...
        # JSON output
        json_out = run_client(["bkr", "system-status", fqdn, "--format", "json"])
        json_out = loads(json_out)
        current_reservation = json_out.get("current_reservation")
        self.assertEqual(current_reservation.get("user_name"), unicode(system.user))
        self.assertEqual(
            current_reservation.get("recipe_id"),
            "%s" % system.open_reservation.recipe.id,
        )
    ...
    def test_reserve_manually(self):
        ...
        json_out = run_client(["bkr", "system-status", system.fqdn, "--format", "json"])
        json_out = loads(json_out)
