...
class PoolModifyTest(ClientTestCase):
    ...
    def test_pool_change_name(self):
        ...
        try:
            ...
            self.assertIn("System pool %s already exists" % pool1.name, e.stderr_output)

    def test_pool_change_description(self):
        with session.begin():
            pool = data_setup.create_system_pool()
        run_client(["bkr", "pool-modify", "--description", "Oh my pool", pool.name])
        with session.begin():
            session.refresh(pool)
    ...
    def test_pool_change_owner_to_group(self):
        ...
        run_client(["bkr", "pool-modify", "--owning-group", group_name, pool_name])
        with session.begin():
            session.refresh(pool)
    ...
    def test_pool_change_owner_to_another_user(self):
        with session.begin():
            ...
            pool = data_setup.create_system_pool()
        run_client(["bkr", "pool-modify", "--owner", new_owner, pool.name])
        with session.begin():
            session.refresh(pool)
