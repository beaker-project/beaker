...
class JobCommentTest(ClientTestCase):
    ...
    def test_post_comment_to_recipeset(self):
        ...
        out = run_client(["bkr", "job-comment", recipe.t_id, "--message", comment_text])
        with session.begin():
            session.expire_all()
    ...
    def test_post_comment_to_recipetask(self):
        ...
        out = run_client(["bkr", "job-comment", task.t_id, "--message", comment_text])
        with session.begin():
            session.expire_all()
    ...
    def test_post_comment_to_task_result(self):
        ...
        out = run_client(["bkr", "job-comment", result.t_id, "--message", comment_text])
    ...
    def test_empty_comment_is_rejected(self):
        try:
            run_client(
                ["bkr", "job-comment", self.job.recipesets[0].t_id, "--message", ""]
            )
            self.fail("should raise")
        except ClientError as e:
    ...
    def test_post_comment_to_tr_taskspec_string_fails(self):
        ...
        try:
            run_client(
                ["bkr", "job-comment", "TR:thisisnotanint", "--message", comment_text]
            )
            self.fail("should raise")
        except ClientError as e:
