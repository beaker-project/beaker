...
class JobListTest(ClientTestCase):
    ...
    def setUp(self):
        ...
        self.users = [
            data_setup.create_user(password="mypass") for _ in range(jobs_to_generate)
        ]
        ...
        self.jobs = [
            data_setup.create_completed_job(
                product=self.products[x], owner=self.users[x], group=self.groups[x]
            )
            for x in range(jobs_to_generate)
        ]
        self.client_configs = [
            create_client_config(username=user.user_name, password="mypass")
    ...
    def test_list_jobs_by_product(self):
        ...
        self.assertRaises(
            ClientError, run_client, ["bkr", "job-list", "--product", "foobar"]
        )
    ...
    def test_list_jobs_by_owner(self):
        ...
        out = run_client(
            ["bkr", "job-list", "--owner", self.users[0].user_name, "--limit", "1"]
        )
        ...
        out = run_client(
            [
                ...
                "job-list",
                "--owner",
                self.users[0].user_name,
                "--min-id",
                "{0}".format(self.jobs[0].id),
                "--max-id",
                "{0}".format(self.jobs[0].id),
            ]
        )
    ...
    def test_list_jobs_by_whiteboard_substring(self):
        with session.begin():
            included_job = data_setup.create_completed_job(
                whiteboard=u"Prince of Penzance"
            )
            excluded_job = data_setup.create_completed_job(
                whiteboard=u"Princess of Persia"
            )
    ...
    def test_list_jobs_mine(self):
        ...
        self.assertRaises(
            ClientError,
            run_client,
            ["bkr", "job-list", "--mine", "--username", "xyz", "--password", "xyz"],
        )

    def test_list_jobs_my_groups(self):
        out = run_client(
            ["bkr", "job-list", "--my-groups"], config=self.client_configs[0]
        )
        self.assert_(self.jobs[0].t_id in out and self.jobs[1].t_id not in out, out)

        out = run_client(
            ["bkr", "job-list", "--my-groups"], config=self.client_configs[1]
        )
        self.assert_(self.jobs[1].t_id in out and self.jobs[0].t_id not in out, out)

        out = run_client(
            ["bkr", "job-list", "--my-groups", "--format", "json"],
            config=self.client_configs[0],
        )
        out = json.loads(out)
        self.assertIn(self.jobs[0].t_id, out)
    ...
    def test_list_jobs_by_group(self):
        ...
        out = run_client(
            ["bkr", "job-list", "--group", self.groups[1].group_name, "--limit", "1"]
        )
        self.assert_(len(json.loads(out)) == 1, out)
    ...
    def test_list_jobs_both_by_mine_and_owner(self):
        out = run_client(
            ["bkr", "job-list", "--mine", "--owner", self.users[1].user_name],
            config=self.client_configs[0],
        )
    ...
    def test_cannot_specify_finished_and_unfinished_at_the_same_time(self):
        try:
            ...
            self.assertIn(
                "Only one of --finished or --unfinished may be specified",
                e.stderr_output,
            )
