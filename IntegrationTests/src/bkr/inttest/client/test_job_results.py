...
class JobResultsTest(ClientTestCase):
    ...
    def test_by_recipe(self):
        out = run_client(["bkr", "job-results", self.job.recipesets[0].recipes[0].t_id])
        self.assert_(out.startswith("<recipe "))

    def test_by_recipetask(self):
        out = run_client(
            ["bkr", "job-results", self.job.recipesets[0].recipes[0].tasks[0].t_id]
        )
    ...
    def test_job_result_does_not_fail_from_pre_upgrade_recipes(self):
        with session.begin():
            distro_tree = data_setup.create_distro_tree(
                distro_name=u"DansAwesomeLinux6.9-48",
