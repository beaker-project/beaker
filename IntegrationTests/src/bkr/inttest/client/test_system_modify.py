...
class ModifySystemTest(ClientTestCase):
    ...
    def test_change_owner(self):
        ...
        with session.begin():
            ...
            for system in systems:
                ...
                self.assertEquals(system.activity[-1].new_value, new_owner.user_name)

        # invalid user
        try:
            run_client(["bkr", "system-modify", "--owner", "idontexist", system1.fqdn])
            self.fail("Must raise")
        except ClientError as e:
        ...
        try:
            ...
            self.assertIn(
                "Insufficient permissions: Cannot edit system", e.stderr_output
            )
    ...
    def test_invalid_condition(self):
        ...
        try:
            run_client(
                ["bkr", "system-modify", "--condition=Unconditional", system.fqdn]
            )
            self.fail("Must raise")
        except ClientError as e:
    ...
    def test_modify_active_access_policy(self):
        ...
        with session.begin():
            ...
            for s in [system1, system2]:
                ...
                self.assertEquals(
                    s.activity[-1].new_value, "Pool policy: %s" % pool.name
                )
        ...
        try:
            run_client(
                [
                    ...
                    "system-modify",
                    "--pool-policy",
                    data_setup.create_system_pool().name,
                    system1.fqdn,
                ]
            )
        except ClientError as e:
        ...
        run_client(
            ["bkr", "system-modify", "--use-custom-policy", system1.fqdn, system2.fqdn]
        )
    ...
    def test_cannot_change_active_policy_without_permission(self):
        ...
        try:
            ...
            self.assertIn("Cannot edit system access policy", e.stderr_output)
    ...
    def test_change_power_type(self):
        ...
        run_client(
            [
                "bkr",
                "system-modify",
                "--power-type=apc_snmp_then_etherwake",
                system.fqdn,
            ]
        )
        with session.begin():
            session.refresh(system)
            self.assertEquals(
                system.power.power_type, PowerType.by_name(u"apc_snmp_then_etherwake")
            )
        # set back to ilo
    ...
    def test_change_power_address(self):
        ...
        run_client(
            ["bkr", "system-modify", "--power-address=%s" % address, system.fqdn]
        )
        with session.begin():
            session.refresh(system)
    ...
    def test_change_power_password(self):
        ...
        run_client(
            ["bkr", "system-modify", "--power-password=dummypassword", system.fqdn]
        )
        with session.begin():
            session.refresh(system)
        ...
        run_client(
            ["bkr", "system-modify", "--power-password=%s" % password, system.fqdn]
        )
        with session.begin():
            session.refresh(system)
