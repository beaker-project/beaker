...
class MachineTestTest(ClientTestCase):
    ...
    def test_machine_test(self):
        ...
        try:
            out = run_client(
                ["bkr", "machine-test", "--machine", self.system.fqdn, "--tag", "aTAG"]
            )
    ...
    def test_no_requested_tasks(self):
        ...
        with session.begin():
            ...
            self.assertEqual(
                rawhide_recipe.tasks[0].name, "/distribution/check-install"
            )
            rhel7_recipe = new_job.recipesets[1].recipes[0]
            self.assertEqual(
                rhel7_recipe.installation.osmajor, u"RedHatEnterpriseLinux7"
            )
