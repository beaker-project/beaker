...
class GroupModifyTest(ClientTestCase):
    ...
    def test_group_modify_no_criteria(self):
        try:
            out = run_client(
                ["bkr", "group-modify", self.group.group_name],
                config=self.client_config,
            )
            self.fail("Must fail or die")
        except ClientError as e:
            self.assert_(
                "Please specify an attribute to modify" in e.stderr_output,
                e.stderr_output,
            )
    ...
    def test_group_nonexistent(self):
        ...
        try:
            ...
            self.assert_(
                "Group group-like-non-other does not exist" in e.stderr_output,
                e.stderr_output,
            )
    ...
    def test_group_modify_display_name(self):
        ...
        with session.begin():
            ...
            self.assertEquals(group.activity[-1].user.user_id, self.user.user_id)
    ...
    def test_group_modify_group_name(self):
        ...
        out = run_client(
            ["bkr", "group-modify", "--group-name", group_name, self.group.group_name],
            config=self.client_config,
        )
        ...
        try:
            ...
            self.assertIn(
                "Group name must be not more than 255 characters long", e.stderr_output
            )
    ...
    def test_group_modify_password(self):
        ...
        try:
            ...
            self.assertTrue(
                re.search(
                    "The group root password is shorter than . characters", str(e)
                )
            )
            session.expire(self.group)
            with session.begin():
    ...
    def test_group_modify_group_name_duplicate(self):
        ...
        try:
            ...
            self.assert_(
                "Group %s already exists" % group1.group_name in e.stderr_output
            )
    ...
    def test_admin_cannot_rename_protected_group(self):
        ...
        try:
            ...
            self.assert_(
                "Cannot rename protected group" in e.stderr_output, e.stderr_output
            )
    ...
    def test_group_modify_add_member(self):
        ...
        try:
            ...
            self.assert_(
                "User idontexist does not exist" in e.stderr_output, e.stderr_output
            )
        ...
        try:
            ...
            self.assert_(
                "Cannot edit membership of group %s" % self.fake_ldap_group.group_name
                in e.stderr_output,
                e.stderr_output,
            )
    ...
    def test_group_modify_remove_member(self):
        ...
        with session.begin():
            session.refresh(self.group)
            group = Group.by_name(self.group.group_name)
            self.assert_(user.user_name not in [u.user_name for u in group.users])
        ...
        try:
            ...
            self.assert_("Cannot remove user" in e.stderr_output, e.stderr_output)
        ...
        for usr in group_users:
            if usr.user_id != 1:
                out = run_client(
                    ["bkr", "group-modify", "--remove-member", usr.user_name, "admin"],
                    config=self.admin_client_config,
                )

        try:
            out = run_client(
                ["bkr", "group-modify", "--remove-member", "admin", "admin"]
            )
            self.fail("Must fail or die")
        except ClientError as e:
            self.assert_("Cannot remove user" in e.stderr_output, e.stderr_output)
    ...
    def test_group_modify_grant_owner(self):
        ...
        try:
            ...
            self.assert_(
                "Cannot edit ownership of group" in e.stderr_output, e.stderr_output
            )
    ...
    def test_escapes_uri_characters_in_group_name(self):
        ...
        run_client(
            ["bkr", "group-modify", bad_group_name, "--add-member", self.user.user_name]
        )
        run_client(
            [
                "bkr",
