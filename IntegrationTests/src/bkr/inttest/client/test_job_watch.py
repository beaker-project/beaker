...
from bkr.inttest.client import start_client, run_client, ClientError, ClientTestCase
...
class JobWatchTest(ClientTestCase):
    def test_watch_job(self):
        ...
        self.assertEquals(
            p.stdout.readline(), "Watching tasks (this may be safely interrupted)...\n"
        )
    ...
    def test_watch_recipetask(self):
        ...
        self.assertEquals(
            p.stdout.readline(), "%s /distribution/reservesys: New\n" % recipetask.t_id
        )
        self.assertEquals(p.stdout.readline(), "--> New: 1 [total: 1]\n")
        with session.begin():
            data_setup.mark_job_complete(job)
