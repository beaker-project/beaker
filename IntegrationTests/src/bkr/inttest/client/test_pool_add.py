...
class TestSystemPoolAdd(ClientTestCase):
    ...
    def test_add_systems_to_non_existent_pool(self):
        ...
        with session.begin():
            s1 = data_setup.create_system()
        try:
            run_client(["bkr", "pool-add", "--pool", pool_name, "--system", s1.fqdn])
            self.fail("Must fail")
    ...
    def test_add_nonexistent_system_to_pool(self):
        ...
        try:
            run_client(
                ["bkr", "pool-add", "--pool", pool_name, "--system", system_fqdn]
            )
            self.fail("Must fail")
        except ClientError as e:
            self.assertIn("System '%s' does not exist" % system_fqdn, e.stderr_output)
    ...
    def test_add_systems_pool_privileges(self):
        ...
        with session.begin():
            ...
            pool = data_setup.create_system_pool(name=pool_name, owning_user=pool_owner)
        try:
            run_client(
                ["bkr", "pool-add", "--pool", pool_name, "--system", s1.fqdn],
                config=create_client_config(
                    username=system_owner.user_name, password="password"
                ),
            )
            self.fail("Must fail")
        except ClientError as e:
            self.assertIn(
                "You do not have permission to" " add systems to pool %s" % pool_name,
                e.stderr_output,
            )
        ...
        try:
            run_client(
                ["bkr", "pool-add", "--pool", pool_name, "--system", s1.fqdn],
                config=create_client_config(
                    username=pool_owner.user_name, password="password"
                ),
            )
            self.fail("Must fail")
        except ClientError as e:
            self.assertIn(
                "You do not have permission to" " modify system %s" % s1.fqdn,
                e.stderr_output,
            )
