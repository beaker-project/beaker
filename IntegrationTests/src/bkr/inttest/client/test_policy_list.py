...
class PolicyListTest(ClientTestCase):
    ...
    def test_list_policy(self):
        ...
        expected_output = (
            self.gen_expected_pretty_table(
                (
        ...
        out = run_client(
            ["bkr", "policy-list", self.system_public.fqdn, "--format", "tabular"]
        )
        expected_output = (
            self.gen_expected_pretty_table(
        ...
        out = run_client(["bkr", "policy-list", self.system.fqdn, "--format", "json"])
        out = json.loads(out)
    ...
    def test_private_system(self):
        ...
        out = run_client(
            ["bkr", "policy-list", system.fqdn], config=owner_client_config
        )
        self.assertNotEqual("", out)
        # to an unprivileged user the system should appear not to exist
        try:
            run_client(["bkr", "policy-list", system.fqdn], config=other_client_config)
    ...
    def test_list_policy_filter_mine(self):
        ...
        out = run_client(["bkr", "policy-list", "--mine", self.system.fqdn])
        expected_output = (
            self.gen_expected_pretty_table((["edit_system", "admin", "X", "No"],))
            + "\n"
        )
        self.assertEquals(out, expected_output)
    ...
    def test_list_policy_filter_user(self):
        ...
        out = run_client(
            [
                ...
                "policy-list",
                "--user",
                self.user1.user_name,
                "--user",
                self.user2.user_name,
                self.system.fqdn,
            ]
        )
        expected_output = (
        ...
        with session.begin():
            self.system.active_access_policy = self.pool.access_policy
        out = run_client(
            ["bkr", "policy-list", "--user", self.user3.user_name, self.system.fqdn]
        )
        expected_output = (
            self.gen_expected_pretty_table(
    ...
    def test_list_policy_filter_multiple(self):
        try:
            ...
            self.assertIn("Only one filtering criteria allowed", e.stderr_output)
