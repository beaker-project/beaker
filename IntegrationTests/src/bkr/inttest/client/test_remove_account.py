...
class RemoveAccountTest(ClientTestCase):
    ...
    def test_non_admin_cannot_delete(self):
        with session.begin():
            ...
            client_config1 = create_client_config(
                username=user3.user_name, password="qwerty"
            )
        try:
            # it's okay to use the same user since we won't reach there anyway :-)
            run_client(
                ["bkr", "remove-account", user3.user_name], config=client_config1
            )
            self.fail("Must fail or die")
        except ClientError as e:
    ...
    def test_remove_account(self):
        ...
        with session.begin():
            ...
            self.assertEqual(
                reserved_system.activity[1].user.user_name, data_setup.ADMIN_USER
            )
            ...
            self.assertEqual(
                loaned_system.activity[0].user.user_name, data_setup.ADMIN_USER
            )
            ...
            self.assertEqual(
                ...
                [
                    rule
                    for rule in reserved_system.custom_access_policy.rules
                    if rule.user == user
                ],
            )
            self.assertEqual(
                reserved_system.activity[0].user.user_name, data_setup.ADMIN_USER
            )
            self.assertEqual(
                reserved_system.activity[0].field_name, u"Access Policy Rule"
            )
            self.assertEqual(reserved_system.activity[0].action, u"Removed")
            self.assertEqual(
                reserved_system.activity[0].old_value,
            ...
            self.assertEqual(
                owned_system.activity[0].user.user_name, data_setup.ADMIN_USER
            )
            ...
            self.assertEqual(
                owned_pool.activity[0].user.user_name, data_setup.ADMIN_USER
            )
    ...
    def test_close_account_transfer_ownership(self):
        ...
        run_client(
            [
                "bkr",
                "remove-account",
                "--new-owner=%s" % new_owner.user_name,
                user.user_name,
            ]
        )
    ...
    def test_invalid_newowner_errors(self):
        ...
        with session.begin():
            ...
            self.assertFalse(
                session.query(User).filter_by(user_name=invalid_username).count()
            )
