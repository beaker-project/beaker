...
class TaskAddTest(ClientTestCase):
    def test_add_invalid_task(self):
        try:
            ...
            self.assert_(
                "error reading package header" in e.stderr_output, e.stderr_output
            )
    ...
    def test_add_new_task_successfully(self):
        ...
        new_rpm = path.join(
            config.get("basepath.rpms"), "tmp-test-add-new-task-successfully.noarch.rpm"
        )
    ...
    def test_adding_task_with_releases_list(self):
        ...
        rpm_path = pkg_resources.resource_filename(
            "bkr.inttest.server",
        ...
        with session.begin():
            ...
            self.assertItemsEqual(
                [OSMajor.by_name(u"RedHatEnterpriseLinux5")], task.exclusive_osmajors
            )

    def test_cannot_add_duplicate_tasks(self):
        import_rpm = pkg_resources.resource_filename(
            "bkr.inttest.server",
    ...
    def test_cannot_add_task_without_specifying_name(self):
        unde_rpm = pkg_resources.resource_filename(
    ...
    def test_cannot_add_task_with_a_very_long_name(self):
        overlen_rpm = pkg_resources.resource_filename(
            "bkr.inttest.server",
    ...
    def test_cannot_add_task_with_name_having_redundant_slashes(self):
        redundant_rpm = pkg_resources.resource_filename(
            "bkr.inttest.server",
        ...
        try:
            ...
            self.assertEqual(e.status, 1)
            self.assertIn(
                "Task name must not contain redundant slashes", e.stderr_output
            )

    def test_cannot_add_task_with_name_having_slash_end(self):
        slash_rpm = pkg_resources.resource_filename(
            "bkr.inttest.server",
