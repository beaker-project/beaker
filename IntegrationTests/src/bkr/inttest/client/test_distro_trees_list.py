...
class DistroTreesListTest(DatabaseTestCase):
    ...
    def test_list_by_distro_id(self):
        output = run_client(
            [
                ...
                "--format=json",
                "--distro-id",
                str(self.distro_tree.distro.id),
            ]
        )
        trees = json.loads(output)
        self.assertEquals(len(trees), 1)
    ...
    def test_filtering_by_lab_controller(self):
        with session.begin():
            ...
            distro_tree_in.lab_controller_assocs.append(
                LabControllerDistroTree(
                    lab_controller=good_lc, url=u"http://notimportant"
                )
            )
            distro_tree_out.lab_controller_assocs.append(
                LabControllerDistroTree(
        ...
        self.assert_(
            not any(distro_tree_out.id == tree["distro_tree_id"] for tree in trees)
        )
    ...
    def test_filtering_by_treepath(self):
        ...
        output = run_client(
            ["bkr", "distro-trees-list", "--format=json", "--treepath", "%somewhere%"]
        )
        trees = json.loads(output)
        self.assert_(any(distro_tree_in.id == tree["distro_tree_id"] for tree in trees))
        self.assert_(
    ...
    def test_tabular_format_works(self):
        ...
        self.assert_(
            "OSVersion: %s" % self.distro_tree.distro.osversion in output, output
        )
    ...
    def test_output_is_ordered_by_date_created(self):
        ...
        with session.begin():
            session.expire_all()
            returned_trees = [
                DistroTree.query.get(entry["distro_tree_id"])
                for entry in json.loads(output)
            ]
            for i in range(1, len(returned_trees)):
                self.assertGreaterEqual(
                    returned_trees[i - 1].date_created, returned_trees[i].date_created
                )
