...
class TestSystemPoolRemove(ClientTestCase):
    def test_remove_systems_from_pool(self):
        ...
        with session.begin():
            ...
            pool = data_setup.create_system_pool(name=pool_name, systems=[s1, s2, s3])
        run_client(
            [
                "bkr",
                "pool-remove",
        ...
        try:
            run_client(["bkr", "pool-remove", "--pool", pool_name, "--system", s4.fqdn])
            self.fail("Must fail")
        except ClientError as e:
            self.assertIn(
                "System %s is not in pool %s" % (s4.fqdn, pool_name), e.stderr_output
            )
    ...
    def test_remove_systems_pool_privileges(self):
        ...
        with session.begin():
            ...
            pool = data_setup.create_system_pool(
                name=pool_name, owning_user=pool_owner, systems=[s1, s2, s3]
            )
        run_client(
        ...
        try:
            run_client(
                ...
                config=create_client_config(
                    username=random_user.user_name, password="password"
                ),
            )
            self.fail("Must fail")
        except ClientError as e:
