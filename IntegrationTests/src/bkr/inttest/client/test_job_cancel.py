...
class JobCancelTest(ClientTestCase):
    ...
    def test_can_cancel_group_job(self):
        ...
        out = run_client(["bkr", "job-cancel", self.job.t_id], config=client_config)

    def test_cannot_cancel_recipe(self):
        try:
            run_client(["bkr", "job-cancel", self.job.recipesets[0].recipes[0].t_id])
            self.fail("should raise")
        except ClientError as e:
            self.assert_(
                "Taskspec type must be one of" in e.stderr_output, e.stderr_output
            )
