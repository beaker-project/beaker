...
class JobLogsTest(ClientTestCase):
    def setUp(self):
        ...
        with session.begin():
            ...
            self.recipe.logs[:] = [
                LogRecipe(server=self.log_server_url, path=u"R", filename=u"dummy.txt")
            ]
            os.mkdir(os.path.join(self.logs_dir, "T"))
            open(os.path.join(self.logs_dir, "T", "dummy.txt"), "w").write("task\n")
            self.recipe.tasks[0].logs[:] = [
                LogRecipeTask(
                    server=self.log_server_url, path=u"T", filename=u"dummy.txt"
                )
            ]
            os.mkdir(os.path.join(self.logs_dir, "TR"))
            open(os.path.join(self.logs_dir, "TR", "dummy.txt"), "w").write("result\n")
            self.recipe.tasks[0].results[0].logs[:] = [
                LogRecipeTaskResult(
                    server=self.log_server_url, path=u"TR", filename=u"dummy.txt"
                )
            ]
    ...
    def test_by_job(self):
        ...
        logs = out.splitlines()
        self.assertEquals(
            logs[0], get_server_base() + u"recipes/%s/logs/R/dummy.txt" % self.recipe.id
        )
        self.assertEquals(
            logs[1],
            get_server_base()
            + u"recipes/%s/tasks/%s/logs/T/dummy.txt"
            % (self.recipe.id, self.recipe.tasks[0].id),
        )
        self.assertEquals(
            logs[2],
            get_server_base()
            + u"recipes/%s/tasks/%s/results/%s/logs/TR/dummy.txt"
            % (
                self.recipe.id,
    ...
    def test_by_recipe(self):
        out = run_client(["bkr", "job-logs", self.job.recipesets[0].recipes[0].t_id])
        logs = out.splitlines()
        self.assertEquals(
            logs[0], get_server_base() + u"recipes/%s/logs/R/dummy.txt" % self.recipe.id
        )
    ...
    def test_by_task(self):
        out = run_client(
            ["bkr", "job-logs", self.job.recipesets[0].recipes[0].tasks[0].t_id]
        )
        logs = out.splitlines()
        self.assertEquals(
            logs[0],
    ...
    def test_prints_sizes(self):
        ...
        self.assertEquals(
            lines[0],
            "     7 %srecipes/%s/logs/R/dummy.txt"
            % (get_server_base(), self.recipe.id),
        )
        self.assertEquals(
            lines[1],
            "     5 %srecipes/%s/tasks/%s/logs/T/dummy.txt"
            % (get_server_base(), self.recipe.id, self.recipe.tasks[0].id),
        )
        self.assertEquals(
            lines[2],
            "     7 %srecipes/%s/tasks/%s/results/%s/logs/TR/dummy.txt"
            % (
                get_server_base(),
                self.recipe.id,
                self.recipe.tasks[0].id,
    ...
    def test_size_handles_404(self):
        ...
        self.assertEquals(
            lines[0],
            "<missing> %srecipes/%s/logs/R/idontexist.txt"
            % (get_server_base(), self.recipe.id),
        )
    ...
    def test_size_handles_http_errors(self):
        ...
        self.assertEquals(
            lines[0],
            "<error:500> %srecipes/%s/logs/error/500"
            % (get_server_base(), self.recipe.id),
        )
