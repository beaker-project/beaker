...
CONFIGURED_PATHS = {
    ...
    "armlinux": (("arm", "pxelinux.cfg", netboot.pxe_basename(TEST_ADDRESS)),),
    "pxelinux": (("pxelinux.cfg", netboot.pxe_basename(TEST_ADDRESS)),),
    "ipxe": (("ipxe", netboot.pxe_basename(TEST_ADDRESS).lower()),),
    "efigrub": (("grub", netboot.pxe_basename(TEST_ADDRESS)),),
    "zpxe": (
        ("s390x", "s_%s_conf" % TEST_FQDN),
        ("s390x", "s_%s_parm" % TEST_FQDN),
    ),
    "elilo": ((netboot.pxe_basename(TEST_ADDRESS) + ".conf",),),
    "yaboot": (
...
class NetBootTestCase(unittest.TestCase):
    ...
    def check_netboot_absent(self, category):
        ...
        for path in paths:
            self.assertFalse(
                os.path.lexists(path), "Unexpected %r file: %r" % (category, path)
            )
    ...
    def check_netboot_configured(self, category):
        ...
        for path in paths:
            self.assertTrue(
                os.path.lexists(path), "Missing %r file: %r" % (category, path)
            )
    ...
    def check_netboot_cleared(self, category):
        ...
        for path in persistent:
            self.assertTrue(
                ...
                "Missing persistent %r file: %r" % (category, path),
            )
...
class ImagesBaseTestCase(NetBootTestCase):
    def setUp(self):
        ...
        self.kernel = tempfile.NamedTemporaryFile(
...
class ImagesTest(ImagesBaseTestCase):
    def test_fetch_then_clear(self):
        ...
        self.assertEqual(4 * 1024 * 1024, os.path.getsize(kernel_path))
        self.assertEqual(8 * 1024 * 1024, os.path.getsize(initrd_path))
...
""",
    ...
    def test_configure_then_clear(self):
        netboot.configure_pxelinux(
            TEST_FQDN, "console=ttyS0,115200 ks=http://lol/", self.tftp_root
        )
...
""",
    ...
    def test_multiple_initrds(self):
        netboot.configure_pxelinux(
            TEST_FQDN, "initrd=/mydriverdisk.img ks=http://lol/", self.tftp_root
        )
        pxelinux_config_path = os.path.join(self.tftp_root, "pxelinux.cfg", "7F0000FF")
        self.assertEqual(
            open(pxelinux_config_path).read(),
...
class ZpxeTest(NetBootTestCase):
    def test_configure_then_clear(self):
        ...
        self.assertEqual(
            open(
                os.path.join(self.tftp_root, "s390x", "s_fqdn.example.invalid")
            ).read(),
            """ftp://lab.example.invalid/kernel.img
ftp://lab.example.invalid/initrd.img
...
""",
        ...
        self.assertEqual(
            open(
                os.path.join(self.tftp_root, "s390x", "s_fqdn.example.invalid_parm")
            ).read(),
...
""",
        ...
        self.assertEqual(
            open(
                os.path.join(self.tftp_root, "s390x", "s_fqdn.example.invalid_conf")
            ).read(),
            "",
        )
...
class Grub2PPC64Test(NetBootTestCase):
    def test_configure_then_clear(self):
        netboot.configure_ppc64(
            TEST_FQDN, "console=ttyS0,115200 ks=http://lol/", self.tftp_root
        )
        grub2_configs_path = [
            os.path.join(self.tftp_root, "ppc", "grub.cfg-7F0000FF"),
...
""",
        ...
        self.assertEqual(
            os.readlink(grub2_symlink_path), "../boot/grub2/powerpc-ieee1275/core.elf"
        )
...
class Grub2TestX8664(NetBootTestCase):
    def test_configure_then_clear(self):
        netboot.configure_x86_64(
            TEST_FQDN, "console=ttyS0,115200 ks=http://lol/", self.tftp_root
        )
        grub2_configs_path = [
            os.path.join(self.tftp_root, "x86_64", "grub.cfg-7F0000FF"),
            os.path.join(self.tftp_root, "boot", "grub2", "grub.cfg-7F0000FF"),
        ]
        grub2_default_path = [
...
""",
    ...
    def test_alternate_devicetree(self):
        netboot.configure_aarch64(
            TEST_FQDN, "devicetree=custom.dtb ks=http://lol/", self.tftp_root
        )
        grub_config_path = os.path.join(self.tftp_root, "aarch64", "grub.cfg-7F0000FF")
        self.assertEqual(
            open(grub_config_path).read(),
...
class PetitbootTest(NetBootTestCase):
    def test_configure_then_clear(self):
        netboot.configure_petitboot(
            TEST_FQDN, "ks=http://lol/ ksdevice=bootif", self.tftp_root
        )
        petitboot_config_path = os.path.join(
            self.tftp_root, "bootloader", TEST_FQDN, "petitboot.cfg"
        )
        self.assertEqual(
            open(petitboot_config_path).read(),
            """\
...
class NetbootloaderTest(ImagesBaseTestCase):
    def test_configure_then_clear(self):
        ...
        bootloader_config_symlink = os.path.join(
            self.tftp_root, "bootloader", TEST_FQDN, "image"
        )
        ...
        grub2_config_file = os.path.join(
            self.tftp_root, "bootloader", TEST_FQDN, "grub.cfg-7F0000FF"
        )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.tftp_root, "bootloader", TEST_FQDN))
        )
