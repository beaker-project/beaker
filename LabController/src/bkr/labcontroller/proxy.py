...
class ProxyHelper(object):
    ...
    def recipe_upload_file(self, recipe_id, path, name, size, md5sum, offset, data):
        ...
        with self.log_storage.recipe(
            str(recipe_id), os.path.join(path, name)
        ) as log_file:
    ...
    def task_result(
        ...
        return self.hub.recipes.tasks.result(
            task_id, result_type, result_path, result_score, result_summary
        )
...
class ConsoleWatchLogFiles(object):
    ...
    def __init__(self, logdir, system_name, watchdog, proxy, panic):
        ...
        for filename, logfile_name in get_console_files(
            ...
            logger.info(
                "Watching console log file %s for recipe %s",
                filename,
                self.watchdog["recipe_id"],
            )
    ...
    def _load_patterns(self):
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                ...
                raise
        package_patterns = pkg_resources.resource_listdir(
            "bkr.labcontroller", "install-failure-patterns"
        )
        ...
        for p in package_patterns:
            patterns.append(
                pkg_resources.resource_string(
                    "bkr.labcontroller", "install-failure-patterns/" + p
                )
            )
        return patterns
...
class LogArchiver(ProxyHelper):
    ...
    def transfer_recipe_logs(self, recipe_id):
        ...
        try:
            ...
            rsync_succeeded = self.rsync(
                "%s/" % tmpdir, "%s" % self.conf.get("ARCHIVE_RSYNC")
            )
            ...
            self.hub.recipes.change_files(
                recipe_id,
                self.conf.get("ARCHIVE_SERVER"),
    ...
    def rsync(self, src, dst):
        ...
        if p.returncode != 0:
            logger.error(
                "Failed to rsync recipe logs from %s to %s\nExit status: %s\n%s",
                src,
                dst,
                p.returncode,
                err,
...
class Monitor(ProxyHelper):
    ...
    def __init__(self, watchdog, obj, *args, **kwargs):
        ...
        if self.watchdog["is_virt_recipe"]:
            logger.info(
                "Watching OpenStack console for recipe %s", self.watchdog["recipe_id"]
            )
            ...
            self.console_watch = ConsoleWatchLogFiles(
                ...
                system_name=self.watchdog["system"],
                watchdog=self.watchdog,
                proxy=self,
                panic=self.conf["PANIC_REGEX"],
            )
    ...
    def report_panic(self, watchdog, panic_message):
        logger.info(
            "Panic detected for recipe %s on system %s: "
            "console log contains string %r",
            watchdog["recipe_id"],
            watchdog["system"],
            panic_message,
        )
        job = lxml.etree.fromstring(
            self.get_my_recipe(dict(recipe_id=watchdog["recipe_id"]))
        )
    ...
    def report_install_failure(self, watchdog, failure_message):
        logger.info(
            "Install failure detected for recipe %s on system %s: "
            "console log contains string %r",
            watchdog["recipe_id"],
            watchdog["system"],
            failure_message,
        ...
        elif recipe.find("installation") is not None and recipe.find(
            "installation"
        ).get("install_finished"):
...
class Proxy(ProxyHelper):
    def task_upload_file(self, task_id, path, name, size, md5sum, offset, data):
        """Upload a file in chunks
         path: the relative path to upload to
         name: the name of the file
         size: size of the contents (bytes)
        ...
        # Originally offset=-1 had special meaning, but that was unused
        logger.debug(
            "task_upload_file task_id:%s name:%s offset:%s size:%s",
            task_id,
            name,
            offset,
            size,
    ...
    def clear_netboot(self, fqdn):
        ...
        p = subprocess.Popen(
            ["sudo", "/usr/bin/beaker-clear-netboot", fqdn],
    ...
    def result_upload_file(self, result_id, path, name, size, md5sum, offset, data):
        ...
        # Originally offset=-1 had special meaning, but that was unused
        logger.debug(
            "result_upload_file result_id:%s name:%s offset:%s size:%s",
            result_id,
            name,
            offset,
            size,
        )
        with self.log_storage.result(
            str(result_id), os.path.join(path, name)
        ) as log_file:
            log_file.update_chunk(base64.decodestring(data), int(offset or 0))
        return True
...
class ProxyHTTP(object):
    ...
    def get_recipe(self, req, recipe_id):
        if (
            req.accept_mimetypes.provided
            and "application/xml" not in req.accept_mimetypes
        ):
            raise NotAcceptable()
        return Response(
            self.hub.recipes.to_xml(recipe_id), content_type="application/xml"
        )
    ...
    def post_result(self, req, recipe_id, task_id):
        ...
        try:
            result_id = self.hub.recipes.tasks.result(
                task_id,
            ...
            if "Cannot record result for finished task" in fault.faultString:
                return Response(
                    status=409, response=fault.faultString, content_type="text/plain"
                )
            elif "Too many results in recipe" in fault.faultString:
                return Response(
                    status=403, response=fault.faultString, content_type="text/plain"
                )
            else:
                raise
        return redirect(
            "/recipes/%s/tasks/%s/results/%s" % (recipe_id, task_id, result_id),
            code=201,
        )
    ...
    def post_recipe_status(self, req, recipe_id):
        ...
        self.hub.recipes.stop(recipe_id, "abort", req.form.get("message"))
    ...
    def _update_status(self, task_id, status, message):
        ...
        try:
            ...
            if (
                "Cannot restart finished task" in fault.faultString
    ...
    def patch_task(self, request, recipe_id, task_id):
        ...
        return Response(
            status=200, response=json.dumps(updated), content_type="application/json"
        )
    ...
    def _get_log(self, log_file, req):
        ...
        return Response(
            status=200,
            response=wrap_file(req.environ, f),
    ...
    def _html_log_index(self, logs):
        hrefs = [
            os.path.join((log["path"] or "").lstrip("/"), log["filename"])
            for log in logs
        ]
        lis = [
            "<li><a href=%s>%s</a></li>" % (xml_quoteattr(href), xml_escape(href))
            for href in hrefs
        ]
    ...
    def _atom_log_index(self, logs):
        ...
        entries = [
            '<entry><link rel="alternate" href=%s /><title type="text">%s</title></entry>'
            % (xml_quoteattr(href), xml_escape(href))
            for href in hrefs
        ]
    ...
    def _log_index(self, req, logs):
        if not req.accept_mimetypes.provided:
            ...
            response_type = req.accept_mimetypes.best_match(
