...
def check_nfs(tree):
    ...
    if not os.path.exists(server_path):
        raise NFSServerInaccessible(
            "Cannot access NFS server %s "
            "or autofs not running (%s does not exist)" % (nfs_server, server_path)
        )
...
def check_all_trees(
            ...
            for lc, url in distro_tree["available"]:
                try:
                    if check_url(url):
                        ...
                        print(
                            "{0} is missing [Distro Tree ID {1}]".format(
                                url, distro_tree["distro_tree_id"]
                            )
                        )
                except (
                    ...
                    if ignore_errors:
                        ...
                        sys.stderr.write(
                            "Error checking for existence of URL %s "
                            "for distro tree %s:\n%s\n"
    ...
    if len(distro_trees) != len(rdistro_trees) or remove_all:
        for distro_tree in rdistro_trees:
            if dry_run:
                print(
                    "Distro marked for remove %s:%d"
                    % (distro_tree["distro_name"], distro_tree["distro_tree_id"])
                )
            else:
                print(
                    "Removing distro %s:%d"
