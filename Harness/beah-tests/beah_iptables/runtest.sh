...
dbg_run_noout iptables -D INPUT -p tcp -m state --state NEW -m tcp --dport 12432 -j ACCEPT
dbg_run iptables -I INPUT -p tcp -m state --state NEW -m tcp --dport 12432 -j ACCEPT && \
