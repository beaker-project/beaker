...
from bkr.server.model import System, SystemStatus, SystemActivity, SystemStatusDuration
from bkr.server.test.assertions import (
...
def populate_status_durations(system):
    ...
    if status is None:
        status = get_status(u"Broken")
    system.status_durations.append(
        SystemStatusDuration(status=status, start_time=start_time, finish_time=None)
    )
