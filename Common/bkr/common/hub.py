...
if six.PY2:
    from bkr.common.xmlrpc2 import (
        CookieTransport,
        SafeCookieTransport,
        ...
        **kwargs
        ...
        if transport is not None:
            ...
            if self._hub_url.startswith("https://"):
                ...
                elif hasattr(ssl, "_create_unverified_context") and not self._conf.get(
                    "SSL_VERIFY", True
                ):
    ...
    def _login(self, force=False):
        ...
        if not hasattr(self, login_method_name):
            raise ImproperlyConfigured(
                "Unknown authentication method: %s" % self._auth_method
            )
    ...
    def _login_krbv(self):
        ...
        if keytab:
            ...
            store = {
                ...
                "ccache": ccache
                or tempfile.NamedTemporaryFile(prefix="krb5cc_bkr_").name,
            }
        elif ccache:
            store = {"ccache": ccache}
        ...
        try:
            res = gssapi.raw.init_sec_context(
                target_name,
                creds,
                flags=(
            ...
            if (
                ex.min_code == krb5kdc_err_s_principal_unknown
            ):  # pylint: disable=no-member
                ex.message += (
                    ". Make sure you correctly set KRB_REALM (current value: %s)."
                    % realm
                )
                ex.args = (ex.message,)
            raise ex
