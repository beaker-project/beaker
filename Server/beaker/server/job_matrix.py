...
class JobMatrix:
    ...
    def inner_data_grid(self,data,show_headers): 
        ...
        my_list = []
        for objs in data:
            whiteboard_title = whiteboard_name = orig_whiteboard_name= objs[0]
            if not whiteboard_name:
                whiteboard_name = 'N/A'
                whiteboard_title = 'N/A'
            my_list += list(objs[self.OBJECTS_TUPLE_INDEX]) 
            fields.append(InnerGrid.Column(name='%s' % (whiteboard_name),
                                           getter=self.display_whiteboard_results(orig_whiteboard_name), 
