...
class build_py_and_kid(build_py):
    ...
    def byte_compile(self, files):
        ...
        try:
            ...
            log.warn(
                "Kid templates cannot be compiled," " because Kid is not installed."
            )
            return
        if self.dry_run:
            return
...
def find_data_recursive(dest_dir, src_dir, exclude=frozenset()):
    ...
    for dirpath, dirnames, filenames in os.walk(src_dir):
        ...
        yield (
            os.path.join(dest_dir, dirpath[len(src_dir) :]),
            [
...
data_files = list(find_data_recursive("bkr/server/static", "bkr/server/static/")) + [
...
setup(
    ...
    long_description="Beaker is a system for full stack software integration testing "
