...
window.SystemAccessPolicyView = Backbone.View.extend({
    ...
    initialize: function (options) {
        this.listenTo(this.model, 'change:can_edit_policy change:pools', this.render);
        this.render();
    },
    ...
    submit: function (evt) {
        ...
        if (policy_type == 'pool') {
            ...
            this.model.save({'active_access_policy': {'custom': true}},
                    {patch: true, wait: true})
                .then(function () { return policy.save_access_policy()})
                .done(_.bind(this.render, this))
                .fail(_.bind(this.sync_error, this));
        }
    },
