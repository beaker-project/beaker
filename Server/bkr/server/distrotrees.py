...
class DistroTrees(RPCRoot):
    ...
    def index(self, **kwargs):
        ...
        grid = myPaginateDataGrid(
            fields=[
                ...
                myPaginateDataGrid.Column(
                    name="distro.osversion.osmajor.osmajor",
                    title=u"OS Major Version",
                    options=dict(sortable=True),
                ),
                myPaginateDataGrid.Column(
                    name="distro.osversion.osminor",
                ...
                Utility.direct_column(
                    title=u"Provision", getter=self._provision_system_link
                ),
            ]
        )
    ...
    def _provision_system_link(self, distro_tree):
        return make_link(
            "/reserveworkflow/?distro_tree_id=%s" % distro_tree.id,
            "Provision",
            elem_class="btn",
        )
    ...
    def default(self, id, *args, **kwargs):
        ...
        form_task = TaskSearchForm(
            action="/tasks/do_search",
            hidden=dict(arch_id=True, distro=True, osmajor_id=True),
            options=dict(),
        )
        lab_controllers = (
            LabController.query.filter_by(removed=None)
            .order_by(LabController.fqdn)
            .all()
        )
        ...
        if not kwargs.get("lab"):
            ...
            try:
                ...
                raise cherrypy.HTTPError(
                    status=400, message="No such lab controller %r" % kwargs["lab"]
                )
        base = distro_tree.url_in_lab(lc, scheme="http")
        if not base:
            raise cherrypy.HTTPError(
                status=404, message="%s is not present in lab %s" % (distro_tree, lc)
            )
...
"""
    ...
    def filter(self, filter):
        ...
        """
        query = (
            DistroTree.query.join(
                DistroTree.distro, Distro.osversion, OSVersion.osmajor
            )
            .join(DistroTree.arch)
            .options(contains_eager(DistroTree.distro), contains_eager(DistroTree.arch))
        )
