...
from bkr.server.flask_util import BadRequest400, convert_internal_errors, auth_required
...
def doit():
    ...
    if job_details["pick"] == "fqdn":
        try:
            job_details["system"] = System.by_fqdn(
                request.form.get("system"), identity.current.user
            )
    ...
    return (
        "Created %s" % job.t_id,
        201,
        [("Location", absolute_url("/jobs/%s" % job.id))],
    )
...
def get_unsupported_lab_controllers():
    ...
    for distro_tree_id in distro_tree_ids:
        try:
            ...
            unsupported_lab_controllers[name] = [
                lab_controller.fqdn
                for lab_controller in LabController.query.filter(
                    and_(
                        LabController.disabled == 0,
                        not_(
                            LabController._distro_trees.any(
                                LabControllerDistroTree.distro_tree_id == distro_tree_id
                            )
                        ),
                    )
                ).all()
...
class ReserveWorkflow:
    ...
    def index(self, **kwargs):
        ...
        options["lab"] = [
            lc.fqdn for lc in LabController.query.filter(LabController.removed == None)
        ]
        return dict(title=_(u"Reserve Workflow"), selection=kwargs, options=options)
    ...
    def _get_distro_options(self, osmajor=None, tag=None, system=None, **kwargs):
        ...
        if not osmajor:
            return []
        distros = (
            Distro.query.join(Distro.osversion, OSVersion.osmajor)
    ...
    def _get_distro_tree_options(self, distro=None, system=None, **kwargs):
        ...
        try:
            ...
            return []
        trees = (
            distro.dyn_trees.join(DistroTree.arch)
