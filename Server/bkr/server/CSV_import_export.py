...
class CSV(RPCRoot):
    ...
    upload = widgets.FileField(
        name="csv_file", label="Import CSV", help_text=import_help_text
    )
    download = RadioButtonList(
        name="csv_type",
        label="CSV Type",
        options=[
            ("system", "Systems"),
    ...
    def _import_row(self, data, log):
        if data["csv_type"] in system_types and ("fqdn" in data or "id" in data):
            ...
            raise ValueError(
                "Invalid csv_type %s or missing required fields" % data["csv_type"]
            )
    ...
    def action_import(self, csv_file, *args, **kw):
        ...
        try:
            ...
            for data in reader:
                ...
                if missing in data:
                    log.append(
                        "Too many fields on line %s (expecting %s)"
                        % (reader.line_num, len(reader.fieldnames))
                    )
                    continue
                if any(value is missing for value in data.itervalues()):
                    missing_fields = [
                        field for field, value in data.iteritems() if value is missing
                    ]
                    log.append(
                        "Missing fields on line %s: %s"
    ...
    def _to_csv(cls, file):
        ...
        for item in cls.query():
            for data in cls.to_datastruct(item):
                ...
                writer.writerow(
                    dict((k, unicode(v).encode("utf8")) for k, v in data.iteritems())
                )
...
class CSV_System(CSV):
    ...
    spec_keys = ["arch", "lab_controller", "owner", "status", "type", "cc"]
    ...
    def _from_csv(cls, system, data, csv_type, log):
            ...
            if data["arch"]:
                ...
                for arch in arches:
                    try:
                        ...
                        raise ValueError("%s: Invalid arch %s" % (system.fqdn, arch))
        ...
        if "owner" in data:
            if data["owner"]:
                ...
                if not owner:
                    raise ValueError(
                        "%s: Invalid User %s" % (system.fqdn, data["owner"])
                    )
                if owner.removed:
                    raise ValueError(
                        "%s: user %s is deleted" % (system.fqdn, owner.user_name)
                    )
            else:
                owner = None
        ...
        if "status" in data and data["status"]:
            try:
                ...
                raise ValueError(
                    "%s: Invalid Status %s" % (system.fqdn, data["status"])
                )
        ...
        if "type" in data:
            ...
            try:
                ...
                raise ValueError("%s: Invalid Type %s" % (system.fqdn, data["type"]))
...
class CSV_Power(CSV):
    csv_type = "power"
    reg_keys = ["fqdn", "power_address", "power_user", "power_passwd", "power_id"]
    ...
    def _from_csv(cls, system, data, csv_type, log):
        ...
        if "power_type" in data:
            ...
            try:
                ...
                log.append(
                    "%s: Invalid Power Type %s" % (system.fqdn, data["power_type"])
                )
                return False
...
class CSV_Exclude(CSV):
    ...
    def _from_csv(cls, system, data, csv_type, log):
        ...
        if data["update"] and data["family"]:
            try:
                osversion = OSVersion.by_name(
                    OSMajor.by_name(unicode(data["family"])), unicode(data["update"])
                )
            except InvalidRequestError:
                log.append(
                    "%s: Invalid Family %s Update %s"
                    % (system.fqdn, data["family"], data["update"])
                )
                return False
            if osversion not in [
                oldosversion.osversion
                for oldosversion in system.excluded_osversion_byarch(arch)
            ]:
                if data["excluded"] == "True":
                    exclude_osversion = ExcludeOSVersion(osversion=osversion, arch=arch)
        ...
        if not data["update"] and data["family"]:
            try:
                ...
                log.append("%s: Invalid family %s " % (system.fqdn, data["family"]))
                return False
            if osmajor not in [
                oldosmajor.osmajor
                for oldosmajor in system.excluded_osmajor_byarch(arch)
            ]:
                    ...
                    for old_osmajor in system.excluded_osmajor_byarch(arch):
                        if old_osmajor.osmajor == osmajor:
                            system.record_activity(
                                ...
                                action=u"Removed",
                                field=u"Excluded_families",
                                old=u"%s/%s" % (old_osmajor.osmajor, arch),
                                new=u"",
                            )
    ...
    def _from_csv(cls, system, data, csv_type, log):
        ...
        if "family" in data:
            ...
            if family:
                try:
                    ...
                    log.append(
                        "%s: Error! Invalid family %s" % (system.fqdn, data["family"])
                    )
                    return False
        if "update" in data:
            update = data["update"]
            if update:
                ...
                try:
                    ...
                    log.append(
                        "%s: Error! Invalid update %s" % (system.fqdn, data["update"])
                    )
                    return False
        ...
        if "ks_meta" in data and prov.ks_meta != data["ks_meta"]:
            system.record_activity(
                ...
                service=u"CSV",
                action=u"Changed",
                field=u"InstallOption:ks_meta:%s" % installlog,
                old=prov.ks_meta,
                new=data["ks_meta"],
            )
        ...
        if "kernel_options" in data and prov.kernel_options != data["kernel_options"]:
            system.record_activity(
                ...
                service=u"CSV",
                action=u"Changed",
                field=u"InstallOption:kernel_options:%s" % installlog,
                old=prov.kernel_options,
                new=data["kernel_options"],
            )
            prov.kernel_options = data["kernel_options"]
        if (
            "kernel_options_post" in data
            and prov.kernel_options_post != data["kernel_options_post"]
        ):
            system.record_activity(
                user=identity.current.user,
                service=u"CSV",
    ...
    def to_datastruct(self):
        ...
        if self.install.provision_families:
            for family in self.install.provision_families.keys():
                ...
                datastruct["kernel_options_post"] = (
                    prov_family.kernel_options_post or ""
                )
                yield datastruct
                if prov_family.provision_family_updates:
                    for update in prov_family.provision_family_updates.keys():
                        ...
                        datastruct["kernel_options_post"] = (
                            prov_update.kernel_options_post or ""
                        )
                        yield datastruct
                    datastruct["update"] = ""
...
class CSV_KeyValue(CSV):
    ...
    def query(cls):
        for system in System.all(identity.current.user):
            for key_int in (
                Key_Value_Int.query.join("system")
                .yield_per(10000)
                .filter(System.id == system.id)
            ):
                yield CSV_KeyValue(key_int)
            for key_string in (
                Key_Value_String.query.join("system")
                .yield_per(10000)
                .filter(System.id == system.id)
            ):
    ...
    def _from_csv(cls, system, data, csv_type, log):
        ...
        if "key_value" in data and data["key_value"]:
            if key.numeric:
                ...
                try:
                    key_value = Key_Value_Int.by_key_value(
                        system, key, data["key_value"]
                    )
                except InvalidRequestError:
                    key_value = Key_Value_Int(key=key, key_value=data["key_value"])
            else:
                system_key_values = system.key_values_string
                try:
                    key_value = Key_Value_String.by_key_value(
                        system, key, data["key_value"]
                    )
                except InvalidRequestError:
                    key_value = Key_Value_String(key=key, key_value=data["key_value"])
        else:
        ...
        if deleted:
            if key_value in system_key_values:
                system.record_activity(
                    ...
                    action=u"Removed",
                    field=u"Key/Value",
                    old=u"%s/%s" % (data["key"], data["key_value"]),
                    new=u"",
                )
    ...
    def from_csv(cls, user, data, log):
        ...
        if "group" in data and data["group"]:
            try:
                ...
                group = Group(group_name=data["group"], display_name=data["group"])
                session.add(group)
            deleted = False
            if "deleted" in data:
                deleted = smart_bool(data["deleted"])
            if deleted:
                if group in user.groups:
                    group.remove_member(
                        user, service=u"CSV", agent=identity.current.user
                    )
            else:
                if group not in user.groups:
                    group.add_member(user, service=u"CSV", agent=identity.current.user)
        else:
