...
class VirtManager(object):
    def __init__(self, user):
        ...
        self.is_create_floating_ip = bool(
            config.get("openstack.create_floating_ip", True)
        )
    ...
    def _do_sanity_check(self):
        ...
        if not has_novaclient:
            raise RuntimeError(
                "Openstack was configured, but python-novaclient " "is not installed"
            )
        if not has_neutronclient:
            raise RuntimeError(
                "Openstack was configured, but python-neutronclient " "is not installed"
            )
        if not has_novaclient:
            raise RuntimeError(
                "Openstack was configured, but python-novaclient " "is not installed"
        ...
        if not has_keystoneclient:
            raise RuntimeError(
                "Openstack was configured, but python-keystoneclient "
                "is not installed"
            )
        if not self.beaker_os_username and not self.beaker_os_password:
            raise RuntimeError(
                "Openstack was configured, but Beaker's OpenStack "
    ...
    def _create_keystone_session(self):
        log.debug(
            "_create_keystone_session user_domain_name: %s username: %s",
            self.beaker_os_user_domain_name,
            self.beaker_os_username,
        )
        ...
        with VirtNetwork(self.neutronclient, name) as net:
            ...
            try:
                ...
                try:
                    ...
                    log.exception(
                        "Failed to clean up %r during create_vm, leaked!", instance
                    )
    ...
    def get_vm_ipv4_address(self, instance, network_name):
        ...
        if not provider_addresses:
            raise RuntimeError(
                "Server %s does not have addresses in network %s"
                % (instance.name, network_name)
            )
        ipv4_found = next(
            (address for address in provider_addresses if address.get("version") == 4),
            None,
        )
        if not ipv4_found:
            raise RuntimeError("No IPv4 address record found for VM %s" % instance.name)
    ...
    def _create_vm_with_fixed_network(self, system_name, flavor, image_id):
        ...
        try:
            self._wait_for_build(instance)
            ipv4_address = self.get_vm_ipv4_address(instance, network.name)
    ...
    def _wait_for_build(self, instance):
        ...
        if instance.status != "ACTIVE":
            raise RuntimeError(
                "%r failed to build, status %s" % (instance, instance.status)
            )
    ...
    def _wait_for_stop(self, instance):
        ...
        if instance.status != "SHUTOFF":
            raise RuntimeError(
                "%r failed to stop, status %s" % (instance, instance.status)
            )
    ...
    def _wait_for_delete(self, instance):
        ...
        for attempt in range(5, 50, 5):
            ...
            try:
                instance.get()
                log.debug("%r still deleting, status %s", instance, instance.status)
            except:
                is_deleted = True
                break
        if not is_deleted:
            raise RuntimeError(
                "%r failed to delete, status %s" % (instance, instance.status)
            )
    ...
    def destroy_vm(self, vm):
        ...
        if self.is_create_floating_ip:
            self.neutronclient.remove_interface_router(
                str(vm.router_id), {"subnet_id": str(vm.subnet_id)}
            )
...
def create_keystone_trust(
    ...
    openstack_user_domain_name = config.get(
        "openstack.user_domain_name", trustor_user_domain_name
    )
    ...
    try:
        ...
        trustor = keystoneclient.v3.client.Client(
            session=trustor_session, interface="public"
        )
        ...
        if keystoneclient.__version__.startswith("0.11."):
            ...
            body = dict(
                trust=dict(
                    trustor_user_id=trustor_auth_ref.user_id,
            ...
            trust = trustor.trusts.create(
                trustor_user=trustor_auth_ref.user_id,
    ...
    def _get_external_network(self, neutronclient):
        ...
        if network_name:
            ...
            if not external_network.get("router:external"):
                raise RuntimeError(
                    "Network %s is not an external network" % network_name
                )
        else:
            external_network = next(
                (network for network in networks if network.get("router:external")),
                None,
            )
            if not external_network:
...
class VirtNetwork(object):
    ...
    def __repr__(self):
        return (
            "%s(name=%r, network_id=%r, router_id=%r, subnet_id=%r, interface_id=%r)"
            % (
                self.__class__.__name__,
                self.name,
                self.network_id,
    ...
    def _create_subnet(self, network_id):
        cidr = ConfigItem.by_name(u"guest_private_network").current_value(
            "192.168.10.0/24"
        )
        subnet_info = {
            "name": self.name,
    ...
    def _add_interface_to_router(self, router_id, subnet_id):
        interface = self.neutron.add_interface_router(
    ...
    def _configure_network(self):
        ...
        self.interface_id = self._add_interface_to_router(
            self.router_id, self.subnet_id
        )
