...
def remove_labcontroller(labcontroller):
    ...
    watchdogs = Watchdog.by_status(labcontroller=labcontroller, status="active")
    for w in watchdogs:
    ...
    # remove distro trees
    distro_tree_assocs = LabControllerDistroTree.query.filter(
        LabControllerDistroTree.lab_controller == labcontroller
    )
...
def get_labcontrollers():
    ...
    return render_tg_template(
        "bkr.server.templates.labcontrollers",
        {
...
def _create_labcontroller_helper(data):
    with convert_internal_errors():
        ...
        labcontroller.record_activity(
            ...
            field=u"Disabled",
            old=u"",
            new=unicode(labcontroller.disabled),
        )
...
def save_labcontroller():
    data = request.form
    return _create_labcontroller_helper(
        dict(
            user_name=data["lusername"],
...
class LabControllers(RPCRoot):
    ...
    def add_distro_tree(self, new_distro):
        ...
        try:
            ...
            raise BX(
                _(
                    "Cannot import distro as %s: it is configured as an alias for %s"
                    % (new_distro["osmajor"], osmajor.osmajor)
                )
            )

        osversion = OSVersion.lazy_create(
            osmajor=osmajor, osminor=new_distro["osminor"]
        )
        ...
        distro_tree = DistroTree.lazy_create(distro=distro, variant=variant, arch=arch)
        ...
        if "repos" in new_distro:
            for repo in new_distro["repos"]:
                dtr = DistroTreeRepo.lazy_create(
        ...
        if "images" in new_distro:
            for image in new_distro["images"]:
                ...
                try:
                    ...
                    continue  # ignore
                dti = DistroTreeImage.lazy_create(
                    distro_tree=distro_tree,
    ...
    def get_running_command_ids(self):
        lab_controller = identity.current.user.lab_controller
        running_commands = (
            Command.query.join(Command.system)
        ...
        for cmd in query:
            ...
            if cmd.action in (u"on", u"off", u"reboot", u"interrupt"):
                ...
                if distro_tree:
                    schemes = ["http", "ftp"]
                    if (
                        distro_tree.arch.arch == "s390"
                        or distro_tree.arch.arch == "s390x"
                    ):
                        ...
                        schemes = ["ftp"]
                    distro_tree_url = distro_tree.url_in_lab(
                        lab_controller, scheme=schemes
                    )
                else:
                ...
                d["netboot"] = {
                    "kernel_url": urlparse.urljoin(
                        distro_tree_url, installation.kernel_path
                    ),
                    "initrd_url": urlparse.urljoin(
                        distro_tree_url, installation.initrd_path
                    ),
    ...
    def get_installation_for_system(self, fqdn):
        ...
        if not kernel:
            raise ValueError(
                "Kernel image not found for distro tree %s" % distro_tree.id
            )
        initrd = distro_tree.image_by_type(by_initrd, system.kernel_type)
        if not initrd:
            raise ValueError(
                "Initrd image not found for distro tree %s" % distro_tree.id
            )
        return {
    ...
    def mark_command_running(self, command_id):
        ...
        if cmd.system.lab_controller != lab_controller:
            raise ValueError(
                "%s cannot update command for %s in wrong lab"
    ...
    def add_completed_command(self, fqdn, action):
        ...
        cmd = Command(
            user=user, service=u"XMLRPC", action=action, status=CommandStatus.completed
        )
    ...
    def mark_command_failed(self, command_id, message=None, system_broken=True):
        ...
        if (
            system_broken
            and cmd.action != "interrupt"
            and cmd.system.status == SystemStatus.automated
        ):
        ...
        if cmd.installation:
            ...
            queued_commands = [
                c for c in cmd.installation.commands if c.status == CommandStatus.queued
            ]
    ...
    def clear_running_commands(self, message=None):
        ...
        purged = (
            ...
            .where(Command.queue_time < datetime.utcnow() - timedelta(days=1))
            .values(status=CommandStatus.aborted)
            .execute()
        )
        if purged.rowcount:
            msg = (
                "Aborted %d stale commands before aborting "
    ...
    def get_distro_trees(self, filter=None):
        ...
        for dt in distro_trees:
            dt["available"] = [
                (lc, url) for lc, url in dt["available"] if lc == lab_controller.fqdn
            ]
        return distro_trees
