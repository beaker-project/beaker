...
def get_pools():
    ...
    query = (
        query.outerjoin(SystemPool.owning_user)
    ...
    return render_tg_template(
        "bkr.server.templates.backgrid",
        {
            ...
            "grid_add_view_type": "PoolCreateModal"
...
def pools_typeahead():
    if "q" in request.args:
        pools = SystemPool.query.filter(
            SystemPool.name.like("%%%s%%" % request.args["q"])
        )
    else:
        pools = SystemPool.query
    data = [
        {"name": pool.name, "tokens": _typeahead_split_pattern.split(pool.name.strip())}
...
def get_pool(pool_name):
    ...
    return render_tg_template(
        "bkr.server.templates.system_pool",
        {
            "title": pool.name,
            "system_pool": pool,
        },
...
def _get_owner(data):
    ...
    if user_name:
        ...
        if owner.removed:
            raise BadRequest400(
                "System pool cannot be owned by deleted user %s" % owner.user_name
            )
        owner_type = "user"
    if group_name:
        try:
...
def create_pool():
    ...
    with convert_internal_errors():
        ...
        pool.record_activity(
            user=u, service=u"HTTP", action=u"Created", field=u"Pool", new=unicode(pool)
        )
    ...
    if not pool in system.pools:
        if pool.can_edit(u) and system.can_edit(u):
            ...
            if not system.can_edit(u):
                raise Forbidden403(
                    "You do not have permission to " "modify system %s" % system.fqdn
                )
...
def add_access_policy_rule(pool_name):
    ...
    if rule.get("user", None):
        ...
        if user.removed:
            raise BadRequest400(
                "Cannot add deleted user %s to access policy" % user.user_name
            )
    else:
        user = None
    ...
    new_rule = policy.add_rule(
        user=user, group=group, everybody=rule["everybody"], permission=permission
    )
    pool.record_activity(
        user=identity.current.user,
        service=u"HTTP",
...
def delete_access_policy_rules(pool_name):
    ...
    if "user" in request.args:
        query = query.join(SystemAccessPolicyRule.user).filter(
            User.user_name.in_(request.args.getlist("user"))
        )
    elif "group" in request.args:
        query = query.join(SystemAccessPolicyRule.group).filter(
            Group.group_name.in_(request.args.getlist("group"))
        )
