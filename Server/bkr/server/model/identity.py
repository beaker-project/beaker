...
class GroupActivity(Activity):
    ...
    group_id = Column(
        Integer, ForeignKey("tg_group.group_id"), nullable=False, index=True
    )
...
class UserActivity(Activity):
    ...
    id = Column(Integer, ForeignKey("activity.id"), primary_key=True)
    object_id = Column(
        "user_id", Integer, ForeignKey("tg_user.user_id"), nullable=False
    )
    object = relationship(
        "User",
...
class SubmissionDelegate(DeclarativeMappedObject):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "delegate_id"),
        {"mysql_engine": "InnoDB"},
    )

    id = Column(Integer, nullable=False, primary_key=True)
    user_id = Column(
        Integer, ForeignKey("tg_user.user_id", name="tg_user_id_fk1"), nullable=False
    )
    delegate_id = Column(
        Integer, ForeignKey("tg_user.user_id", name="tg_user_id_fk2"), nullable=False
    )
...
class User(DeclarativeMappedObject, ActivityMixin):
    ...
    submission_delegates = relationship(
        "User",
        secondary=SubmissionDelegate.__table__,
    ...
    group_user_assocs = relationship(
        "UserGroup", back_populates="user", cascade="all, delete-orphan"
    )
    excluded_group_user_assocs = relationship(
        "ExcludedUserGroup", back_populates="user", cascade="all,delete-orphan"
    )
    sshpubkeys = relationship("SSHPubKey", back_populates="user")
    reservations = relationship(
        "Reservation", back_populates="user", order_by="Reservation.start_time.desc()"
    )
    system_access_policy_rules = relationship(
        "SystemAccessPolicyRule",
        back_populates="user",
        cascade="all, delete, delete-orphan",
    )
    notes = relationship("Note", back_populates="user")
    lab_controller = relationship("LabController", uselist=False, back_populates="user")
    jobs = relationship(
        "Job",
    ...
    def to_json(self):
        ...
        if identity.current.user:
            ...
            data["can_change_password"] = self.can_change_password(
    ...
    def add_submission_delegate(self, delegate, service=u"WEBUI"):
        if delegate.is_delegate_for(self):
            raise NoChangeException(
                "%s is already a" " submission delegate for %s" % (delegate, self)
            )
        else:
    ...
    def is_delegate_for(self, user):
        ...
        return (
            SubmissionDelegate.query.filter_by(
                delegate_id=self.user_id, user_id=user.user_id
            ).first()
            is not None
        )
    ...
    def by_user_name(cls, user_name):
        ...
        if not user and ldapenabled and autocreate and "/" not in user_name:
            ...
            objects = ldapcon.search_st(
                get("identity.soldapprovider.basedn", ""),
                ldap.SCOPE_SUBTREE,
                filter,
    ...
    def list_by_name(cls, username, find_anywhere=False, find_ldap_users=True):
        ...
        if ldapenabled and find_ldap_users is True:
            ...
            ldap_users = [
                (object[1]["uid"][0].decode("utf8"), object[1]["cn"][0].decode("utf8"))
    ...
    def can_change_password(self, user):
        ...
        if get("identity.ldap.enabled", False):
            filter = ldap.filter.filter_format(
                "(uid=%s)", [self.user_name.encode("utf8")]
            )
            ldapcon = ldap.initialize(get("identity.soldapprovider.uri"))
            objects = ldapcon.search_st(
                get("identity.soldapprovider.basedn", ""),
    ...
    def in_group(cls, group):  # pylint: disable=E0213
        if group.membership_type == GroupMembershipType.inverted:
            return not_(
                cls.excluded_group_user_assocs.any(ExcludedUserGroup.group == group)
            )
        else:
...
class Group(DeclarativeMappedObject, ActivityMixin):
    ...
    membership_type = Column(
        GroupMembershipType.db_type(),
        nullable=False,
        default=GroupMembershipType.normal,
        index=True,
    )
    created = Column(DateTime, default=datetime.utcnow)
    activity = relationship(
        GroupActivity, back_populates="object", cascade="all, delete-orphan"
    )
    permissions = relationship(
        "Permission", back_populates="groups", secondary=group_permission_table
    )
    user_group_assocs = relationship(
        "UserGroup", back_populates="group", cascade="all, delete-orphan"
    )
    excluded_user_group_assocs = relationship(
        "ExcludedUserGroup", back_populates="group", cascade="all, delete-orphan"
    )
    system_access_policy_rules = relationship(
        "SystemAccessPolicyRule",
    ...
    def by_name(cls, name, lockmode=False):
        if lockmode:
            return (
                cls.query.with_lockmode(lockmode).filter(cls.group_name == name).one()
            )
        else:
    ...
    def __repr__(self):
        return "Group(group_name=%r, display_name=%r)" % (
            self.group_name,
            self.display_name,
        )
    ...
    def to_json(self):
        ...
        data.update(
            {
                ...
                "permissions": [
                    permission.permission_name for permission in self.permissions
                ],
            }
        )
        if self.membership_type == GroupMembershipType.inverted:
            data["members"] = []
            data["excluded_users"] = [
                euga.user for euga in self.excluded_user_group_assocs
            ]
        else:
    ...
    def owners(self):
        return [uga.user for uga in self.user_group_assocs if uga.is_owner]

    def has_owner(self, user):
        return any(uga.user == user for uga in self.user_group_assocs if uga.is_owner)
    ...
    def set_name(self, user, service, group_name):
        ...
        if group_name != old_group_name:
            if self.is_protected_group():
                raise BX(
                    _(
                        u"Cannot rename protected group %r as %r"
    ...
    def readd_user(self, user, service=u"HTTP", agent=None):
        ...
        (assoc,) = [
            euga for euga in self.excluded_user_group_assocs if euga.user == user
        ]
    ...
    def validate_name(self, key, name):
        ...
        if name != name.strip():
            raise ValueError(
                "Group %s must not contain leading " "or trailing whitespace" % text
            )
        if len(name) > 255:
            raise ValueError(
                "Group %s must be not more than 255 characters long" % text
            )
    ...
    def has_member(self, user):
        return (
            self.membership_type != GroupMembershipType.inverted
            and any(user.id == assoc.user.id for assoc in self.user_group_assocs)
        ) or (
            self.membership_type == GroupMembershipType.inverted
            and not any(
                user.id == assoc.user.id for assoc in self.excluded_user_group_assocs
            )
        )
    ...
    def has_member(cls, user):  # pylint: disable=E0213
        return or_(
            and_(
                cls.membership_type != GroupMembershipType.inverted,
                cls.user_group_assocs.any(UserGroup.user == user),
            ),
            and_(
                cls.membership_type == GroupMembershipType.inverted,
                not_(
                    cls.excluded_user_group_assocs.any(ExcludedUserGroup.user == user)
                ),
            ),
        )
    ...
    @hybrid_property
    def dyn_users(self):
        return session.object_session(self).query(User).filter(User.in_group(self))
    ...
    def dyn_users(cls):  # pylint: disable=E0213
        ...
        class GroupDynamicUsers(object):
            def any(self, clause):
                return or_(
                    and_(
                        Group.membership_type != GroupMembershipType.inverted,
                        exists(
                            [1],
                            from_obj=UserGroup.__table__.join(User.__table__),
                            whereclause=and_(
                                UserGroup.group_id == Group.group_id, clause
                            ),
                        ),
                    ),
                    and_(
                        ...
                        not_(
                            exists(
                                [1],
                                from_obj=ExcludedUserGroup.__table__.join(
                                    User.__table__
                                ),
                                whereclause=and_(
                                    ExcludedUserGroup.group_id == Group.group_id, clause
                                ),
                            )
                        ),
                    ),
...
class Permission(DeclarativeMappedObject):
    ...
    groups = relationship(
        Group, back_populates="permissions", secondary=group_permission_table
    )
    ...
    def by_name(cls, permission_name, anywhere=False):
        if anywhere:
            return cls.query.filter(
                cls.permission_name.like("%%%s%%" % permission_name)
            ).all()
