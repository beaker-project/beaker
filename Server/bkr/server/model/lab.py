...
import logging
import urllib
from sqlalchemy import Column, ForeignKey, Integer, Unicode, Boolean, DateTime, event
...
class LabControllerActivity(Activity):
    ...
    lab_controller_id = Column(
        Integer, ForeignKey("lab_controller.id"), nullable=False, index=True
    )
...
class LabController(DeclarativeMappedObject, ActivityMixin):
    ...
    user_id = Column(
        Integer, ForeignKey("tg_user.user_id"), nullable=False, unique=True
    )
    ...
    activity = relationship(
        ...
        cascade="all, delete",
        order_by=[
            LabControllerActivity.created.desc(),
            LabControllerActivity.id.desc(),
        ],
    )
    _distro_trees = relationship(
        "LabControllerDistroTree",
        cascade="all, delete-orphan",
        back_populates="lab_controller",
    )
    ...
    if new_value is True:
        for system in labcontroller.systems:
            if system.scheduler_status == SystemSchedulerStatus.idle:
                log.debug(
                    "Idle system %s is in newly enabled lab %s, flagging it for scheduling",
                    system,
                    labcontroller,
                )
