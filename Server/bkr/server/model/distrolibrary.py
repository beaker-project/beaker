...
from sqlalchemy.orm import relationship, backref, dynamic_loader, synonym, validates
...
def default_install_options_for_distro(osmajor_name, osminor, variant, arch):
    ...
    if rhel in ("5", "6") or (fedora and fedora != "rawhide" and int(fedora) < 18):
    ...
    if rhel in ("5", "6") or (fedora and fedora != "rawhide" and int(fedora) < 23):
        del ks_meta["has_reqpart"]
    # systemd vs. SysV init
    ks_meta["has_systemd"] = True
    if rhel in ("5", "6") or (fedora and fedora != "rawhide" and int(fedora) < 15):
    ...
    if (fedora and fedora != "rawhide" and int(fedora) < 22) or (
        rhel and int(rhel) <= 6
    ):
    ...
    # clearpart --cdl
    if arch.arch == "s390x" and (
        rhel == "7" and int(osminor) >= 6 or rhel == "8" and int(osminor) >= 0
    ):
    ...
    if (fedora and (fedora == "rawhide" or int(fedora) > 30)) or (
        rhel and int(rhel) >= 9
    ):
    ...
    if arch.arch in ["ppc", "ppc64", "ppc64le"]:
        if (
            rhel
            and int(rhel) < 9
            or (fedora and fedora != "rawhide" and int(fedora) < 34)
        ):
    ...
    if rhel and int(rhel) < 9 or (fedora and fedora != "rawhide" and int(fedora) < 34):
...
def install_options_for_distro(osmajor_name, osminor, variant, arch):
    ...
    sources.append(
        default_install_options_for_distro(osmajor_name, osminor, variant, arch)
    )
    try:
...
distro_tag_map = Table(
    "distro_tag_map",
    DeclarativeMappedObject.metadata,
    Column("distro_id", Integer, ForeignKey("distro.id"), primary_key=True),
    Column("distro_tag_id", Integer, ForeignKey("distro_tag.id"), primary_key=True),
    mysql_engine="InnoDB",
)
...
class DistroTreeActivity(Activity):
    ...
    __table_args__ = {"mysql_engine": "InnoDB"}
    id = Column(Integer, ForeignKey("activity.id"), primary_key=True)
    distro_tree_id = Column(
        Integer, ForeignKey("distro_tree.id"), index=True, nullable=False
    )
...
class OSMajor(DeclarativeMappedObject):
    ...
    install_options_by_arch = relationship(
        "OSMajorInstallOptions",
        collection_class=attribute_mapped_collection("arch"),
        back_populates="osmajor",
        cascade="all, delete-orphan",
    )
    osversions = relationship(
        "OSVersion", order_by="OSVersion.osminor", back_populates="osmajor"
    )
    ...
    def by_name_alias(cls, name_alias):
        return cls.query.filter(
            or_(OSMajor.osmajor == name_alias, OSMajor.alias == name_alias)
        ).one()
    ...
    def in_any_lab(cls, query=None):
        ...
        return query.filter(
            exists(
                [1],
                from_obj=LabControllerDistroTree.__table__.join(DistroTree.__table__)
    ...
    def used_by_any_recipe(cls, query=None):
        ...
        return query.filter(
            exists(
                [1],
                from_obj=Recipe.__table__.join(DistroTree.__table__)
                .join(Distro.__table__)
                .join(OSVersion.__table__),
            )
            .where(OSVersion.osmajor_id == OSMajor.id)
...
class OSVersion(DeclarativeMappedObject):
    ...
    def lazy_create(cls, osmajor, osminor):
        return super(OSVersion, cls).lazy_create(osmajor_id=osmajor.id, osminor=osminor)
...
class Distro(DeclarativeMappedObject, ActivityMixin):
    ...
    _tags = relationship(
        "DistroTag", secondary=distro_tag_map, back_populates="distros"
    )
...
class DistroTree(DeclarativeMappedObject, ActivityMixin):
    ...
    date_created = Column(
        "date_created", DateTime, nullable=False, default=datetime.utcnow
    )
    distro = relationship(Distro, back_populates="trees")
    arch = relationship(Arch, back_populates="distro_trees")
    lab_controller_assocs = relationship(
        LabControllerDistroTree,
        back_populates="distro_tree",
        cascade="all, delete-orphan",
    )
    activity = relationship(
        DistroTreeActivity,
    ...
    images = relationship(
        "DistroTreeImage", back_populates="distro_tree", cascade="all, delete-orphan"
    )
    recipes = relationship(
        "Recipe", back_populates="distro_tree", cascade_backrefs=False
    )
    ...
    def lazy_create(cls, distro, variant, arch):
        return super(DistroTree, cls).lazy_create(
            distro_id=distro.id, variant=variant, arch_id=arch.id
        )
    ...
    def url_in_lab(self, lab_controller, scheme=None, required=False):
        ...
        if isinstance(scheme, basestring):
            scheme = [scheme]
        urls = dict(
            (urlparse.urlparse(lca.url).scheme, lca.url)
        ...
        if required:
            raise ValueError(
                "No usable URL found for %r in %r" % (self, lab_controller)
            )
        else:
            return None
    ...
    def image_by_type(self, image_type, kernel_type):
        for image in self.images:
            if image.image_type == image_type and image.kernel_type == kernel_type:
                return image

    def install_options(self):
        return InstallOptions.from_strings(
            self.ks_meta, self.kernel_options, self.kernel_options_post
        )
