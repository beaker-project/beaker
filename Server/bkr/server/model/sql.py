...
class ConditionalInsert(Insert):
    def __init__(self, table, unique_values, extra_values=()):
        ...
        self.unique_condition = and_(
            *[col == value for col, value in unique_values.iteritems()]
        )
...
def visit_conditional_insert(element, compiler, **kwargs):
    ...
    text += " (%s)\n" % ", ".join(
        compiler.preparer.format_column(c[0]) for c in colparams
    )
    ...
    nonexistence_clause = not_(
        exists(
            Select(
                columns=[sqltext("1")],
                from_obj=[element.table],
                whereclause=element.unique_condition,
                for_update=True,
            )
        )
    )
