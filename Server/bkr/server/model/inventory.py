...
class SystemActivity(Activity):
    ...
    system_id = Column(Integer, ForeignKey("system.id"), nullable=False, index=True)
...
class Command(DeclarativeMappedObject):
    ...
    installation_id = Column(
        Integer, ForeignKey("installation.id", name="command_queue_installation_id_fk")
    )
    ...
    def get_queue_stats(cls, query):
        ...
        query = (
            query.filter(cls.status.in_(active_statuses))
            .group_by(cls.status)
    ...
    def change_status(self, new_status):
        ...
        if (
            ...
            .execute(
                Command.__table__.update(
                    and_(
                        Command.__table__.c.id == self.id,
            ...
            raise StaleCommandStatusException(
                "Status for command %s updated in another transaction" % self.id
            )
        ...
        if self.status.finished:
            ...
            if self.system.lab_controller:
                categories.append(
                    "by_lab.%s" % self.system.lab_controller.fqdn.replace(".", "_")
                )
            if self.system.power and self.system.power.power_type:
                categories.append(
                    "by_power_type.%s" % self.system.power.power_type.name
                )
            categories.extend("by_arch.%s" % arch.arch for arch in self.system.arch)
            for category in categories:
                metrics.increment(
                    "counters.system_commands_%s.%s" % (self.status.name, category)
                )

    def log_to_system_history(self):
        self.system.record_activity(
            user=self.user,
            service=self.service,
            action=self.action,
            field=u"Power",
            old=u"",
            new=self.error_message
            and u"%s: %s" % (self.status, self.error_message)
    ...
    def abort(self, msg=None):
        ...
        if self.installation and self.installation.recipe:
            self.installation.recipe.abort("Command %s aborted: %s" % (self.id, msg))
...
class Reservation(DeclarativeMappedObject):
    ...
    start_time = Column(DateTime, index=True, nullable=False, default=datetime.utcnow)
    finish_time = Column(DateTime, index=True)
...
system_device_map = Table(
    ...
    Column("device_id", Integer, ForeignKey("device.id"), primary_key=True, index=True),
    mysql_engine="InnoDB",
)

system_arch_map = Table(
    "system_arch_map",
    DeclarativeMappedObject.metadata,
    Column(
        "system_id",
    ...
    Column("arch_id", Integer, ForeignKey("arch.id"), primary_key=True, index=True),
    mysql_engine="InnoDB",
)
...
class System(DeclarativeMappedObject, ActivityMixin):
    ...
    release_action = Column(
        ReleaseAction.db_type(), default=ReleaseAction.power_off, nullable=False
    )
    ...
    kernel_type_id = Column(
        Integer,
        ForeignKey("kernel_type.id"),
        default=select([KernelType.id], limit=1)
        .where(KernelType.kernel_type == u"default")
        .correlate(None),
        nullable=False,
    )
    kernel_type = relationship("KernelType")
    devices = relationship(
        "Device", secondary=system_device_map, back_populates="systems"
    )
    disks = relationship(
        "Disk", back_populates="system", cascade="all, delete, delete-orphan"
    )
    arch = relationship(
        Arch, order_by=[Arch.arch], secondary=system_arch_map, back_populates="systems"
    )
    labinfo = relationship(
        "LabInfo",
        uselist=False,
    ...
    excluded_osmajor = relationship(
        "ExcludeOSMajor", back_populates="system", cascade="all, delete, delete-orphan"
    )
    excluded_osversion = relationship(
        "ExcludeOSVersion",
        back_populates="system",
        cascade="all, delete, delete-orphan",
    )
    ...
    pools = relationship(
        "SystemPool", secondary="system_pool_map", back_populates="systems"
    )
    key_values_int = relationship(
        "Key_Value_Int", back_populates="system", cascade="all, delete, delete-orphan"
    )
    key_values_string = relationship(
        "Key_Value_String",
        back_populates="system",
        cascade="all, delete, delete-orphan",
    )
    ...
    dyn_activity = dynamic_loader(
        SystemActivity, order_by=[SystemActivity.__table__.c.id.desc()]
    )
    command_queue = relationship(
        Command,
        back_populates="system",
        cascade="all, delete, delete-orphan",
        order_by=[Command.queue_time.desc(), Command.id.desc()],
    )
    dyn_command_queue = dynamic_loader(
        Command, order_by=[Command.queue_time.desc(), Command.id.desc()]
    )
    _system_ccs = relationship(
        "SystemCc", back_populates="system", cascade="all, delete, delete-orphan"
    )
    reservations = relationship(
        Reservation, back_populates="system", order_by=[Reservation.start_time.desc()]
    )
    dyn_reservations = dynamic_loader(Reservation)
    open_reservation = relationship(
        Reservation,
        uselist=False,
        viewonly=True,
    ...
    status_durations = relationship(
        SystemStatusDuration,
        back_populates="system",
        cascade="all, delete, delete-orphan",
        order_by=[
            SystemStatusDuration.start_time.desc(),
    ...
    queued_recipes = relationship(
        "Recipe", back_populates="systems", secondary="system_recipe_map"
    )
    dyn_queued_recipes = dynamic_loader("Recipe", secondary="system_recipe_map")

    custom_access_policy_id = Column(
        Integer,
        ForeignKey("system_access_policy.id", name="custom_access_policy_id_fk"),
    )
    custom_access_policy = relationship(
        "SystemAccessPolicy",
        uselist=False,
        back_populates="system",
        cascade="all, delete",
    ...
    active_access_policy_id = Column(
        Integer,
        ForeignKey("system_access_policy.id", name="active_access_policy_id_fk"),
    )
    active_access_policy = relationship(
        "SystemAccessPolicy", uselist=False, foreign_keys=[active_access_policy_id]
    )
    activity_type = SystemActivity
    hardware_scan_recipes = relationship(
        "Recipe", secondary="system_hardware_scan_recipe_map", lazy="dynamic"
    )
    ...
    def validate_status_reason(self, key, value):
        ...
        if not self.status.bad:
            raise ValueError("Cannot set status reason when status is %s" % self.status)
        max_length = object_mapper(self).columns[key].type.length
        if len(value) > max_length:
            raise ValueError(
                "System condition report is longer than " "%s characters" % max_length
            )
        return value
    ...
    def validate_status(self, key, value):
        if (
            key == "status"
            and value == SystemStatus.automated
            and not self.lab_controller
        ):
            raise ValueError(
                "System status cannot be Automated without an associated Lab Controller"
            )
        elif (
            key == "lab_controller"
            and value is None
            and self.status == SystemStatus.automated
            or key == "lab_controller_id"
            and value is None
            and self.status == SystemStatus.automated
        ):
            raise ValueError(
                "System cannot be removed from its Lab Controller when status is Automated"
            )
        return value
    ...
    def find_current_hardware_scan_recipe(self):
        ...
        current_scan = self.hardware_scan_recipes.filter(
            Recipe.status.in_(in_progress_states)
        ).first()
        return current_scan
    ...
    def __json__(self):
        ...
        if self.lab_controller:
            ...
            data["possible_osmajors"] = [
                osmajor.osmajor
                for osmajor in OSMajor.ordered_by_osmajor(
                    OSMajor.in_lab(self.lab_controller)
                )
            ]
        ...
        if self.status == SystemStatus.automated:
            data["queue_size"] = (
                Recipe.query.filter(Recipe.status == TaskStatus.queued)
        ...
        data["previous_reservation"] = (
            self.dyn_reservations.filter(Reservation.finish_time != None)
            .order_by(Reservation.finish_time.desc())
            .first()
        )
        ...
        if identity.current.user:
            ...
            for pool in self.pools:
                data["can_remove_from_pool"][pool.name] = pool.can_edit(
                    u
                ) or self.can_edit(u)
            ...
            data["can_return"] = (
                self.open_reservation is not None
            ...
            data["can_return_loan"] = self.loaned is not None and self.can_return_loan(
                u
            )
    ...
    def all(cls, user):
        ...
        return (
            cls.query.outerjoin(System.lab_controller)
    ...
    def visible_to_user(self, user):
        ...
        return (
            (
                self.custom_access_policy
                and self.active_access_policy.grants(user, SystemPermission.view)
            )
            or self.owner == user
            or self.loaned == user
            or self.user == user
    ...
    def visible_to_user(cls, user):  # pylint: disable=E0213
        if user.is_admin() or user.has_permission(u"secret_visible"):
            return true()
        return or_(
            SystemAccessPolicy.grants(user, SystemPermission.view),
            cls.owner == user,
            cls.loaned == user,
            cls.user == user,
        )
    ...
    def diskspace(cls):  # pylint: disable=E0213
        return (
            select([func.sum(Disk.size)])
            .where(Disk.system_id == cls.id)
            .label("diskspace")
        )
    ...
    def visible_to_anonymous(self):
        return self.active_access_policy and self.active_access_policy.grants_everybody(
    ...
    def compatible_with_distro_tree(self, arch, osmajor, osminor=None):
        ...
        if arch not in self.arch:
            return False
        if any(
            e.osmajor.osmajor == osmajor and e.arch == arch
        ...
        if osminor is None:
            ...
            if any(
                e.osversion.osmajor.osmajor == osmajor and e.arch == arch
                for e in self.excluded_osversion
            ):
                return False
        else:
            if any(
                e.osversion.osmajor.osmajor == osmajor
    ...
    def compatible_with_distro_tree(
        cls, arch, osmajor, osminor
    ):  # pylint: disable=E0213
        clauses = []
        clauses.append(cls.arch.contains(arch))
        clauses.append(
            not_(
                cls.excluded_osmajor.any(
                    and_(
    ...
    def in_lab_with_distro_tree(self, distro_tree):
        return (
            ...
            and distro_tree.url_in_lab(self.lab_controller) is not None
        )

    @in_lab_with_distro_tree.expression
    def in_lab_with_distro_tree(self, distro_tree):
        # we assume System.lab_controller was joined, System.all() does that
        return LabController._distro_trees.any(
            LabControllerDistroTree.distro_tree == distro_tree
        )
    ...
    def mine(cls, user):
        ...
        user can see
        """
        return cls.query.filter(or_(System.user == user, System.loaned == user))
    ...
    def has_manual_reservation(self, user):
        ...
        return (
            reservation and reservation.type == u"manual" and user and self.user == user
        )
    ...
    def excluded_families(self):
        ...
        for arch in self.arch:
            major[arch.arch] = [
                osmajor.osmajor.id for osmajor in self.excluded_osmajor_byarch(arch)
            ]
            version[arch.arch] = [
                osversion.osversion.id
                for osversion in self.excluded_osversion_byarch(arch)
            ]
    ...
    def install_options(self, arch, osmajor, osminor):
        ...
        if arch in self.provisions:
            ...
            yield InstallOptions.from_strings(
                pa.ks_meta, pa.kernel_options, pa.kernel_options_post
            )
            if osmajor in pa.provision_families:
                pf = pa.provision_families[osmajor]
                yield InstallOptions.from_strings(
                    pf.ks_meta, pf.kernel_options, pf.kernel_options_post
                )
                if osversion in pf.provision_family_updates:
                    pfu = pf.provision_family_updates[osversion]
                    yield InstallOptions.from_strings(
                        pfu.ks_meta, pfu.kernel_options, pfu.kernel_options_post
                    )
    ...
    def has_efi(self):
        ...
        return any(
            kv.key.key_name == u"NETBOOT_METHOD"
            and (kv.key_value == u"efigrub" or kv.key_value == u"grub2")
    ...
    def is_free(self, user):
        ...
        return (
            self.user is None
            and (self.loaned is None or self.loaned == user)
            and (self.lab_controller is None or not self.lab_controller.disabled)
        )
    ...
    def _ensure_user_is_authenticated(user):
        if user is None:
            raise RuntimeError(
                "Cannot check permissions for an " "unauthenticated user."
            )
    ...
    def can_edit_policy(self, user):
        ...
        if user.is_admin():
            return True
        if self.active_access_policy and self.active_access_policy.grants(
            user, SystemPermission.edit_policy
        ):
            return True
        return False
    ...
    def can_edit(cls, user):  # pylint: disable=E0213
        ...
        if user.is_admin():
            return true()
        return or_(
            SystemAccessPolicy.grants(user, SystemPermission.edit_system),
            cls.owner == user,
        )
    ...
    def can_view_power(self, user):
        ...
        if user.is_admin():
            return True
        if (
            self.active_access_policy
            and self.active_access_policy.grants(user, SystemPermission.edit_system)
            or self.active_access_policy.grants(user, SystemPermission.view_power)
        ):
            return True
        return False
    ...
    def can_lend(self, user):
        ...
        if self.active_access_policy and self.active_access_policy.grants(
            user, SystemPermission.loan_any
        ):
            return True
        return False
    ...
    def can_borrow(self, user):
        ...
        if (
            (not self.loaned or self.loaned == user)
            and self.active_access_policy
            and self.active_access_policy.grants(user, SystemPermission.loan_self)
        ):
            return True
        return False
    ...
    def can_reserve_manually(self, user):
        ...
        if self.status == SystemStatus.manual or (self.loaned and self.loaned == user):
    ...
    def can_power(self, user):
        ...
        if self.active_access_policy and self.active_access_policy.grants(
            user, SystemPermission.control_system
        ):
            return True
        return False
    ...
    ALLOWED_ATTRS = [
        ...
        "model",
        "memory",
    ]  #: attributes which the inventory scripts may set
    PRESERVED_ATTRS = [
        "vendor",
        "model",
    ]  #: attributes which should only be set when empty

    def get_update_method(self, obj_str):
        methods = dict(
            Cpu=self.updateCpu,
            Arch=self.updateArch,
            Devices=self.updateDevices,
            Numa=self.updateNuma,
        ...
        for key in inventory:
            if key in self.ALLOWED_ATTRS:
                ...
                try:
                    ...
                    log.warning(
                        "Attempted to update unknown inventory property '%s' on %s"
                        % (key, self.fqdn)
                    )
                else:
        ...
        for olddisk in list(self.disks):
            ...
            if not exists:
                ...
                self.record_activity(
                    ...
                    action=u"Removed",
                    field=u"Disk:size",
                    old=unicode(olddisk.size),
                )
                self.record_activity(
                    user=identity.current.user,
                    service=u"XMLRPC",
                    action=u"Removed",
                    field=u"Disk:sector_size",
                    old=unicode(olddisk.sector_size),
                )
                self.record_activity(
                    user=identity.current.user,
                    service=u"XMLRPC",
                    ...
                    old=unicode(olddisk.phys_sector_size),
                )
                self.record_activity(
                    user=identity.current.user,
                    service=u"XMLRPC",
        ...
        for newdisk in diskinfo["Disks"]:
            ...
            if not exists:
                ...
                self.record_activity(
                    ...
                    action=u"Added",
                    field=u"Disk:sector_size",
                    new=unicode(newdisk["sector_size"]),
                )
                self.record_activity(
                    user=identity.current.user,
                    service=u"XMLRPC",
    ...
    def updateCpu(self, cpuinfo):
        ...
        self.record_activity(
            ...
            old=None,
            new=None,
        )  # XXX find a good way to record the actual changes
    ...
    def excluded_osmajor_byarch(self, arch):
        ...
        """
        excluded = (
            ExcludeOSMajor.query.join("system")
            .join("arch")
            .filter(and_(System.id == self.id, Arch.id == arch.id))
        )
        return excluded
    ...
    def excluded_osversion_byarch(self, arch):
        ...
        """
        excluded = (
            ExcludeOSVersion.query.join("system")
            .join("arch")
            .filter(and_(System.id == self.id, Arch.id == arch.id))
        )
        return excluded
    ...
    def distros(self, query=None):
        ...
        query = query.join(
            (trees_subquery, trees_subquery.c.inner_distro_id == Distro.id)
        )
        return query
    ...
    def distro_trees(self, only_in_lab=True, query=None, osmajor_name=None):
        ...
        query = query.join(
            ...
            contains_eager(DistroTree.distro, Distro.osversion, OSVersion.osmajor)
        )
        if osmajor_name:
        ...
        query = (
            query.filter(DistroTree.arch_id.in_([a.id for a in self.arch]))
            .filter(
                not_(
                    OSMajor.excluded_osmajors.any(
                        and_(
            ...
            .filter(
                not_(
                    OSVersion.excluded_osversions.any(
                        and_(
    ...
    def distro_tree_for_inventory(self):
        ...
        for osmajor in inventory_osmajors:
            supported_distro_trees = self.distro_trees(
                osmajor_name=osmajor.decode("utf8")
            )
        ...
        if supported_distro_trees.count():
            supported_distro_tree_archs = [
                dt.arch.arch for dt in supported_distro_trees.all()
            ]
            if (
                u"x86_64" in supported_distro_tree_archs
                and len(supported_distro_tree_archs) != 1
            ):
                supported_distro_trees = supported_distro_trees.filter(
                    DistroTree.arch == Arch.by_name(u"x86_64")
                )
            if (
                u"ppc64" in supported_distro_tree_archs
                and len(supported_distro_tree_archs) != 1
            ):
                supported_distro_trees = supported_distro_trees.filter(
                    DistroTree.arch == Arch.by_name(u"ppc64")
                )
            if (
                u"s390x" in supported_distro_tree_archs
                and len(supported_distro_tree_archs) != 1
            ):
                supported_distro_trees = supported_distro_trees.filter(
                    DistroTree.arch == Arch.by_name(u"s390x")
                )
            ...
            if released.count():
                ...
                distro_tree = supported_distro_trees.order_by(
                    Distro.date_created.desc()
                ).first()
            return distro_tree
        else:
            return None
        ...
        if self.release_action == ReleaseAction.power_off:
            ...
            if self.reprovision_distro_tree:
                ...
                try:
                    ...
                    installation = (
                        self.reprovision_distro_tree.create_installation_from_tree()
                    )
                    ...
                    if ks_keyword not in install_options.kernel_options:
                        ...
                        install_options.kernel_options[
                            ks_keyword
                        ] = rendered_kickstart.link
                    else:
                        rendered_kickstart = None
                    by_kernel = (
                        ImageType.uimage
                        if self.kernel_type and self.kernel_type.uboot
                    ...
                    kernel_type = (
                        self.kernel_type
                        if self.kernel_type
                        else KernelType.by_name(u"default")
                    )
                    installation.kernel_path = (
                        self.reprovision_distro_tree.image_by_type(
                            by_kernel, kernel_type
                        ).path
                    )
                    installation.initrd_path = (
                        self.reprovision_distro_tree.image_by_type(
                            by_initrd, kernel_type
                        ).path
                    )
                    ...
                    self.action_power(
                        action=u"reboot", installation=installation, service=service
                    )
                except Exception:
                    log.exception(
                        "Failed to re-provision %s on %s, ignoring",
                        self.reprovision_distro_tree,
                        self,
                    )
        else:
    ...
    def configure_netboot(self, installation, service=u"Scheduler"):
        if not self.lab_controller:
            return
        self.enqueue_command(u"clear_logs", service=service, installation=installation)
        return self.enqueue_command(
            u"configure_netboot", service=service, installation=installation
        )

    def action_power(
        self, action=u"reboot", installation=None, service=u"Scheduler", delay=0
    ):
    ...
    def clear_netboot(self, installation=None, service=u"Scheduler"):
        if not self.lab_controller:
            return
        return self.enqueue_command(
            u"clear_netboot", service=service, installation=installation
        )

    def enqueue_command(
        self, action, service, installation=None, quiescent_period=None, delay=None
    ):
        try:
            user = identity.current.user
        except Exception:
    ...
    def link(self):
        ...
        return make_link(url="/view/%s" % self.fqdn, text=self.fqdn)
    ...
    def suspicious_abort(self):
        ...
        session.flush()
        status_change_subquery = (
            session.query(func.max(SystemActivity.created))
            .filter(
                and_(
        ...
        nonsuspicious_aborted_recipe_subquery = (
            self.dyn_recipes.filter(not_(Recipe.is_suspiciously_aborted))
            .with_entities(func.max(Recipe.finish_time))
            .subquery()
        )
        count = (
            self.dyn_recipes.join(Recipe.distro_tree, DistroTree.distro)
            .filter(
                and_(
                    ...
                    Recipe.finish_time
                    > nonsuspicious_aborted_recipe_subquery.as_scalar().correlate(None),
                )
            )
        ...
        if count >= 2:
            ...
            reason = unicode(
                _(u"System has a run of aborted recipes " "with reliable distros")
            )
            log.warn(reason)
    ...
    def reserve_manually(self, service, user=None):
        ...
        if not self.can_reserve_manually(user):
            raise BX(
                _(
                    u"Cannot manually reserve automated system, "
    ...
    def _check_can_reserve(self, user):
        ...
        if self.user is not None and self.user == user:
            raise StaleSystemUserException(
                _(u"User %s has already reserved " "system %s") % (user, self)
            )
        if not self.can_reserve(user):
            raise InsufficientSystemPermissions(
                _(u"User %s cannot " "reserve system %s") % (user, self)
            )
        if self.loaned:
    ...
    def _reserve(self, service, user, reservation_type):
        ...
        if (
            ...
            .execute(
                System.__table__.update(
                    and_(System.id == self.id, System.user_id == None)
                ),
            ...
            raise StaleSystemUserException(
                _(u"System %r is already " "reserved") % self
            )
        ...
        log.debug(
            "Created reservation for system %r with type %r, service %r, user %r",
            self,
            reservation_type,
            service,
            user,
        ...
        if (
            ...
            .execute(
                Reservation.__table__.update(
                    and_(
                        Reservation.id == my_reservation_id,
    ...
    def add_note(self, text, user, service=u"WEBUI"):
        ...
        self.record_activity(
            user=user, service=service, action="Added", field="Note", old="", new=text
        )
        self.date_modified = datetime.utcnow()
        return note
...
def add_system_status_duration(system, child, oldchild, initiator):
    ...
    now = datetime.utcnow()
    if (
        session.query(SystemStatusDuration)
...
@event.listens_for(System.lab_controller, "set")
def mark_system_pending_when_lab_controller_changes(
    system, new_value, old_value, initiator
):
    if new_value is not None and system.scheduler_status == SystemSchedulerStatus.idle:
        log.debug(
            "Idle system %s changed lab controller, flagging it for scheduling", system
        )
...
def mark_system_pending_when_automated(system, new_value, old_value, initiator):
    if (
        new_value == SystemStatus.automated
        and system.scheduler_status == SystemSchedulerStatus.idle
    ):
        log.debug(
            "Idle system %s changed to Automated, flagging it for scheduling", system
        )
        system.scheduler_status = SystemSchedulerStatus.pending
...
def mark_system_pending_when_lent(system, new_value, old_value, initiator):
    if new_value != old_value and system.scheduler_status == SystemSchedulerStatus.idle:
        log.debug(
            "Idle system %s changed loan status, flagging it for scheduling", system
        )
        system.scheduler_status = SystemSchedulerStatus.pending


@event.listens_for(System.user, "set")
def mark_system_pending_when_manual_reservation_is_returned(
    system, new_value, old_value, initiator
):
    if new_value is None and system.scheduler_status == SystemSchedulerStatus.idle:
        log.debug(
            "Idle system %s reservation was returned, flagging it for scheduling",
            system,
        )
        system.scheduler_status = SystemSchedulerStatus.pending
...
def mark_systems_pending_when_distro_tree_added_to_lab(
    distro_tree, lab_controller_assoc, initiator
):
    ...
    log.debug(
        "Flagging idle systems in %s for scheduling due to distro import",
        lab_controller,
    )
    # delayed import to avoid circular dependency
    ...
    System.query.filter(System.lab_controller == lab_controller).filter(
        ...
        or_(
            ...
            exists(
                [1],
                from_obj=system_recipe_map.join(Recipe.__table__)
                .join(
                    machine_guest_map,
                    Recipe.id == machine_guest_map.c.machine_recipe_id,
                )
                .join(
                    guestrecipe, guestrecipe.c.id == machine_guest_map.c.guest_recipe_id
                ),
            )
        ...
        dict(scheduler_status=SystemSchedulerStatus.pending), synchronize_session=False
    )


system_pool_map = Table(
    "system_pool_map",
    DeclarativeMappedObject.metadata,
    Column(
        "system_id",
...
class SystemPool(DeclarativeMappedObject, ActivityMixin):
    ...
    systems = relationship(
        "System", secondary="system_pool_map", back_populates="pools"
    )
    activity = relationship(
        SystemPoolActivity,
        back_populates="object",
    ...
    access_policy_id = Column(
        Integer,
        ForeignKey("system_access_policy.id", name="system_pool_access_policy_id_fk"),
        nullable=False,
    )
    ...
    def __json__(self):
        ...
        if identity.current.user:
            ...
            data["systems"] = [
                system.fqdn if system.visible_to_user(user) else "<restricted>"
            ...
            data["can_edit_policy"] = False
            data["systems"] = [
                system.fqdn if system.visible_to_anonymous else "<restricted>"
                for system in self.systems
            ]
        return data
    ...
    def validate_name(self, key, name):
        ...
        if name != name.strip():
            raise ValueError(
                "Pool name must not cannot contain leading " "or trailing whitespace"
            )
        return name
    ...
    def can_edit_policy(self, user):
        return self.can_edit(user) or self.access_policy.grants(
            user, SystemPermission.edit_policy
        )
...
class SystemCc(DeclarativeMappedObject):
    ...
    system_id = Column(
        Integer,
        ForeignKey("system.id", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
    )
...
class SystemAccessPolicy(DeclarativeMappedObject):
    ...
    def __json__(self):
        return {
            ...
            "possible_permissions": [
                {"value": unicode(permission), "label": unicode(permission.label)}
    ...
    def grants(self, user, permission):
        ...
        return any(
            ...
            and (rule.user == user or rule.group in user.groups or rule.everybody)
    ...
    def grants(cls, user, permission):  # pylint: disable=E0213
        ...
        clauses = [
            SystemAccessPolicyRule.user == user,
            SystemAccessPolicyRule.everybody,
        ]
        if user.groups:
            clauses.append(
                SystemAccessPolicyRule.group_id.in_([g.group_id for g in user.groups])
            )
        return cls.rules.any(
            and_(SystemAccessPolicyRule.permission == permission, or_(*clauses))
        )
    ...
    def grants_everybody(self, permission):
        ...
        """
        return any(
            rule.permission == permission and rule.everybody for rule in self.rules
        )
    ...
    def grants_everybody(cls, permission):  # pylint: disable=E0213
        return cls.rules.any(
            and_(
                SystemAccessPolicyRule.permission == permission,
                SystemAccessPolicyRule.everybody,
            )
        )
    ...
    def add_rule(self, permission, user=None, group=None, everybody=False):
        ...
        self.rules.append(
            SystemAccessPolicyRule.lazy_create(
                policy_id=self.id,
...
class SystemAccessPolicyRule(DeclarativeMappedObject):
    ...
    policy_id = Column(
        Integer,
        ForeignKey(
            "system_access_policy.id", name="system_access_policy_rule_policy_id_fk"
        ),
        nullable=False,
    )
    ...
    user_id = Column(
        Integer,
        ForeignKey("tg_user.user_id", name="system_access_policy_rule_user_id_fk"),
    )
    user = relationship(User, back_populates="system_access_policy_rules")
    group_id = Column(
        Integer,
        ForeignKey("tg_group.group_id", name="system_access_policy_rule_group_id_fk"),
    )
...
class Provision(DeclarativeMappedObject):
    ...
    provision_families = relationship(
        "ProvisionFamily",
...
class ProvisionFamily(DeclarativeMappedObject):
    ...
    provision_id = Column(
        Integer,
        ForeignKey("provision.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    kernel_options = Column(String(1024))
    kernel_options_post = Column(String(1024))
    provision_family_updates = relationship(
...
class ProvisionFamilyUpdate(DeclarativeMappedObject):
    ...
    __table_args__ = {"mysql_engine": "InnoDB"}
    id = Column(Integer, autoincrement=True, primary_key=True)
    provision_family_id = Column(
        Integer,
        ForeignKey("provision_family.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    )
...
class CpuFlag(DeclarativeMappedObject):
    ...
    cpu_id = Column(
        Integer,
        ForeignKey("cpu.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    )
...
class DeviceClass(DeclarativeMappedObject):
    ...
    def lazy_create(cls, device_class=None, **kwargs):
        ...
        if not device_class:
            device_class = "NONE"
        return super(DeviceClass, cls).lazy_create(device_class=device_class, **kwargs)
...
class Device(DeclarativeMappedObject):
    ...
    date_added = Column(DateTime, default=datetime.utcnow, nullable=False)
    systems = relationship(
        System, secondary=system_device_map, back_populates="devices"
    )
...
class Disk(DeclarativeMappedObject):
    ...
    def matches(self, other):
        ...
        return (
            ...
            and self.sector_size == other["sector_size"]
            and self.phys_sector_size == other["phys_sector_size"]
...
class Power(DeclarativeMappedObject):
    ...
    power_quiescent_period = Column(
        Integer, default=default_quiescent_period, nullable=False
    )
    ...
    def __json__(self):
        return {
            ...
            "possible_power_types": [
                t.name for t in PowerType.query.order_by(PowerType.name)
            ],
        }
    ...
    def validate_quiescent_period(self, key, power_quiescent_period):
        if not power_quiescent_period >= 0:
            raise ValueError(
                "Quiescent period for power control must be greater"
...
class Key(DeclarativeMappedObject):
    ...
    key_value_string = relationship(
        "Key_Value_String", back_populates="key", cascade="all, delete-orphan"
    )
    key_value_int = relationship(
        "Key_Value_Int", back_populates="key", cascade="all, delete-orphan"
    )
    ...
    def get_all_keys(cls):
        ...
        return [
            key.key_name for key in all_keys if key.key_name not in cls.obsoleted_keys
        ]
...
class Key_Value_String(DeclarativeMappedObject):
    ...
    key_id = Column(
        Integer,
        ForeignKey("key_.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    def by_key_value(cls, system, key, value):
        return cls.query.filter(
            and_(
                ...
                Key_Value_String.system == system,
            )
        ).one()
