...
class Installation(DeclarativeMappedObject):
    ...
    distro_tree_id = Column(
        Integer,
        ForeignKey("distro_tree.id", name="installation_distro_tree_id_fk"),
        nullable=True,
    )
    distro_tree = relationship(DistroTree)
    kernel_options = Column(UnicodeText)
    rendered_kickstart_id = Column(
        Integer,
        ForeignKey(
            "rendered_kickstart.id",
            name="installation_rendered_kickstart_id_fk",
            ondelete="SET NULL",
        ),
    )
    ...
    system_id = Column(
        Integer, ForeignKey("system.id", name="installation_system_id_fk")
    )
    ...
    # ("manual provision").
    recipe_id = Column(
        Integer, ForeignKey("recipe.id", name="installation_recipe_id_fk")
    )
...
class RenderedKickstart(DeclarativeMappedObject):
    ...
    def __repr__(self):
        return "%s(id=%r, kickstart=%s, url=%r)" % (
            self.__class__.__name__,
            self.id,
            "<%s chars>" % len(self.kickstart)
    ...
    def link(self):
        ...
        url = absolute_url("/kickstart/%s" % self.id, scheme="http", labdomain=True)
        return url
