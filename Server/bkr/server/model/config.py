...
from sqlalchemy import Column, ForeignKey, Integer, Unicode, Boolean, DateTime, TEXT
...
class ConfigItem(DeclarativeMappedObject):
    ...
    def lazy_create(cls, name, description, numeric):
        return super(ConfigItem, cls).lazy_create(
            name=name, _extra_attrs=dict(description=description, numeric=numeric)
        )
    ...
    def current_value(self, default=None):
        v = (
            ...
            .filter(
                and_(
                    self.value_class.valid_from <= datetime.utcnow(),
                    self.value_class.config_item_id == self.id,
                )
            )
            .order_by(self.value_class.valid_from.desc())
            .first()
        )
        if v:
            return v.value
    ...
    def next_value(self):
        return (
            self.values()
            .filter(self.value_class.valid_from > datetime.utcnow())
            .order_by(self.value_class.valid_from.asc())
            .first()
        )
...
class ConfigValueString(DeclarativeMappedObject):
    ...
    config_item_id = Column(
        Integer,
        ForeignKey("config_item.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    )
