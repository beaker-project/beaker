...
class_mapper(Group).add_properties(
    {
        "dyn_owners": dynamic_loader(
            User,
            secondary=UserGroup.__table__,
            viewonly=True,
            primaryjoin=and_(
                UserGroup.group_id == Group.group_id, UserGroup.is_owner == True
            ),
        ),
    }
)
...
class_mapper(System).add_properties(
    {
        ...
        "recipes": relationship(
            ...
            secondaryjoin=and_(
                SystemResource.__table__.c.id == RecipeResource.id,
                RecipeResource.recipe_id == Recipe.id,
            ),
        ),
        "dyn_recipes": dynamic_loader(
            Recipe,
            secondary=RecipeResource.__table__.join(SystemResource.__table__),
            secondaryjoin=and_(
                SystemResource.__table__.c.id == RecipeResource.id,
