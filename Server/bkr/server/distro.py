...
class Distros(RPCRoot):
    ...
    def get_osmajors(self, tags=None):
        ...
        if tags:
            osmajors = (
                osmajors.join(OSMajor.osversions, OSVersion.distros, Distro.trees)
    ...
    def get_arch(self, filter):
        ...
        if "osmajor" in filter:
            distros = (
                distros.join(Distro.osversion)
                .join(OSVersion.osmajor)
                .filter(OSMajor.osmajor == filter["osmajor"])
            )
        if filter.get("variant"):
            distros = distros.join(Distro.trees).filter(
                DistroTree.variant == filter["variant"]
            )
    ...
    def _distros(self, distro, **kw):
        ...
        if "simplesearch" in kw:
            ...
            kw["distrosearch"] = [
                {"table": "Name", "operation": "contains", "value": kw["simplesearch"]}
            ]
        else:
            simplesearch = None
    ...
    def index(self, *args, **kw):
        distro_q = (
            session.query(Distro)
            .outerjoin(Distro.osversion, OSVersion.osmajor)
    ...
    def name(self, *args, **kw):
        distro_q = (
            session.query(Distro)
            .join(Distro.osversion, OSVersion.osmajor)
            .filter(Distro.trees.any(DistroTree.lab_controller_assocs.any()))
    ...
    def distros(self, distros, action=".", *args, **kw):
        ...
        distros_grid = myPaginateDataGrid(
            fields=[
                myPaginateDataGrid.Column(
                    name="id",
                    getter=lambda x: make_link(
                        url="/distros/view?id=%s" % x.id, text=x.id
                    ),
                    title="ID",
                ...
                myPaginateDataGrid.Column(
                    name="name",
                    getter=lambda x: make_link(
                        url="/distros/view?id=%s" % x.id, text=x.name
                    ),
                    title="Name",
                    options=dict(sortable=True),
                ),
                myPaginateDataGrid.Column(
                    name="osversion.osmajor.osmajor",
                    getter=lambda x: x.osversion.osmajor,
                    title="OS Major Version",
                    options=dict(sortable=True),
                ),
                myPaginateDataGrid.Column(
                    name="osversion.osminor",
