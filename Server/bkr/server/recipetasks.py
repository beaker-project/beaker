...
class RecipeTasks(RPCRoot):
    ...
    def _warn_once(self, recipetask, msg):
        ...
        if (
            RecipeTaskResult.query.join(RecipeTaskResult.recipetask)
            .filter(RecipeTask.recipe_id == recipetask.recipe_id)
            .filter(RecipeTaskResult.log == msg)
            .with_entities(func.count(RecipeTaskResult.id))
            .scalar()
            == 0
        ):
    ...
    def _check_log_limit(self, recipetask):
        ...
        if not max_logs or max_logs <= 0:
            return
        task_log_count = (
            LogRecipeTask.query.join(LogRecipeTask.parent)
            .filter(RecipeTask.recipe_id == recipetask.recipe_id)
            .count()
        )
        result_log_count = (
            LogRecipeTaskResult.query.join(
                LogRecipeTaskResult.parent, RecipeTaskResult.recipetask
            )
            .filter(RecipeTask.recipe_id == recipetask.recipe_id)
            .count()
        )
    ...
    def register_file(self, server, task_id, path, filename, basepath):
        ...
        if recipetask.is_finished():
            raise BX("Cannot register file for finished task %s" % recipetask.t_id)
    ...
    def register_result_file(self, server, result_id, path, filename, basepath):
        ...
        if result.recipetask.is_finished():
            raise BX(
                "Cannot register file for finished task %s" % result.recipetask.t_id
            )
    ...
    def watchdogs(self, status="active", lc=None):
        ...
        if lc is None:
            ...
            if not labcontroller:
                raise BX(
                    _(
                        u"Invalid login: %s, must log in as a lab controller"
    ...
    def stop(self, task_id, stop_type, msg=None):
        ...
        if stop_type not in task.stop_types:
            raise BX(
                _(
                    "Invalid stop_type: %s, must be one of %s"
    ...
    def result(self, task_id, result_type, path=None, score=None, summary=None):
        ...
        if result_type not in task.result_types:
            raise BX(
                _(
                    "Invalid result_type: %s, must be one of %s"
...
def post_recipe_task_comment(recipeid, taskid):
    ...
    with convert_internal_errors():
        comment = RecipeTaskComment(user=identity.current.user, comment=data["comment"])
...
@app.route(
    "/recipes/<recipeid>/tasks/<taskid>/results/<resultid>/logs/<path:path>",
    methods=["GET"],
)
def get_recipe_task_result_log(recipeid, taskid, resultid, path):
    ...
    for log in result.logs:
        if log.combined_path == path:
            return redirect(log.absolute_url, code=307)
    return NotFound404(
        "Result log %s for recipe %s task %s result %s not found"
        % (path, recipeid, taskid, resultid)
    )


@app.route(
    "/recipes/<recipeid>/tasks/<taskid>/results/<resultid>/comments/", methods=["GET"]
)
...
@app.route(
    "/recipes/<recipeid>/tasks/<taskid>/results/<resultid>/comments/", methods=["POST"]
)
@auth_required
def post_recipe_task_result_comment(recipeid, taskid, resultid):
    ...
    if "comment" not in data:
        raise BadRequest400('Missing "comment" key')
    with convert_internal_errors():
        comment = RecipeTaskResultComment(
            user=identity.current.user, comment=data["comment"]
        )
