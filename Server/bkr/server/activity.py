...
def get_activity():
    ...
    json_result = json_collection(
        query, columns=common_activity_search_columns, skip_count=True
    )
...
def get_distro_activity():
    ...
    query = query.join(DistroActivity.object).options(
        contains_eager(DistroActivity.object)
    )
    if not request.args.get("q"):
        # need to avoid a filesort due to MySQL picking distro as first table
        query = query.with_hint(
            DistroActivity.__table__,
            "IGNORE INDEX (ix_distro_activity_distro_id)",
            "mysql",
        )
    json_result = json_collection(
        query,
...
def get_distro_tree_activity():
    ...
    if not request.args.get("q"):
        ...
        query = query.with_hint(
            DistroTreeActivity.__table__,
            "IGNORE INDEX (ix_distro_tree_activity_distro_tree_id)",
            "mysql",
        )
    json_result = json_collection(
        query,
...
def get_group_activity():
    ...
    query = query.join(GroupActivity.object).options(
        contains_eager(DistroTreeActivity.object)
    )
    if not request.args.get("q"):
        # need to avoid a filesort due to MySQL picking group as first table
        query = query.with_hint(
            GroupActivity.__table__,
            "IGNORE INDEX (ix_group_activity_group_id)",
            "mysql",
        )
    json_result = json_collection(
        query,
...
def get_lab_controller_activity():
    ...
    query = query.join(LabControllerActivity.object).options(
        contains_eager(LabControllerActivity.object)
    )
    if not request.args.get("q"):
        # need to avoid a filesort due to MySQL picking lab_controller as first table
        query = query.with_hint(
            LabControllerActivity.__table__,
            "IGNORE INDEX (ix_lab_controller_activity_lab_controller_id)",
            "mysql",
        )
    json_result = json_collection(
        query,
...
def get_systems_activity():  # distinct from get_system_activity
    ...
    query = query.join(SystemActivity.object).options(
        contains_eager(SystemActivity.object)
    )
    if not request.args.get("q"):
        # need to avoid a filesort due to MySQL picking system as first table
        query = query.with_hint(
            SystemActivity.__table__,
            "IGNORE INDEX (ix_system_activity_system_id)",
            "mysql",
        )
    json_result = json_collection(
        query,
