...
class ConditionalInsertTest(unittest.TestCase):
    ...
    def test_with_extra_params(self):
        ...
        clause = ConditionalInsert(
            table, {table.c.name: "asdf"}, {table.c.extra: "something"}
        )
        ...
        self.assertEquals(
            compiled.params, {"name": "asdf", "extra": "something", "name_1": "asdf"}
        )
...
class TaskLibraryTest(unittest.TestCase):
    ...
    def test_createrepo_c_command(self):
        ...
        rpm_file = pkg_resources.resource_filename(
            "bkr.server.tests", "tmp-distribution-beaker-task_test-2.0-5.noarch.rpm"
        )
    ...
    def test_unlink_rpm(self):
        ...
        copy(rpm_file, self.tasklibrary.rpmspath)
        self.tasklibrary.unlink_rpm(
            "tmp-distribution-beaker-task_test-2.0-5.noarch.rpm"
        )
        self.assertTrue(
            not os.path.exists(
                os.path.join(
                    self.tasklibrary.rpmspath,
                    "tmp-distribution-beaker-task_test-2.0-5.noarch.rpm",
                )
            )
        )
