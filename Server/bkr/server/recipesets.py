    ...
    with convert_internal_errors():
        if "priority" in data:
            ...
            if priority != recipeset.priority:
                if not recipeset.can_change_priority(identity.current.user):
                    raise Forbidden403(
                        "Cannot change recipe set %s priority" % recipeset.id
                    )
                ...
                if priority not in allowed:
                    raise Forbidden403(
                        ...
                        % (
                            recipeset.id,
                            priority,
                            " ".join(unicode(pri) for pri in allowed),
                        )
                    )
                record_activity(
                    u"Priority", old=recipeset.priority.value, new=priority.value
                )
        ...
        if "waived" in data:
            ...
            if waived != recipeset.waived:
                ...
                record_activity(
                    u"Waived", old=unicode(recipeset.waived), new=unicode(waived)
                )
...
def post_recipeset_comment(id):
    ...
    with convert_internal_errors():
        comment = RecipeSetComment(user=identity.current.user, comment=data["comment"])
...
class RecipeSets(RPCRoot):
    ...
    message = widgets.TextArea(
        name="msg", label=_(u"Reason?"), help_text=_(u"Optional")
    )
    ...
    def cancel(self, id):
        ...
        return dict(
            ...
            action="./really_cancel",
            options={},
            value=dict(id=recipeset.id, confirm="really cancel recipeset %s?" % id),
        )
