...
class Groups(RPCRoot):
    ...
    def revoke_owner(self, group_id=None, id=None, **kw):
        ...
        if len(group.owners()) == 1 and not identity.current.user.is_admin():
            ...
            group.revoke_ownership(
                user=user, agent=identity.current.user, service=service
            )
    ...
    def revoke_ownership(self, group, kw):
        ...
        return self.revoke_owner(group_name=group, member_name=kw["member_name"])
    ...
    def grant_ownership(self, group, kw):
        ...
        """
        return self.grant_owner(group_name=group, member_name=kw["member_name"])
    ...
    def modify(self, group_name, kw):
        ...
        if group_name:
            try:
                ...
                if group_name != group.group_name:
                    raise BX(
                        _(
                            u"Failed to update group %s: Group name already exists: %s"
                            % (group.group_name, group_name)
                        )
                    )
        ...
        if kw.get("add_member", None):
            ...
            if user not in group.users:
                group.add_member(user, service=u"XMLRPC", agent=identity.current.user)
                mail.group_membership_notify(
                    user, group, agent=identity.current.user, action="Added"
                )
            else:
                raise BX(
                    _(u"User %s is already in group %s" % (username, group.group_name))
                )
            ...
            if user not in group.users:
                ...
                for usr in groupUsers:
                    if usr.user_id == user.user_id:
                        group.remove_member(
                            user, service=u"XMLRPC", agent=identity.current.user
                        )
                        removed = user
                        mail.group_membership_notify(
                            user, group, agent=identity.current.user, action="Removed"
                        )
                        break
...
def get_groups():
    ...
    query = query.options(
        joinedload("user_group_assocs"), joinedload("user_group_assocs.user")
    )
...
def groups_mine():
    ...
    return flask_redirect(
        absolute_url(
            "/groups/", q="member.user_name:%s" % identity.current.user.user_name
        )
    )
...
def create_group():
    ...
    with convert_internal_errors():
        ...
        session.add(group)
        group.record_activity(
            user=user, service=u"HTTP", field=u"Group", action=u"Created"
        )
...
def update_group(group_name):
    ...
    with convert_internal_errors():
        ...
        if "membership_type" in data:
            new_type = GroupMembershipType.from_string(data["membership_type"])
            if new_type == GroupMembershipType.ldap and not group.can_edit_ldap(user):
...
def delete_group(group_name):
    ...
    if group.is_protected_group():
        raise BadRequest400(
            "Group '%s' is predefined and cannot be deleted" % group.group_name
        )
    if group.jobs:
    ...
    activity = Activity(
        identity.current.user, u"HTTP", u"Removed", u"Group", group.display_name
    )
...
def add_group_membership(group_name):
    ...
    if user not in group.users:
        ...
        raise Conflict409(
            "User %s is already a member of group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def remove_group_membership(group_name):
    ...
    if user in group.users:
        ...
        raise Conflict409(
            "User %s is not a member of group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def exclude_user(group_name):
    ...
    if user in group.users:
        if not group.can_exclude_member(u, user.id):
            raise Forbidden403(
                "Cannot exclude user %s from group %s" % (user, group_name)
            )
        with convert_internal_errors():
            group.exclude_user(user, agent=identity.current.user)
    else:
        raise Conflict409(
            "User %s is already excluded from group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def readd_user(group_name):
    ...
    if user not in group.users:
        ...
        raise Conflict409(
            "User %s is not excluded from group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def grant_ownership(group_name):
    ...
    if not group.has_owner(user):
        ...
        raise Conflict409(
            "User %s is already an owner of group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def revoke_ownership(group_name):
    ...
    if group.has_owner(user):
        ...
        raise Conflict409(
            "User %s is not an owner of group %s" % (user.user_name, group_name)
        )
    return "", 204
...
def remove_permission(group_name):
    ...
    if permission in group.permissions:
        ...
        raise Conflict409(
            "Group %s does not have permission %s" % (group_name, permission_name)
        )
    return "", 204
