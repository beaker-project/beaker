...
def system_utilisation(system, start, end):
    ...
    status_durations = (
        system.dyn_status_durations.filter(
            and_(
        ...
        .order_by(SystemStatusDuration.start_time)
        .all()
    )
    reservations = (
    ...
    for reservation in reservations:
        ...
        # start of this one
        update_status_durations_in_period(
            retval, status_durations, prev_finish, clamped_res_start
        )
    ...
    # reporting period
    update_status_durations_in_period(retval, status_durations, prev_finish, end)
    return retval
...
def system_utilisation_counts(systems):
    ...
    query = (
        systems.outerjoin(System.open_reservation)
        .with_entities(
            func.coalesce(
                Reservation.type, func.concat("idle_", func.lower(System.status))
            ),
