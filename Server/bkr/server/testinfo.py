...
namespaces = [
    ("desktop", ["evolution", "openoffice.org", "poppler", "shared-mime-info"]),
    ("tools", ["gcc"]),
    ("CoreOS", ["rpm"]),
    ("cluster", []),
    ("rhn", []),
...
class Parser:
    ...
    def __init__(self):
        ...
        self.valid_priorities = ["Low", "Medium", "Normal", "High", "Manual"]
    ...
    def error_if_not_in_array(self, fieldName, value, validValues):
        if not value in validValues:
            errorMsg = '"%s" is not a valid value for %s; valid values are' % (
                value,
                fieldName,
            )
    ...
    def __unique_field(self, fileFieldName, dictFieldName, value, validator=None):
        ...
        if validator:
            if not validator.is_valid(value):
                self.handle_error(
                    '"%s" is not a valid %s field (%s)'
                    % (value, fileFieldName, validator.message())
                )
    ...
    def _handle_dict(
        self, fileFieldName, dictFieldName, value, validator=None, key_validator=None
    ):
        kv = value.split("=", 1)
        if len(kv) < 2:
            self.handle_error(
                "Malformed %s field not matching KEY=VALUE pattern" % fileFieldName
            )
            return
        k, v = kv
        ...
        if key_validator and not key_validator.is_valid(k):
            self.handle_error(
                '"%s" is not a valid key for %s field (%s)'
                % (k, fileFieldName, key_validator.message())
            )
            return
        if validator and not validator.is_valid(v):
            self.handle_error(
                '"%s" is not a valid %s field (%s)'
                % (v, fileFieldName, validator.message())
            )
            return
        d[k] = kv[1]

    def _handle_unique_list(
        self, fileFieldName, dictFieldName, value, validator=None, split_at=" "
    ):
        ...
        if validator:
            for item in items:
                if not validator.is_valid(item):
                    self.handle_error(
                        '"%s" is not a valid %s field (%s)'
                        % (item, fileFieldName, validator.message())
                    )
                    continue
                l.append(item)
        else:
    ...
    def handle_name(self, key, value):
        ...
        self.info.expected_path_under_mnt_tests_from_name = (
            self.info.test_name_under_root_ns
        )
    ...
    def handle_testversion(self, key, value):
        self.__unique_field(
            ...
            RegexValidator(
                r"^([A-Za-z0-9\.]*)$",
                "can only contain numbers, letters and the dot symbol",
            ),
        )
    ...
    def handle_releases(self, key, value):
        ...
        for release in value.split(" "):
            ...
            if num_negative_releases > 0 and num_positive_releases > 0:
                self.handle_warning(
                    "Releases field lists both negated and non-negated release names (should be all negated, or all non-negated)"
                )
    ...
    def handle_archs(self, key, value):
        ...
        for arch in value.split(" "):
            self.error_if_not_in_array(
                "Architecture", arch.lstrip("-"), self.valid_architectures
            )
            archs.append(arch)
        if any(arch.startswith("-") for arch in archs) and not all(
            arch.startswith("-") for arch in archs
        ):
            self.handle_warning(
                "Architectures field lists both negated and non-negated architectures (should be all negated, or all non-negated)"
            )
        self.info.test_archs = archs

    def handle_options(self, key, value):
        self._handle_unique_list(
            key, "options", value, DashListValidator(self.valid_options)
        )
    ...
    def handle_priority(self, key, value):
        self.__unique_field(
            key, "priority", value, ListValidator(self.valid_priorities)
        )
    ...
    def handle_testtime(self, key, value):
        ...
        if m:
            ...
            if suffix == "":
                ...
                self.handle_warning(
                    "TestTime unit is not valid, should be m (minutes) or h (hours)"
                )
                return
    ...
    def handle_bug(self, key, value):
        for bug in value.split(" "):
            ...
            if m:
                ...
                self.handle_error(
                    '"%s" is not a valid Bug value (should be numeric)' % bug
                )
        ...
        if m:
            ...
            self.handle_error(
                ...
                % (
                    value,
                    key,
                    "must be of the form PROPERTYNAME {=|>|>=|<|<=} PROPERTYVALUE",
                )
            )
    ...
    def __handle_siteconfig(self, arg, value):
        if re.match("^/.*", arg):
            ...
            if self.info.test_name:
                ...
                self.handle_error(
                    "A relative SiteConfig(): declaration appeared before a Name: field"
                )
                return
...
class PrintingParser(Parser):
    ...
    def handle_message(self, message, severity):
        ...
        print >>self.outputFileObj, "%s:%i: %s: %s" % (
            self.inputFilename,
            self.lineNum,
            severity,
            message,
