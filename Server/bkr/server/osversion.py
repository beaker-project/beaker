...
from bkr.server.widgets import myPaginateDataGrid, HorizontalForm, CheckBoxList
...
class OSVersions(AdminPage):
    ...
    alias = widgets.TextField(
        name="alias", validator=validators.UnicodeString(if_empty=None)
    )
    arches = CheckBoxList(
        name="arches",
        label="Arches",
    ...
    def edit(self, id=None, *args, **kw):
        try:
            ...
            redirect(".")
        return dict(
            title=unicode(osversion),
            value=dict(id=osversion.id, arches=[arch.id for arch in osversion.arches]),
        ...
        if osmajor.alias != alias:
            if alias:
                try:
                    ...
                    flash(
                        _(u"Cannot save alias %s, it is already used by %s")
                        % (alias, existing)
                    )
                    redirect(".")
            osmajor.alias = alias
    ...
    def save_osmajor_installopts(self, osmajor_id=None, installopts=None):
        ...
        for arch, options in installopts.iteritems():
            ...
            io = OSMajorInstallOptions.lazy_create(
                osmajor_id=osmajor.id, arch_id=Arch.by_name(arch).id if arch else None
            )
    ...
    def osversions(self, osversions=None, *args, **kw):
        q = session.query(
            self.search_mapper
        )  # This line +3 dupes the start of process_search
        if osversions is None:
            for j in self.join:
                q = q.join(j)
            osversions = q
        osversions_grid = myPaginateDataGrid(
            fields=[
                myPaginateDataGrid.Column(
                    name="osmajor.osmajor",
                    getter=lambda x: make_link(
                        url="./edit_osmajor?id=%s" % x.osmajor.id, text=x.osmajor
                    ),
                    title="OS Major",
                ...
                myPaginateDataGrid.Column(
                    name="osmajor.alias",
                    getter=lambda x: x.osmajor.alias,
                    title="Alias",
                    options=dict(sortable=True),
                ),
                myPaginateDataGrid.Column(
                    name="osminor",
                    getter=lambda x: make_link(
                        url="./edit?id=%s" % x.id, text=x.osminor
                    ),
                    title="OS Minor",
                    options=dict(sortable=True),
                ),
                myPaginateDataGrid.Column(
                    name="arches",
                    getter=lambda x: " ".join([arch.arch for arch in x.arches]),
                    title="Arches",
                    options=dict(sortable=True),
                ),
            ]
        ...
        return dict(
            title="OS Versions", grid=osversions_grid, addable=False, list=osversions
        )
