<div xmlns:py="http://purl.org/kid/ns#">
 ...
 <a name="RS_${recipeset.id}" />
  <tr>
   ...
   <td class="value">${recipeset.t_id} <span py:if="recipeset.is_owner(tg.identity.user) or 'admin' in tg.identity.groups"  id="response_${recipeset.id}">${ack_panel_widget.display(recipeset.id,name='response_box_%s' % recipeset.id)}</span> </td>  
