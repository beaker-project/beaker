...
def _testcases_for_task(task):
    if not task.is_finished():
        return
    testcase = E.testcase(classname=task.name, name="(main)")
    ...
    for result in task.results:
        testcase = E.testcase(
            classname=task.name, name=result.short_path.lstrip("/") or "(none)"
        )
        if result.duration:
        ...
        if (
            ...
            and result == task.results[-1]
            and result.result == TaskResult.warn
        ):
...
def job_to_junit_xml(job):
    ...
    return lxml.etree.tostring(
        testsuites, encoding="utf8", xml_declaration=True, pretty_print=True
    )
