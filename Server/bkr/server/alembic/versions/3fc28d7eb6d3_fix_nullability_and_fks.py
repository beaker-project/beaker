...
from bkr.server.alembic.migration_utils import find_fk, create_fk_if_absent, drop_fk


def upgrade():
    # This was an oversight in commit d8df7f836 for bug 598781.
    op.alter_column(
        "recipe", "autopick_random", existing_type=sa.Boolean, nullable=False
    )
    # This was an oversight in the commits for retention tags.
    op.alter_column(
        "retention_tag", "expire_in_days", existing_type=sa.Integer, nullable=False
    )
    op.alter_column(
        "retention_tag", "needs_product", existing_type=sa.Boolean, nullable=False
    )
    # This was an oversight in commits for "removing tasks".
    op.alter_column("task", "valid", existing_type=sa.Boolean, nullable=False)
    # This was an oversight in commit 747902274 for bug 596410.
    op.alter_column(
        "recipe_set_nacked", "created", existing_type=sa.DateTime, nullable=False
    )
    ...
    if find_fk("log_recipe", ["recipe_id"]) is None:
        ...
        op.alter_column(
            "log_recipe", "recipe_id", existing_type=sa.Integer, nullable=False
        )
    ...
    if find_fk("log_recipe_task_result", ["recipe_task_result_id"]) is None:
        ...
        op.alter_column(
            "log_recipe_task_result",
            "recipe_task_result_id",
            existing_type=sa.Integer,
            nullable=False,
        )
    # This was an oversight in commit e591d682 for bug 595801.
    drop_fk("job", ["retention_tag_id"])
    op.alter_column("job", "retention_tag_id", existing_type=sa.Integer, nullable=False)
    op.create_foreign_key(None, "job", "retention_tag", ["retention_tag_id"], ["id"])
    # This was an oversight in commit eb8cded8 for bug 560695.
    op.alter_column(
        "lab_controller", "disabled", existing_type=sa.Boolean, nullable=False
    )
    # This was an oversight in commit 1ec1b4f1 for bug 703885.
    op.alter_column("tg_user", "disabled", existing_type=sa.Boolean, nullable=False)
    # As above, this FK may be missing on very old databases due to MyISAM.
    create_fk_if_absent("task_property_needed", "task", ["task_id"], ["id"])


def downgrade():
    op.alter_column(
        "recipe", "autopick_random", existing_type=sa.Boolean, nullable=True
    )
    op.alter_column(
        "retention_tag", "expire_in_days", existing_type=sa.Integer, nullable=True
    )
    op.alter_column(
        "retention_tag", "needs_product", existing_type=sa.Boolean, nullable=True
    )
    op.alter_column(
        "task", "valid", existing_type=sa.Boolean, nullable=True, server_default="1"
    )
