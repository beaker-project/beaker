...
def upgrade():
    op.add_column(
        "tg_user", Column("openstack_trust_id", Unicode(length=4000), nullable=True)
    )
...
def downgrade():
    ...
    op.add_column(
        "tg_user", Column("openstack_username", Unicode(length=255), nullable=True)
    )
    op.add_column(
        "tg_user", Column("openstack_password", Unicode(length=2048), nullable=True)
    )
    op.add_column(
        "tg_user", Column("openstack_tenant_name", Unicode(length=2048), nullable=True)
    )
