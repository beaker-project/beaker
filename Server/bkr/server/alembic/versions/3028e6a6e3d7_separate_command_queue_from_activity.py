...
def downgrade():
    ...
    op.create_table(
        ...
        Column(
            "status",
            Enum(u"Queued", u"Running", u"Completed", u"Failed", u"Aborted"),
            nullable=False,
            index=True,
        ),
    ...
    if first_conflicting_id:
        ...
        log.debug(
            "Renumbering command_queue rows starting from %s", first_conflicting_id
        )
        ...
        command_queue_table = table(
            ...
            column("status"),
            column("delay_until"),
            column("quiescent_period"),
            column("error_message"),
            column("installation_id"),
        )
        command_queue_downgraded = table(
            "command_queue_downgraded",
            column("id"),
            column("system_id"),
            column("status"),
        ...
        conflicting_rows = connection.execute(
            command_queue_table.select().where(
                command_queue_table.c.id >= first_conflicting_id
            )
        )
        for row in conflicting_rows:
            ...
            connection.execute(
                insert(command_queue_downgraded).values(
                    id=new_id,
