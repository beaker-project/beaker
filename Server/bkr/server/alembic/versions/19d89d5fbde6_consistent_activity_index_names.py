...
def rename_index_if_necessary(table, columns, expected_index_name):
    ...
    if existing_index_name != expected_index_name:
        op.execute(
            ...
            % (table, existing_index_name, expected_index_name, ", ".join(columns))
        )


def upgrade():
    rename_index_if_necessary(
        "distro_activity", ["distro_id"], "ix_distro_activity_distro_id"
    )
    rename_index_if_necessary(
        "distro_tree_activity",
        ["distro_tree_id"],
        "ix_distro_tree_activity_distro_tree_id",
    )
    rename_index_if_necessary(
        "group_activity", ["group_id"], "ix_group_activity_group_id"
    )
    rename_index_if_necessary(
        "lab_controller_activity",
        ["lab_controller_id"],
        "ix_lab_controller_activity_lab_controller_id",
    )
    rename_index_if_necessary(
        "system_activity", ["system_id"], "ix_system_activity_system_id"
    )
