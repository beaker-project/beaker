...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Integer,
            sa.ForeignKey(
                "tg_user.user_id", name="system_access_policy_rule_user_id_fk"
            ),
        ),
        sa.Column(
            "group_id",
    ...
    # Group name length
    op.alter_column("tg_group", "group_name", type_=sa.Unicode(255), nullable=False)
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.Column("visit_key", sa.String(40), primary_key=True),
        sa.Column(
            "user_id", sa.Integer, sa.ForeignKey("tg_user.user_id"), nullable=False
        ),
        sa.Column("proxied_by_user_id", sa.Integer, sa.ForeignKey("tg_user.user_id")),
        mysql_engine="InnoDB",
    )

    # Group name length
    op.alter_column("tg_group", "group_name", type_=sa.Unicode(16), nullable=False)
