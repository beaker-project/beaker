...
def upgrade():
    op.create_table(
        ...
        Column(
            "system_id",
            Integer,
            ForeignKey("system.id", name="installation_system_id_fk"),
            nullable=True,
        ),
        Column(
            "recipe_id",
            Integer,
            ForeignKey("recipe.id", name="installation_recipe_id_fk"),
            nullable=True,
        ),
        mysql_engine="InnoDB",
    )
...
def downgrade():
    ...
    op.add_column(
        "virt_resource", Column("kernel_options", Unicode(2048), nullable=True)
    )
    op.execute(
        """
