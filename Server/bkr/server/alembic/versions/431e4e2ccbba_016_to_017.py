...
def upgrade():
    ...
    op.add_column(
        "tg_user",
        sa.Column("openstack_username", sa.Unicode(length=255), nullable=True),
    )
    op.add_column(
        "tg_user",
        sa.Column("openstack_password", sa.Unicode(length=2048), nullable=True),
    )
    op.add_column(
        "tg_user",
        sa.Column("openstack_tenant_name", sa.Unicode(length=2048), nullable=True),
    )
    op.add_column("virt_resource", sa.Column("instance_id", UUID(), nullable=False))
    op.add_column(
        "virt_resource",
        sa.Column("kernel_options", sa.Unicode(length=2048), nullable=True),
    )
    ...
    (old_index_name,) = [
        index["name"]
        for index in sa.inspect(op.get_bind()).get_indexes("lab_controller")
