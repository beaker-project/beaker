...
def upgrade():
    op.add_column(
        "tg_user",
        Column("notify_job_completion", Boolean, nullable=False, default=True),
    )
    op.add_column(
        "tg_user", Column("notify_broken_system", Boolean, nullable=False, default=True)
    )
    op.add_column(
        "tg_user",
        Column("notify_group_membership", Boolean, nullable=False, default=True),
    )
    op.add_column(
        "tg_user", Column("notify_reservesys", Boolean, nullable=False, default=True)
    )
    op.execute(
        "UPDATE tg_user SET notify_job_completion = TRUE, "
