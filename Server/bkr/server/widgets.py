...
class Hostname(validators.Regex):
    ...
    def __init__(self, **kwargs):
        super(Hostname, self).__init__(VALID_FQDN_REGEX, strip=True, **kwargs)
...
jquery = LocalJSLink(
    "bkr", "/static/javascript/jquery-2.0.2.min.js", order=1
)  # needs to come after MochiKit
...
class DoAndConfirmForm(Form):
    ...
    def __init__(self, *args, **kw):
        self.javascript.extend(
            [
                LocalJSLink(
                    "bkr", "/static/javascript/jquery-ui-1.9.2.min.js", order=3
                ),
                LocalJSLink("bkr", "/static/javascript/util.js"),
            ]
        )
...
class DeleteLinkWidgetAJAX(DeleteLinkWidget):
    ...
    def display(self, value=None, **params):
        missing = [
            (i, True) for i in ["action", "data", "callback"] if not params.get(i)
        ]
        if any(missing):
            raise ValueError(
                "Missing arguments to %s: %s"
...
class MatrixDataGrid(DataGrid):
    ...
    def update_params(self, d):
            ...
            try:
                ...
                try:
                    ...
                    raise BeakerException(
                        "Order number %s has already been specified,it cannot be specified twice"
                        % order
                    )
                except ValueError, e:
        ...
        decorated_headers = [
            (header, occurance, header_order[header])
            for header, occurance in headers.items()
        ]
        sorted_decorated_headers = sorted(decorated_headers, cmp=self._header_cmp)
        d["outer_headers"] = [
            (header, occurance) for header, occurance, order in sorted_decorated_headers
        ]
        if must_use_header:
            # Ensures that columns are sorted in the same manner as outer_headers
            columns = [
                column_to_sort
                for column_to_sort in d["columns"]
                if getattr(column_to_sort, "outer_header", None) is None
            ]
            columns_to_sort = [
                column_to_sort
                for column_to_sort in d["columns"]
                if getattr(column_to_sort, "outer_header", None) is not None
            ]
...
class JobQuickSearch(CompoundWidget):
    ...
    def __init__(self, *args, **kw):
        self.status_running = Button(
            default="Status is Running",
            name="status_running",
            attrs={
                "onclick": "document.location.href('./?jobsearch-0.table=Status&jobsearch-0.operation=is&jobsearch-0.value=Running&Search=Search')"
            },
        )
...
class JobMatrixReport(Form):
    ...
    def __init__(self, *args, **kw):
        ...
        self.whiteboard = MultipleSelectField(
            "whiteboard",
            label="Whiteboard",
            attrs={"size": 5, "class": "whiteboard"},
            validator=self.default_validator,
        )
        self.job_ids = TextArea(
            "job_ids", label="Job ID", rows=7, cols=7, validator=self.default_validator
        )
...
class SearchBar(RepeatingFormField):
    ...
    css = [LocalCSSLink("bkr", "/static/css/smoothness/jquery-ui.css")]
    javascript = [
        LocalJSLink("bkr", "/static/javascript/search_object.js"),
    ...
    def __init__(
        ...
        **kw
        ...
        table_field = SingleSelectFieldJSON(
            name="table", options=sorted(table.keys()), validator=validators.NotEmpty()
        )
        operation_field = SingleSelectFieldJSON(
            name="operation", options=[None], validator=validators.NotEmpty()
        )
        ...
        if extra_selects is not None:
            ...
            for elem in extra_selects:
                ...
                new_select = SingleSelectFieldJSON(
                    name=elem["name"],
                    options=[None],
                    css_classes=new_class,
                    validator=validators.NotEmpty(),
                    for_column=elem["column"],
                )
        ...
        if "quick_searches" in kw:
            if kw["quick_searches"] is not None:
                for elem, name in kw["quick_searches"]:
                    ...
                    if len(vals) != 3:
                        log.error(
                            "Quick searches expects vals as <column>-<operation>-<value>. The following is incorrect: %s"
                            % (elem)
                        )
                    else:
                        self.quickly_searches.append(
                            (name, "%s-%s-%s" % (vals[0], vals[1], vals[2]))
                        )
    ...
    def display(self, value=None, **params):
        if "options" in params:
            ...
            if "enable_custom_columns" in params["options"]:
                params["enable_custom_columns"] = params["options"][
                    "enable_custom_columns"
                ]
            if "extra_hiddens" in params["options"]:
                params["extra_hiddens"] = [
                    (k, v) for k, v in params["options"]["extra_hiddens"].iteritems()
                ]
...
class TaskSearchForm(RemoteForm):
    ...
    fields = [
        HiddenField(name="system_id", validator=validators.Int()),
        ...
        SingleSelectField(
            ...
            + [
                (m.id, m.osmajor)
                for m in model.OSMajor.ordered_by_osmajor(
                    model.OSMajor.used_by_any_recipe()
                )
            ],
        ),
        SingleSelectField(
            name="status",
            label=_(u"Status"),
            validator=ValidEnumValue(model.TaskStatus),
            options=lambda: [(None, "All")]
            + [(status, status.value) for status in model.TaskStatus],
        ),
        SingleSelectField(
            name="result",
            label=_(u"Result"),
            validator=ValidEnumValue(model.TaskResult),
            options=lambda: [(None, "All")]
            + [(result, result.value) for result in model.TaskResult],
        ),
    ]
...
class LabInfoForm(HorizontalForm):
    fields = [
        ...
        TextField(
            name="orig_cost", label=_(u"Original Cost"), validator=validators.Money()
        ),
        TextField(
            name="curr_cost", label=_(u"Current Cost"), validator=validators.Money()
        ),
        TextField(name="dimensions", label=_(u"Dimensions")),
        TextField(name="weight", label=_(u"Weight"), validator=validators.Int()),
        TextField(name="wattage", label=_(u"Wattage"), validator=validators.Int()),
        TextField(name="cooling", label=_(u"Cooling"), validator=validators.Int()),
    ]
...
class ExcludedFamilies(FormField):
    ...
    _selected_verb = "checked"
    _major_categories = [
        {"name": "RHEL", "match": "RedHatEnterprise"},
        {"name": "Fedora", "match": "Fedora"},
    ]
    ...
    def update_params(self, d):
        ...
        for arch, arch_options in d["options"]:
            options = {
                name: []
                for name in [category["name"] for category in self._major_categories]
            }
            options["Other"] = []
            for option in arch_options:
                ...
                options[option_category].append(
                    (option[0], option[1], soptions, option_attrs)
                )
...
class SystemInstallOptions(Form):
    ...
    def __init__(self, *args, **kw):
        ...
        self.id = HiddenField(name="id")
        self.prov_arch = SingleSelectField(
            name="prov_arch",
            label=_(u"Arch"),
            options=[],
            validator=validators.NotEmpty(),
        )
        self.prov_osmajor = SingleSelectField(
            name="prov_osmajor",
            label=_(u"Family"),
            options=lambda: [(0, "All")]
        ...
        self.prov_osversion = SingleSelectField(
            name="prov_osversion",
            label=_(u"Update"),
            options=[(0, u"All")],
            validator=validators.NotEmpty(),
        )
        self.prov_ksmeta = TextField(name="prov_ksmeta", label=_(u"Kickstart Metadata"))
        self.prov_koptions = TextField(name="prov_koptions", label=_(u"Kernel Options"))
        self.prov_koptionspost = TextField(
            name="prov_koptionspost", label=_(u"Kernel Options Post")
        )
...
class TasksWidget(CompoundWidget):
    ...
    link = LinkRemoteFunction(
        name="link", before="task_search_before()", on_complete="task_search_complete()"
    )
...
class ReportProblemForm(RemoteForm):
    ...
    def update_params(self, d):
        ...
        d["hidden_fields"] = []
        d["hidden_fields"].append(
            HiddenField(name="system", attrs={"value": d["system"]})
        )
        if d["recipe"]:
            d["hidden_fields"].append(
                HiddenField(name="recipe_id", attrs={"value": d["recipe"].id})
            )
        d["submit"].attrs.update(
            {
                "onClick": "return ! system_action_remote_form_request('%s', %s, '%s');"
                % (
                    d["options"]["name"],
                    jsonify_for_html(self.get_options(d)),
                    d["action"],
    ...
    def display(self, task, **params):
        if getattr(task.resource, "system", None):
            params["report_problem_options"] = {
                "system": task.resource.system,
                "recipe": task,
                "name": "report_problem_recipe_%s" % task.id,
...
class RecipeWidget(CompoundWidget):
    ...
    def update_params(self, d):
        ...
        d["recipe_systems"] = make_link(
            url("../recipes/systems?recipe_id=%d" % d["recipe"].id),
...
class ProductWidget(SingleSelectField, RPC):
    ...
    def display(self, value=None, *args, **params):
        params["options"] = [(self.product_deselected, "No Product")] + [
            (elem.id, elem.name)
            for elem in model.Product.query.order_by(model.Product.name).all()
        ]
...
class RetentionTagWidget(
    SingleSelectField, RPC
):  # FIXME perhaps I shoudl create a parent that both Retention and Priority inherit from
    ...
    def display(self, value=None, **params):
        params["options"] = [
            (elem.id, elem.tag) for elem in model.RetentionTag.query.all()
        ]
...
class JobWhiteboard(RPC, CompoundWidget):
    ...
    params_doc = {
        "action": "Form action (URL to submit to)",
...
class TaskActionWidget(RPC):
    ...
    def display(self, task, action=None, **params):
        id = task.id
        task_details = {"id": "disable_%s" % id, "t_id": id}
