...
class TimeoutHandler(MessagingHandler):
    ...
    def on_timer_task(self, event):
        if not event.container.connected:
            ...
            log.error(
                "send timeout expired with %s messages unsent, stopping container",
                len(self.msgs),
            )
            event.container.stop()
    ...
    def send_msgs(self, event):
        sender = event.container.create_sender(
            event.connection, target=self.conf["address"]
        )
...
class AMQProducer(object):
    ...
    def _send_all(self, messages):
        ...
        for url in sorted(self.urls, key=lambda k: random.random()):
            ...
            raise RuntimeError(
                "could not send {0} message{1} to any destinations, "
                "errors:\n{2}".format(
                    len(messages),
