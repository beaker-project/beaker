    ...
    def __init__(self, **kw):
        ...
        if "search_url" in kw:
            ...
            raise BeakerException(
                "%s does not specify a search_url" % self.__class__.__name__
            )
        ...
        self.search_auto = AutoCompleteField(
            name=self.search_name,
            search_controller=self.search_url,
            search_param=self.search_param,
            result_name=self.result_name,
        )
        self.search_widget_form = InlineForm(
            "Search",
            fields=[self.search_auto],
            method="get",
        ...
        if s_name in kw:
            if "text" in kw[s_name]:
                ...
                if "starts_with" in kw[s_name]["text"]:
                    q = q.filter(
                        self.search_col.like("%s%%" % kw[s_name]["text"]["starts_with"])
                    )
                else:
