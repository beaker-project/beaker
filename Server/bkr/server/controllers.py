...
class Devices:
    ...
    def view(self, id):
        ...
        systems = (
            System.all(identity.current.user)
            .join("devices")
            .filter_by(id=id)
            .distinct()
        )
        device_grid = myPaginateDataGrid(
            fields=[
                ("System", lambda x: make_link("/view/%s" % x.fqdn, x.fqdn)),
                ("Description", lambda x: device.description),
            ]
        )
        return dict(title="", grid=device_grid, search_bar=None, list=systems)
    ...
    def default(self, *args, **kw):
        ...
        if len(args) == 1:
            devices = (
                ...
                .join("device_class")
                .filter_by(device_class=args[0])
            )

        if len(args) != 1:
            devices = session.query(Device).join("device_class")
        devices_grid = myPaginateDataGrid(
            fields=[
                widgets.PaginateDataGrid.Column(
                    name="description",
                    getter=lambda x: make_link(
                        "/devices/view/%s" % x.id, x.description
                    ),
                    title="Description",
                ...
                widgets.PaginateDataGrid.Column(
                    ...
                    getter=lambda x: x.device_class,
                    title="Type",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="bus",
                    getter=lambda x: x.bus,
                    title="Bus",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="driver",
                    getter=lambda x: x.driver,
                    title="Driver",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="vendor_id",
                    getter=lambda x: x.vendor_id,
                    title="Vendor ID",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="device_id",
                    getter=lambda x: x.device_id,
                    title="Device ID",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    ...
                    title="Subsys Vendor ID",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="subsys_device_id",
                    getter=lambda x: x.subsys_device_id,
                    title="Subsys Device ID",
                    options=dict(sortable=True),
                ),
                widgets.PaginateDataGrid.Column(
                    name="fw_version",
                    getter=lambda x: x.fw_version,
                    title="Firmware Version",
                    options=dict(sortable=True),
                ),
            ]
        )
        return dict(title="Devices", grid=devices_grid, search_bar=None, list=devices)
...
class Root(RPCRoot):
    ...
    for entry_point in pkg_resources.iter_entry_points("bkr.controllers"):
        ...
        log.info(
            "Attaching root extension controller %s as %s", controller, entry_point.name
        )
    ...
    def get_osversions(self, osmajor_id=None):
        ...
        try:
            ...
            osversions.extend(
                [(osversion.id, osversion.osminor) for osversion in osmajor.osversions]
            )
    ...
    def change_priority_recipeset(self, priority, recipeset_id):
        ...
        try:
            ...
            log.error(
                "No rows returned for recipeset_id %s in change_priority_recipeset:%s"
                % (recipeset_id, e)
            )
    ...
    @expose(template="bkr.server.templates.grid")
    @expose(
        template="bkr.server.templates.systems_feed",
        format="xml",
        as_format="atom",
        content_type="application/atom+xml",
        accept_format="application/atom+xml",
    )
    @paginate("list", default_order="fqdn", limit=20, max_limit=None)
    def index(self, *args, **kw):
        return self._systems(
            systems=System.all(identity.current.user).filter(
    ...
    def available(self, *args, **kw):
        ...
        return self._systems(systems=query, title=u"Available Systems", *args, **kw)
    ...
    def free(self, *args, **kw):
        ...
        return self._systems(systems=query, title=u"Free Systems", *args, **kw)
    ...
    def removed(self, *args, **kw):
        query = System.all(identity.current.user).filter(
            System.status == SystemStatus.removed
        )
        return self._systems(
            systems=query, title=u"Removed Systems", exclude_status=True, *args, **kw
        )
    ...
    def mine(self, *args, **kw):
        systems = System.mine(identity.current.user).filter(
            System.status != SystemStatus.removed
        )
        return self._systems(systems=systems, title=u"My Systems", *args, **kw)
    ...
    def reserve_system(self, *args, **kw):
        ...
        return_dict = self._systems(
            systems=query,
            title=u"Reserve Systems",
            direct_columns=[(8, direct_column)],
            *args,
            **kw
        )
    ...
    def _history_search(self, activity, **kw):
        ...
        for search in kw["historysearch"]:
            col = search["table"]
            history_search.append_results(
                search["value"], col, search["operation"], **kw
            )
    ...
    def _systems(self, systems, title, *args, **kw):
        ...
        if "quick_search" in kw:
            ...
            kw["systemsearch"] = [
                {"table": table, "operation": op, "keyvalue": None, "value": value}
            ]
            simplesearch = kw["simplesearch"]
        ...
        if "type" in kw:
            kw["systemsearch"] = [
                {"table": "System/Type", "operation": "is", "value": kw["type"]}
            ]
        ...
        if kw.get("xmlsearch"):
            try:
                systems = XmlHost.from_string(
                    "<and>%s</and>" % kw["xmlsearch"]
                ).apply_filter(systems)
            except ValueError, e:
        ...
        if kw.get("systemsearch"):
            ...
            my_fields = Utility.custom_systems_grid(
                system_columns_desc, use_custom_columns
            )
        else:
            columns = None
            searchvalue = None
        ...
        display_grid = myPaginateDataGrid(fields=my_fields, add_script=add_script)
    ...
    def _view_system_as_html(self, fqdn=None, **kw):
        ...
        distro_picker_options = {
            ...
            "osmajor": [
                ...
                for osmajor in OSMajor.ordered_by_osmajor(
                    OSMajor.in_lab(system.lab_controller)
                )
            ],
        }
        ...
        options["reprovision_distro_tree_id"] = [
            (dt.id, unicode(dt))
            for dt in system.distro_trees().order_by(
                Distro.name, DistroTree.variant, DistroTree.arch_id
            )
        ]
        # Excluded Family options
        options["excluded_families"] = []
        for arch in system.arch:
            options["excluded_families"].append(
                (
                    arch.arch,
                    [
                        (
        ...
        for arch in system.arch:
            ...
            if kw.get("excluded_families") and arch.arch in kw["excluded_families"]:
                if isinstance(kw["excluded_families"][arch.arch], list):
                    excluded_osmajor = [
                        int(i) for i in kw["excluded_families"][arch.arch]
                    ]
                else:
                    excluded_osmajor = [int(kw["excluded_families"][arch.arch])]
                for new_families in excluded_osmajor:
                    if new_families not in [
                        osmajor.osmajor.id
                        for osmajor in system.excluded_osmajor_byarch(arch)
                    ]:
                        new_excluded_osmajor = ExcludeOSMajor(
                            osmajor=OSMajor.by_id(new_families), arch=arch
                        )
                        system.record_activity(
                            user=identity.current.user,
                            service=u"WEBUI",
            ...
            for old_families in system.excluded_osmajor_byarch(arch):
                if old_families.osmajor.id not in excluded_osmajor:
                    system.record_activity(
                        ...
                        action=u"Removed",
                        field=u"Excluded_families",
                        old=u"%s/%s" % (old_families.osmajor, arch),
                        new=u"",
                    )
                    session.delete(old_families)

            if (
                kw.get("excluded_families_subsection")
                and arch.arch in kw["excluded_families_subsection"]
            ):
                if isinstance(kw["excluded_families_subsection"][arch.arch], list):
                    excluded_osversion = [
                        int(i) for i in kw["excluded_families_subsection"][arch.arch]
                    ]
                else:
                    excluded_osversion = [
                        int(kw["excluded_families_subsection"][arch.arch])
                    ]
                for new_osversion in excluded_osversion:
                    if new_osversion not in [
                        osversion.osversion.id
                        for osversion in system.excluded_osversion_byarch(arch)
                    ]:
                        ...
                        system.record_activity(
                            ...
                            field=u"Excluded_families",
                            old=u"",
                            new=u"%s/%s" % (new_excluded_osversion.osversion, arch),
                        )
    ...
    def remove_install(self, system_id, arch_id, **kw):
        ...
        if kw.get("osversion_id"):
            ...
            prov = (
                system.provisions[arch]
                .provision_families[osversion.osmajor]
    ...
    def save_install(self, id, **kw):
        ...
        if (
            kw.get("prov_ksmeta")
            or kw.get("prov_koptions")
            or kw.get("prov_koptionspost")
        ):
            ...
            if int(kw["prov_osversion"]) != 0:
                ...
                if arch in system.provisions:
                    if osversion.osmajor in system.provisions[arch].provision_families:
                        if (
                            osversion
                            in system.provisions[arch]
                            .provision_families[osversion.osmajor]
                            .provision_family_updates
                        ):
                            provision = (
                                system.provisions[arch]
                                .provision_families[osversion.osmajor]
                        ...
                        system.provisions[arch].provision_families[
                            osversion.osmajor
                        ].provision_family_updates[osversion] = provision
    ...
    def to_xml(self, taskid, to_screen=False, pretty=True, *args, **kw):
        ...
        xml_text = lxml.etree.tostring(
            task.to_xml(), pretty_print=pretty, encoding="utf8"
        )
        ...
        if to_screen:  # used for testing contents of XML
            ...
            cherrypy.response.headers["Content-Disposition"] = (
                "attachment; filename=%s.xml" % taskid
            )
    ...
    def login(self, forward_url=None, **kwargs):
        ...
        if cherrypy.request.method == "POST":
            ...
            if (
                user is not None
                and user.can_log_in()
                and user.check_password(kwargs.get("password"))
            ):
                # Attempt successful
