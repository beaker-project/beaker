...
class RetentionTag(AdminPage):
    ...
    default = widgets.SingleSelectField(
        name="default", label=(u"Default"), options=[(0, "False"), (1, "True")]
    )
    id = widgets.HiddenField(name="id")
    expire_in_days = widgets.TextField(
        name="expire_in_days",
        label=_(u"Expire In Days"),
    ...
    def tags(self, tags=None, user=None, *args, **kw):
        ...
        my_fields = [
            myPaginateDataGrid.Column(
                name="tag",
                title="Tags",
                getter=lambda x: show_tag(x),
                options=dict(sortable=True),
            ),
            myPaginateDataGrid.Column(
                name="default",
                title="Default",
                getter=lambda x: x.default,
                options=dict(sortable=True),
            ),
            myPaginateDataGrid.Column(
                name="delete", title="Delete", getter=lambda x: show_delete(x)
            ),
        ]
