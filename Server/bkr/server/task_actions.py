...
class TaskActions(RPCRoot):
    ...
    task_types = dict(J=Job, RS=RecipeSet, R=Recipe, T=RecipeTask, TR=RecipeTaskResult)

    stoppable_task_types = dict(
        [
            (rep, obj)
    ...
    @cherrypy.expose
    def to_xml(
        self, taskid, clone=False, exclude_enclosing_job=True, include_logs=True
    ):
        """
    ...
    def stop(self, taskid, stop_type, msg):
        ...
        if not task.can_stop(identity.current.user):
            raise BX(_("You don't have permission to %s %s" % (stop_type, taskid)))
        kwargs = dict(msg=msg)
        task.record_activity(
