...
def find_task_version_url(task_xml):
    ...
    return xml.find(".").attrib["version"], xml.find("./rpms/rpm").attrib["url"]
...
class TaskLibrarySync:
    ...
    def check_perms(self):
        ...
        if os.geteuid() != task_dir_uid:
            self.logger.critical(
                "You should run this script as user: %s"
    ...
    def _get_task_xml(self, server, task):
        ...
        try:
            self.logger.debug(
                "Getting task XML for %s from %s" % (task, getattr(self, server))
            )
            ...
            self.logger.error(
                "Could not get task XML for %s from %s. Continuing." % (task, server)
            )
    ...
    def sync_tasks(self, urls_to_sync):
        ...
        while rpms_synced < total_number_of_rpms:
            session.begin()
            try:
                tasks_and_writes_current_batch = tasks_and_writes[
                    rpms_synced : rpms_synced + self.batch_size
                ]
                ...
                self.logger.exception(
                    "Error syncing tasks. Got error %s" % (unicode(e))
                )
                break
            session.commit()
    ...
    def tasks_add(self, new_tasks, old_tasks):
        ...
        for task in old_tasks:
            ...
            if None not in [remote_task_xml, local_task_xml]:
                remote_task_version, remote_task_url = find_task_version_url(
                    remote_task_xml
                )
...
def get_parser():
    ...
    parser.add_option(
        "--remote", dest="remote", help="Remote Beaker Instance", metavar="remote"
    )
...
def main():
    ...
    if not options.force:
        if len(old_tasks) > 0:
            task_sync.logger.warning(
                "Warning: %d tasks already present may be overwritten "
