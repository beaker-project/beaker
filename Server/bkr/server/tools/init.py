...
def check_db(metadata, target_version):
    ...
    if current != target:
        logger.info(
            "Current schema %s does not match target revision %s", current, target
        )
        return False
    return True
...
def populate_db(
    user_name=None, password=None, user_display_name=None, user_email_address=None
):
    ...
    try:
        ...
        lab_controller = Group(
            group_name=u"lab_controller", display_name=u"Lab Controller"
        )
    ...
    if RetentionTag.query.count() == 0:
        ...
        session.add(
            RetentionTag(tag=u"120days", needs_product=False, expire_in_days=120)
        )
    ...
    config_items = [
        ...
        (
            u"root_password_validity",
            u"Maximum number of days a user's root password is valid for",
            True,
        ),
        (
            u"guest_name_prefix",
            u"Prefix for names of dynamic guests in OpenStack",
            False,
        ),
        (
            u"guest_private_network",
            u"Network address in CIDR format for private networks"
            " of dynamic guests in OpenStack.",
            False,
        ),
    ]
    ...
    if ConfigItem.by_name(u"guest_private_network").current_value() is None:
        ConfigItem.by_name(u"guest_private_network").set(
            u"192.168.10.0/24", user=admin.users[0]
        )
...
def upgrade_db(metadata):
    ...
    def upgrade(rev, context):
        ...
        if not rev:
            ...
            # really is.
            logger.info(
                "Database has no Alembic version, "
            ...
            if "recipe_reservation" in table_names:
                ...
                col["name"] == "name" for col in inspector.get_columns("recipe_task")
            ):
                ...
                col["name"] == "ldap" for col in inspector.get_columns("tg_group")
            ):
                ...
                col["name"] == "rebooted"
                for col in inspector.get_columns("recipe_resource")
            ):
                rev = None  # 0.11 (base)
            else:
                raise RuntimeError(
                    "Database has no Alembic version and "
...
def create_alembic_env_context(metadata, func=None):
    ...
    return alembic.environment.EnvironmentContext(config=config, script=script, fn=func)
...
def get_parser():
    ...
    parser.add_option(
        ...
        type="string",
        dest="password",
        help="password of Admin account",
    )
    parser.add_option(
        "-e",
        "--email",
    ...
    parser.add_option(
        ...
        type="string",
        dest="display_name",
        help="Full name of Admin account",
    )
    parser.add_option(
        "--downgrade",
        type="string",
...
def main():
    ...
    if not opts.background:
        log_to_stream(
            sys.stderr, level=logging.DEBUG if opts.debug else logging.WARNING
        )
        return doit(opts)
    else:
        ...
        if existing_pid:
            if process_is_alive(existing_pid):
                ...
                sys.stderr.write(
                    "Pid file %s exists but pid %s is dead, "
    ...
    if opts.check:
        ...
        if database_is_empty or only_has_alembic_version_table:
            if not opts.user_name:
                logger.error(
                    "Database is empty, you must pass --user to create an admin user"
                )
                return 1
            init_db(metadata)
        else:
            ...
            upgrade_db(metadata)
        populate_db(
            opts.user_name, opts.password, opts.display_name, opts.email_address
        )
    logger.info("Exiting")
    return 0
