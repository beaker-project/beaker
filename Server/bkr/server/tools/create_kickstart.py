...
from bkr.server.kickstart import (
    generate_kickstart,
    template_env,
    add_to_template_searchpath,
)
...
def main(*args):
    parser = optparse.OptionParser(
        "usage: %prog [options]", description=__description__, version=__version__
    )
    parser.add_option(
        "-u",
        "--user",
    ...
    parser.add_option(
        "-r", "--recipe-id", metavar="ID", help="Recreate kickstart based on recipe ID"
    )
    parser.add_option(
        "-d",
        "--distro-tree-id",
    ...
    parser.add_option(
        "-t", "--template-dir", metavar="DIR", help="Retrieve templates from DIR"
    )
    parser.add_option(
        "-f",
        "--system",
    ...
    parser.add_option(
        "-m", "--ks-meta", metavar="OPTIONS", default="", help="Kickstart meta data"
    )
    parser.add_option(
        "-p",
        "--kernel-options-post",
    ...
    with session.begin():
        ...
        if options.distro_tree_id:
            try:
                ...
                raise RuntimeError(
                    "Distro tree id '%s' does not exist" % options.distro_tree_id
                )
        if options.system:
            ...
            if distro_tree and not options.recipe_id:
                install_options = system.manual_provision_install_options(
                    distro_tree
                ).combined_with(
        ...
        if options.recipe_id:
            ...
            if not recipe.resource and not options.system:
                raise RuntimeError(
                    "Recipe must have (or had) a resource" " assigned to it"
                )
            if not system:
            ...
            osmajor = (
                ...
                if distro_tree
                else recipe.installation.osmajor
            )
            osminor = (
                distro_tree.distro.osversion.osminor
                if distro_tree
                else recipe.installation.osminor
            )
            variant = (
                distro_tree.variant if distro_tree else recipe.installation.variant
            )
            ...
            sources.append(
                InstallOptions.from_strings(
                    recipe.ks_meta, recipe.kernel_options, recipe.kernel_options_post
                )
            )
        ...
        installation = (
            recipe.installation
            if recipe and recipe.installation
            else FakeInstallation(
                distro_tree.distro.osversion.osmajor.osmajor,
                distro_tree.distro.osversion.osminor,
