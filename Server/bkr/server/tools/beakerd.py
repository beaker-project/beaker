...
from bkr.server.util import load_config_or_exit, log_traceback, get_reports_engine
...
def update_dirty_jobs():
    ...
    if job_ids:
        log.debug(
            "Updating dirty jobs [%s ... %s] (%d total)",
            job_ids[0],
            job_ids[-1],
            len(job_ids),
        )
...
def process_new_recipes(*args):
    ...
    with session.begin():
        recipes = (
            MachineRecipe.query.join(MachineRecipe.recipeset)
            .join(RecipeSet.job)
    ...
    if recipe_ids:
        log.debug(
            "Processing new recipes [%s ... %s] (%d total)",
            recipe_ids[0],
            recipe_ids[-1],
            len(recipe_ids),
        )
...
def process_new_recipe(recipe_id):
    ...
    if config.get("beaker.priority_bumping_enabled", True) and len(recipe.systems) == 1:
        ...
        try:
            ...
            recipe.recipeset.record_activity(
                user=None,
                service=u"Scheduler",
                action=u"Changed",
                field=u"Priority",
...
def queue_processed_recipesets(*args):
    ...
    with session.begin():
        recipesets = (
            RecipeSet.by_recipe_status(TaskStatus.processed)
    ...
    if recipeset_ids:
        log.debug(
            "Queuing processed recipe sets [%s ... %s] (%d total)",
            recipeset_ids[0],
            recipeset_ids[-1],
            len(recipeset_ids),
        )
...
def queue_processed_recipeset(recipeset_id):
    ...
    if len(list(recipeset.machine_recipes)) > 1:
        ...
        rsl_controllers = set(
            LabController.query.join("systems", "queued_recipes", "recipeset")
            .filter(RecipeSet.id == recipeset.id)
            .all()
        )
        ...
        for recipe in recipeset.machine_recipes:
            rl_controllers = set(
                LabController.query.join("systems", "queued_recipes")
            ...
            if not enough_systems:
                ...
                for recipe in recipeset.machine_recipes:
                    ...
                    for tmprecipe in recipeset.machine_recipes:
                        ...
                        if recipe != tmprecipe and systems == tmpsystems:
                            count += 1
        ...
        for recipe in recipeset.machine_recipes:
            for l_controller in bad_l_controllers:
                ...
                log.debug(
                    "recipe: %s labController: %s Removing lab"
                    % (recipe.id, l_controller)
                )
        ...
        if dead_recipes:
            ...
            log.debug(
                ...
                recipeset.t_id,
                ", ".join(recipe.t_id for recipe in dead_recipes),
            )
            log.info("%s moved from Processed to Aborted" % recipeset.t_id)
            recipeset.abort(
                u"Recipe ID %s does not match any systems"
...
def abort_dead_recipes(*args):
    ...
    with session.begin():
        ...
        recipes = (
            ...
            .filter(
                or_(DistroTree.id.isnot(None), System.status == SystemStatus.broken)
            )
            .filter(or_(*filters))
        )
        recipe_ids = [recipe_id for recipe_id, in recipes.values(MachineRecipe.id)]
    if recipe_ids:
        log.debug(
            "Aborting dead recipes [%s ... %s] (%d total)",
            recipe_ids[0],
            recipe_ids[-1],
            len(recipe_ids),
        )
...
def schedule_pending_system(system_id):
    ...
    # * Recipe id
    recipes = (
        recipes.order_by(RecipeSet.lab_controller == None)
        .order_by(RecipeSet.priority.desc())
        .order_by(RecipeSet.id)
        .order_by(Recipe.id)
    )
    ...
    if not recipe.candidate_systems().filter(System.id == system.id).first():
        log.debug(
            "System : %s recipe: %s no longer has access. removing"
            % (system, recipe.id)
        )
        recipe.systems.remove(system)
...
def schedule_recipe_on_system(recipe, system):
    ...
    for guestrecipe in recipe.guests:
        ...
        log.info(
            "recipe ID %s guest %s moved from Queued to Scheduled",
            recipe.id,
            guestrecipe.id,
        )
...
def provision_virt_recipes(*args):
    ...
    with session.begin():
        recipes = (
            MachineRecipe.query.join(Recipe.recipeset)
            .join(RecipeSet.job)
    ...
    if recipe_ids:
        log.debug(
            "Provisioning dynamic virt guests for recipes [%s ... %s] (%d total)",
            recipe_ids[0],
            recipe_ids[-1],
            len(recipe_ids),
        )
...
def provision_virt_recipe(recipe_id):
    ...
    try:
        ...
        flavor = min(
            possible_flavors, key=lambda flavor: (flavor.ram, flavor.disk, flavor.id)
        )
        vm_name = "%srecipe-%s" % (
...
def provision_scheduled_recipesets(*args):
    ...
    if recipeset_ids:
        log.debug(
            "Provisioning scheduled recipe sets [%s ... %s] (%d total)",
            recipeset_ids[0],
            recipeset_ids[-1],
            len(recipeset_ids),
        )
    ...
    for recipe in recipeset.recipes:
        ...
        try:
            ...
            recipe.recipeset.abort(
                u"Failed to provision recipeid %s, %s" % (recipe.id, e)
            )
            return
...
def _recipe_count_metrics_for_query_grouped(name, grouping, query):
    ...
    for group, counts in group_counts.iteritems():
        for status, count in counts.iteritems():
            metrics.measure("gauges.recipes_%s.%s.%s" % (status, name, group), count)
...
def recipe_count_metrics():
    ...
    _recipe_count_metrics_for_query_grouped(
        "by_arch", Arch.arch, MachineRecipe.query.join(DistroTree).join(Arch)
    )
...
def _system_count_metrics_for_query_grouped(name, grouping, query):
    ...
    for group, counts in group_counts.iteritems():
        for state, count in counts.iteritems():
            if state != "idle_removed":
                metrics.measure(
                    "gauges.systems_%s.%s.%s" % (state, name, group.replace(".", "_")),
                    count,
                )
...
def system_count_metrics():
    ...
    _system_count_metrics_for_query(
        "shared",
        System.query.outerjoin(System.active_access_policy).filter(
            SystemAccessPolicy.grants_everybody(SystemPermission.reserve)
        ),
    )
    _system_count_metrics_for_query_grouped(
        "by_arch", Arch.arch, System.query.join(System.arch)
    )
    _system_count_metrics_for_query_grouped(
        "by_lab", LabController.fqdn, System.query.join(System.lab_controller)
    )
...
def system_command_metrics():
    ...
    _system_command_metrics_for_query_grouped(
        "by_lab",
        LabController.fqdn,
        Command.query.join(Command.system).join(System.lab_controller),
    )
    _system_command_metrics_for_query_grouped(
        "by_arch", Arch.arch, Command.query.join(Command.system).join(System.arch)
    )
    _system_command_metrics_for_query_grouped(
        "by_power_type",
        PowerType.name,
        Command.query.join(Command.system).join(System.power).join(Power.power_type),
    )
...
def schedule():
    ...
    if _outstanding_data_migrations:
        log.debug(
            "Incomplete data migrations will be run: %s",
    ...
    main_recipes_thread = threading.Thread(
        target=main_recipes_loop, name="main_recipes"
    )
