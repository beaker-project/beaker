...
def generate_image(delete=True):
    ...
    subprocess.check_call(
        ["mformat", "-i", f.name, "-C", "-t", "4", "-h", "64", "-n", "32", "::"]
    )
    subprocess.check_call(["syslinux", "--install", f.name])
    subprocess.check_call(
        ["mcopy", "-i", f.name, "/usr/share/ipxe/ipxe.lkrn", "::ipxe.lkrn"]
    )
    mcopy = subprocess.Popen(
        ["mcopy", "-i", f.name, "-", "::syslinux.cfg"], stdin=subprocess.PIPE
    )
    mcopy.communicate(
        """\
DEFAULT ipxe
...
"""
        % absolute_url(
            "/systems/by-uuid/${uuid}/ipxe-script", scheme="http", labdomain=True
        )
    )
    if mcopy.returncode != 0:
        raise RuntimeError(
            "mcopy syslinux.cfg failed with return code %s" % mcopy.returncode
        )
    return f
...
def main():
    ...
    parser.add_option(
        "--image-visibility",
        help="OpenStack Image visibility",
        type="choice",
    ...
    if options.upload:
        ...
        if not project_name:
            parser.error(
                "Specify project with --os-project-name or env[OS_PROJECT_NAME]"
            )
        ...
        user_domain_name = options.os_user_domain_name or os.environ.get(
            "OS_USER_DOMAIN_NAME"
        )
        project_domain_name = options.os_project_domain_name or os.environ.get(
