...
class System_Power(BeakerCommand):
    ...
    def options(self):
        ...
        self.parser.add_option(
            "--clear-netboot",
            action="store_true",
            help="Clear system's netboot configuration " "before performing action",
        )
        self.parser.add_option(
            "--force",
            action="store_true",
            help="Perform action even if system is " "currently in use by another user",
        )
    ...
    def run(self, *args, **kwargs):
        ...
        if kwargs["action"] not in self.valid_actions:
            self.parser.error("Power action must be one of: %r" % (self.valid_actions,))

        json_data = {"only_if_current_user_matches": True}
        if kwargs["force"]:
