...
class Workflow_Installer_Test(BeakerWorkflow):
    ...
    def options(self):
        ...
        self.parser.add_option(
            "--template", default="", help="Kickstart template to use for installation"
        )
    ...
    def run(self, *args, **kwargs):
        sys.stderr.write(
            "workflow-installer-test is deprecated, use "
        ...
        if not ksTemplate:
            sys.stderr.write(
                "You must specify kickstart template to run this workflow\n"
            )
            sys.exit(1)
        ...
        if family:
            ...
            for distro in self.hub.distrotrees.filter(
                {"name": distro, "family": family}
            ):
                ...
                kwargs["os_minor"] = int(
                    os_version.replace(family, "").replace(".", "")
                )
                break
