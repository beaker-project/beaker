...
class JobArguments(object):
    ...
    def get_next_argument_on_tag(self, tag_name):
        ...
        for key in self.arguments.keys():
            if (
                self.arguments[key]["processed"] is False
    ...
    def create_tag(self, key):
        ...
        if rxp:
            ...
            if len(res) != 4:
                raise Exception(
                    "Error in <tag/> - value '%s' is not parsable" % tagname
                )
    ...
    def is_valid(self, key):
        ...
        """
        return not (
            self.arguments[key]["optional"] is False
...
class JobConfig(object):
    ...
    def __init__(self, argpars, jobdefaults):
        ...
        self.grparser = OptionGroup(
            argpars,
            "XSLT Job Config Specific Options",
            "These options are specific to and defined in " "the Job Configuration XML",
        )
    ...
    def parse_job_xml(self, parser, jobxml):
        ...
        try:
            ...
            raise Exception(
                "Job XML (%s) is missing <name/> and/or <xslt/> tags" % jobxml
            )
        ...
        for xnode in job_cfg.xpath("/jobConfig/arguments/arg"):
            ...
            for node in xnode.getchildren():
                if node.tag == "text":
                    ...
                    tagvaltype = (
                        "type" in node.attrib and node.attrib["type"].strip() or None
                    )
                    tagvalname = (
                        "attrname" in node.attrib
                        and node.attrib["attrname"].strip()
                        or None
                    )
                    tagnamelmnt = None
                    if tagvaltype == "list":
                        tagnamelmnt = (
                            "element_tag" in node.attrib
                            and node.attrib["element_tag"]
                            or "value"
                        )
            ...
            if tagname is None or tagvaltype is None:
                raise Exception(
                    "The <tag/> tag on '%s' must be present with a "
            ...
            if (
                tagvaltype != "value"
                and tagvaltype != "attribute"
                and tagvaltype != "list"
            ):
                raise Exception(
                    "The <tag/> type attribute on '%s' must be either "
            ...
            if tagvaltype == "attribute" and tagvalname is None:
                raise Exception(
                    "The <tag/> on '%s' is missing a 'name' attribute" % arglong
                )
    ...
    def validate_arguments(self, kwargs):
        ...
        if self.__jobxml_parsed is False:
            self.argpars.error(
                "Missing --job-xml <filename>.  See --help for more information."
            )
    ...
    def load_xslt(self):
        ...
        try:
                ...
                if len(self.xslt_nodes) > 1:
                    for n in self.xslt_nodes:
                        if self.xslt_name is None and n.attrib.get("name") is None:
                            ...
                            self.xslt_name is not None
                            and n.attrib.get("name") is not None
                        ):
            ...
            if self.xsltfile is None:
                ...
                if self.xslt_name:
                    raise Exception(
                        "No matching <xslt/> tag was found for the named XSLT '%s'."
                        % self.xslt_name
                    )
                else:
            ...
            if not no_error_print:
                print(
                    "** ERROR ** Failed to parse the XSLT template: %s" % self.xsltfile
                )
            raise e
            ...
            while arg is not None:
                if arg["value"]:
                    ...
                    if arg["tagvaluetype"] == "value":
                        tag_element.text = self.__format_xml_value(
    ...
    def save_internal_xml(self, filename):
        ...
        with open(filename, "wb") as fd:
            fd.write(
                etree.tostring(self.internalxml, encoding="utf-8", pretty_print=True)
            )
...
class JobDefaults(configparser.ConfigParser):
    ...
    def __init__(self):
        ...
        deffiles = (
            "%s/.beaker-client/bks-defaults" % os.path.expanduser("~"),
            "bks-defaults",
        )
        self.read(deffiles)
    ...
    def get_job_defaults(self, job_file_name):
        ...
        try:
            ...
            section = (
                self.vars["profile"] is not None
                and "%s:%s" % (file_name, self.vars["profile"])
                or file_name
            )
            ...
            if self.vars["profile"] is not None:
                ...
                raise OptionValueError(
                    "There is no defaults profile '%s' defined for %s"
        ...
        if not dryrun:
            ...
            try:
                submitted_jobs.append(
                    self.hub.jobs.upload(self.job_cfg.get_beaker_xml_string())
                )
                print("Success")
