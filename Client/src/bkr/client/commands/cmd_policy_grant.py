...
class Policy_Grant(BeakerCommand):
    ...
    def options(self):
        ...
        self.parser.add_option(
            "--system", metavar="FQDN", help="Modify custom access policy for FQDN"
        )
        self.parser.add_option(
            "--pool", metavar="NAME", help="Modify access policy for system pool NAME"
        )
        self.parser.add_option(
            "--permission",
            metavar="PERMISSION",
    ...
    def run(
        ...
        **kwargs
        ...
        if (not system and not pool) or (system and pool):
            self.parser.error(
                "Specify system using --system or system pool using --pool"
            )
        if not permissions:
            self.parser.error(
                "Specify at least one permission to grant using --permission"
            )
        ...
        for permission in permissions:
            for user in users:
                res = requests_session.post(
                    rules_url,
                    json=dict(
                        permission=permission, user=user, group=None, everybody=False
                    ),
                )
            ...
            for group in groups:
                res = requests_session.post(
                    rules_url,
                    json=dict(
                        permission=permission, group=group, user=None, everybody=False
                    ),
                )
                res.raise_for_status()
            if everybody:
                res = requests_session.post(
                    rules_url,
                    json=dict(
                        permission=permission, user=None, group=None, everybody=True
                    ),
                )
                res.raise_for_status()
