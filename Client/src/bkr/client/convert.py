...
class Convert(object):
    ...
    def toxml(self):
        self.handle_invalid(
            self.jobxml.getElementsByTagName("job"), self.invalidjobtags
        )
        self.handle_invalid(
            self.jobxml.getElementsByTagName("recipe"), self.invalidrecipetags
        )
        self.handle_invalid(
            self.jobxml.getElementsByTagName("guestrecipe"), self.invalidrecipetags
        )
        ...
        for recipe in recipes:
            ...
            for child in recipe.childNodes:
                if child.nodeType == child.ELEMENT_NODE and child.tagName == "bootargs":
                    ...
                    kernel_options = "%s%s" % (
                        kernel_options,
                        self.getText(child.childNodes),
                    )
                    recipe.setAttribute("kernel_options", kernel_options)
                if (
                    child.nodeType == child.ELEMENT_NODE
                    and child.tagName == "distroRequires"
                ):
                    del_nodes.append(child)
                ...
                if (
                    child.nodeType == child.ELEMENT_NODE
                    and child.tagName == "hostRequires"
                ):
                    del_nodes.append(child)
                ...
                if (
                    child.nodeType == child.ELEMENT_NODE
                    and child.tagName == "installPackage"
                ):
                    del_nodes.append(child)
