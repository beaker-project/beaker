...
class BeakerOptionParser(CommandOptionParser):
    standard_option_list = [
        ...
        Option("--proxy-user", help=SUPPRESS_HELP),
    ]
...
def main():
    ...
    if not cmd_opts.hub and not conf:
        sys.stderr.write(
            "Configuration file not found. Please create an /etc/beaker/client.conf "
    ...
    try:
        ...
        if e.min_code == krb5krb_ap_err_tkt_expired:  # pylint: disable=no-member
            ...
            sys.stderr.write(
                "No Kerberos credential cache file found (run kinit to create one)\n"
            )
            return 1
        elif e.min_code == kg_empty_ccache:  # pylint: disable=no-member
            sys.stderr.write(
                "Kerberos credential cache is empty (run kinit to create one)\n"
            )
            return 1
        else:
            raise
