...
def host_filter_presets():
    ...
    config_files = sorted(
        glob.glob(pkg_resources.resource_filename("bkr.client", "host-filters/*.conf"))
...
class BeakerCommand(Command):
    ...
    def set_hub(self, username=None, password=None, **kwargs):
        ...
        self.container.set_hub(
            username, password, auto_login=self.requires_login, proxy_user=proxy_user
        )
    ...
    def requests_session(self):
        ...
        class BeakerClientRequestsSession(requests.Session):
            ...
            def __init__(self):
                super(
                    BeakerClientRequestsSession, self
    ...
    t_id_types = dict(
        T="RecipeTask", TR="RecipeTaskResult", R="Recipe", RS="RecipeSet", J="Job"
    )
...
class BeakerWorkflow(BeakerCommand):
    ...
    def options(self):
        ...
        distro_options = OptionGroup(
            self.parser, "Options for selecting distro tree(s)"
        )
        ...
        system_options = OptionGroup(self.parser, "Options for selecting system(s)")
        ...
        # for compat only
        task_options.add_option("--type", action="append", help=optparse.SUPPRESS_HELP)
        ...
        job_options.add_option(
            "--job-group", metavar="GROUPNAME", help="Associate a group to this job"
        )
        job_options.add_option(
            "--whiteboard",
            default="",
        ...
        installation_options.add_option(
            ...
            default=[],
            help=(
                "Configure repo at <URL> in the kickstart. The repo "
        ...
        installation_options.add_option(
            ...
            action="append",
            help=(
                "Configure repo at <URL> as part of kickstart %post "
                "execution. The repo will NOT be available during "
                "initial package installation."
            ),
        )
        # for compat only
        installation_options.add_option("--kernel_options", help=optparse.SUPPRESS_HELP)
        installation_options.add_option(
            "--kernel_options_post", help=optparse.SUPPRESS_HELP
        )
    ...
    def get_arches(self, *args, **kwargs):
        ...
        if distro:
            ...
            return self.hub.distros.get_arch(
                dict(osmajor=family, tags=tags, variant=variant)
            )
    ...
    def get_tasks(self, *args, **kwargs):
        ...
        if task_names:
            for task in self.hub.tasks.filter(
                dict(names=task_names, osmajor=filter["osmajor"])
            ):
        ...
        **kwargs
        ...
        if actualTasks or reserve or kwargs.get("allow_empty_recipe", False):
            ...
            if ksTemplate:
                ...
                kernel_options = (
                    recipe.kernel_options + " " + generateKernelOptions(ksTemplate)
                )
    ...
    def add_recipe_set(self, recipeSet=None):
        ...
        if recipeSet:
            if isinstance(recipeSet, BeakerRecipeSet):
                ...
                raise TypeError(
                    "recipeSet must be BeakerRecipeSet or xml.dom.minidom.Element"
                )
    ...
    def add_recipe(self, recipe=None):
        ...
        if recipe:
            if isinstance(recipe, BeakerRecipe):
                ...
                raise TypeError(
                    "recipe must be BeakerRecipe or xml.dom.minidom.Element"
                )
...
class BeakerRecipeBase(BeakerBase):
    ...
    def _addBaseHostRequires(self, **kwargs):
        ...
        if machine and force:
            ...
            for opt in ["hostrequire", "keyvalue", "random", "systype", "host_filter"]:
            ...
            if host_filter:
                ...
                if host_filter_expanded:
                    self.addHostRequires(
                        xml.dom.minidom.parseString(
                            host_filter_expanded
                        ).documentElement
                    )
                else:
                    sys.stderr.write(
                        "Pre-defined host-filter does not exist: %s\n" % host_filter
                    )
                    sys.exit(1)
...
""" % dict(
    ...
    def add_partition(self, name=None, type=None, fs=None, size=None):
        ...
        if name:
            ...
            raise BeakerJobTemplateError(
                u"You must specify name when adding a partition"
            )
        if size:
            partition.setAttribute("size", str(size))
        else:
            raise BeakerJobTemplateError(
                u"You must specify size when adding a partition"
            )
        if type:
    ...
    def get_and_distro_requires(self):
        return self.node.getElementsByTagName("distroRequires")[0].getElementsByTagName(
            "and"
        )[0]
...
class BeakerRecipe(BeakerRecipeBase):
    ...
    def add_guest_recipe(self, guestrecipe):
        ...
        if isinstance(guestrecipe, BeakerGuestRecipe):
            ...
            isinstance(guestrecipe, xml.dom.minidom.Element)
